...
const CONTENT_ZH = {
  ...
  roadmap: {
    ...
    architecture_points: [
      ...
      {
        title: "事件与任务",
        desc: "Cron 任务、事件引擎摘要、`/missed` 回查、通知偏好与渠道投递共享 Rust 后端、SQLite/JSON 或 PG 执行历史和用户归属模型；Web scheduler 结果先落入会话历史，SSE 只负责在线实时提示，因此浏览器离线不会把已落库结果误记为送达失败，执行错误会写入产品化失败提示并通过同一 `scheduled_message` 事件推送给在线控制台；MCP/ACP runner 现在使用绝对 `HONE_CONFIG_PATH`，并在父进程没有显式 `HONE_DATA_DIR` 时从 runtime dir 反推数据根，目标是让 Feishu / Web / scheduler 工具读取同一份 Cron 与持仓数据；但最新 bug ledger 仍在跟踪 Feishu direct Cron 作用域读空的 P1 运行态复现，公开文案不把这条链路表述为完全闭环；Feishu 等渠道的 scheduler heartbeat 已补齐 revision-aware 重复抑制、stale running 行终结、cloud cron 操作超时保护与 listener 内 scheduler loop 监督重启，event-engine 默认 LLM 配置已切到当前可用的 `x-ai/grok-4.3`，避免继续依赖已下线的 Grok 4.1 Fast。",
      },
    ],
    ...
    now: {
      ...
      items: [
        ...
        "MCP / ACP 子进程运行时边界已收口：`hone-cli start` 会显式传递 `HONE_MCP_BIN`，runner 请求使用绝对配置路径，MCP server 会继承或反推出同一份 `HONE_DATA_DIR`。该改动用于收敛 sandbox cwd 下误读空数据树的问题；Feishu direct Cron 作用域读空仍有活跃 P1 运行态复现，继续以 bug ledger 为准",
