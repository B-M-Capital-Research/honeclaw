...
import {
  ...
  reviewHistoricalOutcomeValidationEvaluationFirstExecutionAuthorization,
} from "@/lib/api";
import type {
  HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationRegistry,
  HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationVerdict,
} from "@/lib/types";

const AUTHORIZATION_CHECKS = [
  "已逐项核对精确 Stage 61 runner、Stage 60/59/58/57、九个候选工件与完整哈希链",
  "复核者不属于评估实现登记/复核、runner 登记或完整上游的任何历史角色",
  "已经独立重算 runner 工件 SHA-256，结果与登记摘要完全一致",
  "不可变代码版本可复现，绑定实现与 runner 工件仍可用且未被替换",
  "未来只能只读挂载精确 validation features/labels 和九个候选；当前不挂载任何输入",
  ...
  "固定零预测/岭回归/梯度提升三臂、17/29/43、65 项特征、九项目标与逐目标逐种子协议未变化",
  "validation 只评估，禁止训练或预处理更新；sealed holdout features/labels 永久隐藏",
  "未来只读挂载必须精确绑定 validation 分区和九候选，禁止读取其它训练、标签或生产数据",
  "授权只在 24 小时内有效，最多消费一次",
  "授权、claim、执行、输出独立校验与候选选择职责严格分离",
  "本复核不读取数据、不评估、不选模、不写模型/指标库，也不授权奖励、影子、订单、券商或交易",
] as const;

export function PublicAdminHistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationPanel() {
  ...
  const [verdict, setVerdict] =
    createSignal<HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationVerdict>(
      "changes_requested",
    );
  ...
  const load = async () => {
    try {
      const next = await getHistoricalOutcomeValidationEvaluationFirstExecutionAuthorizations();
      setRegistry(next);
      ...
      setError(cause instanceof Error ? cause.message : "validation 评估首次执行授权复核读取失败");
    }
  };
  ...
  const approvalSelected = createMemo(
    () => verdict() === "approved_for_one_future_isolated_validation_evaluation_invocation",
  );
  ...
  const submit = async () => {
    ...
    try {
      const next =
        await reviewHistoricalOutcomeValidationEvaluationFirstExecutionAuthorization(
          runner.isolated_runner_id,
          {
            ...
            future_exact_read_only_validation_and_candidate_mounts_confirmed: confirmed[4],
      ...
      setNotice(
        approvalSelected()
          ? "已追加独立批准：24 小时内最多允许一次未来隔离 validation 评估调用；尚未 claim、未读取标签、未评估或选模。"
          : "已追加复核记录；没有授予未来调用资格。",
      );
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "validation 评估首次执行授权复核失败");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <Show when={registry()}>
      {(currentRegistry) => (
        <section class="public-admin-reward-governance" aria-label="validation 评估首次执行授权复核">
          <header>
            <strong>第 62 阶段 · validation 评估首次执行授权复核</strong>
          ...
          <article class="public-admin-reward-governance">
            ...
            <p>批准只建立短期的一次性未来资格。本页没有 claim 或执行按钮，不启动进程、不挂载 validation 标签，也不评估或选择候选。</p>
            <p class="public-admin-anchor-boundary">下一门禁只能是另行开发和复核的第 63 阶段 claim-first 单次隔离评估尝试；sealed holdout、模型/指标库、奖励、影子组合、订单、券商和交易全部关闭。</p>
          </article>
          ...
          <Show when={currentRegistry().items.length > 0}>
            ...
            <label>
              <span>复核结论</span>
              <select value={verdict()} onChange={(event) => setVerdict(event.currentTarget.value as HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationVerdict)}>
                <option value="changes_requested">要求修改</option>
                <option value="rejected">拒绝</option>
                <option value="approved_for_one_future_isolated_validation_evaluation_invocation">批准未来单次隔离 validation 评估调用</option>
              </select>
            </label>
            <label><span>复核依据</span><textarea value={rationale()} onInput={(event) => setRationale(event.currentTarget.value)} placeholder="记录独立重算、完整上游、九候选、validation 只读挂载与 sealed holdout 边界" /></label>
          ...
          <For each={currentRegistry().items}>
            {(item) => (
              <article class="public-admin-reward-governance">
                ...
                <p class="public-admin-anchor-boundary">本页不能 claim 或执行；validation 标签读取、评估、选模、sealed holdout、模型/指标库、输出校验、奖励、影子、订单、券商和交易仍全部关闭。</p>
              </article>
            )}
          </For>
        </section>
