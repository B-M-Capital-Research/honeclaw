...
export function InfluencerDigestDashboard(props: Props) {
  ...
  const send = () => { const s = snapshot(); const q = question().trim(); if (!s || !q) return; const report = { reportDate: s.report_date, generatedAtBJT: s.generated_at_beijing, status: s.status, authors: s.authors, items: s.items.map(({ author_name, public_handle, title, published_at_beijing, source_url, aggregation_source, aggregation_url, post_kind, summary, stance, horizon, content_type, topics, tickers, counterpoint, analysis_status }) => ({ author_name, public_handle, title, published_at_beijing, source_url, aggregation_source, aggregation_url, post_kind, summary, stance, horizon, content_type, topics, tickers, counterpoint, analysis_status })) }; props.onAsk(`<!-- HONE_SAVED_INFLUENCER_DIGEST\n${JSON.stringify(report)}\nEND_HONE_SAVED_INFLUENCER_DIGEST -->\n基于已保存的大V速报回答：${q}\n要求：作者观点不等于事实或 HONE 结论；附原文链接与北京时间；聚合翻译不是独立事实来源；不得补造未配置作者内容，不得把观点直接转换为买卖或仓位动作。`); setOpen(false); setQuestion(""); };
  return <>
    ...
    <Show when={open()}><Portal><div class="influencer-digest-backdrop" onClick={() => setOpen(false)}><section class="influencer-digest-dialog" role="dialog" aria-modal="true" aria-labelledby="influencer-title" onClick={(e) => e.stopPropagation()}>
      ...
      <div class="influencer-digest-meta"><b>{statusLabel(snapshot()?.status ?? "")}</b><span>报告日 {snapshot()?.report_date ?? "—"}</span><span>{snapshot()?.generated_at_beijing ?? "—"} 北京时间</span><button onClick={() => void load()}>重新读取</button></div>
