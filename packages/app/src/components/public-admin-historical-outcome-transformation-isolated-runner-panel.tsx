...
import {
  getHistoricalOutcomeOfflineDatasetTransformationIsolatedRunners,
  registerHistoricalOutcomeOfflineDatasetTransformationIsolatedRunner,
} from "@/lib/api";
import type {
  HistoricalOutcomeOfflineDatasetTransformationIsolatedRunnerKind,
...
const REGISTRATION_CHECKS = [
  ...
  "运行时身份和单 subject、2048 MiB 等静态资源上限已经固定",
  "没有入口、环境继承/变量、密钥、网络、工具、子进程或生产访问",
  "runner 登记、首次执行授权和输出校验严格分开",
  "不生成 manifest/bundle、不 join、不写目标、不训练、奖励、影子、订单、券商或交易",
] as const;

export function PublicAdminHistoricalOutcomeTransformationIsolatedRunnerPanel() {
  const [registry, setRegistry] =
    createSignal<HistoricalOutcomeOfflineDatasetTransformationIsolatedRunnerRegistry>();
  ...
  const [runnerKind, setRunnerKind] =
    createSignal<HistoricalOutcomeOfflineDatasetTransformationIsolatedRunnerKind>(
  ...
  const load = async () => {
    try {
      const next =
        await getHistoricalOutcomeOfflineDatasetTransformationIsolatedRunners();
      setRegistry(next);
      ...
      setError(cause instanceof Error ? cause.message : "隔离转换 runner 规范登记表读取失败");
    }
  };
  ...
  const submit = async () => {
    ...
    try {
      const next =
        await registerHistoricalOutcomeOfflineDatasetTransformationIsolatedRunner({
      ...
      setError(cause instanceof Error ? cause.message : "隔离转换 runner 规范登记失败");
    } finally {
      setBusy(false);
    }
  };
    ...
    <Show when={registry()}>
      {(currentRegistry) => (
        <section
          ...
          aria-label="隔离转换 runner 规范登记"
        >
          <header>
            <strong>第 29 阶段 · 隔离转换 runner 规范登记</strong>
          ...
          <article class="public-admin-reward-governance">
            ...
            <p>runner 登记只冻结工件、代码、运行时、只读输入、create-once 内容寻址输出与资源上限。它不会创建运行目录或转换任何数据。</p>
          ...
          <Show when={currentRegistry().registration_allowed}>
            ...
            <label><span>runner 名称</span><input value={runnerName()} onInput={(event) => setRunnerName(event.currentTarget.value)} placeholder="例如：单 subject 确定性转换 runner" /></label>
            <label>
              <span>runner 类型</span>
              <select value={runnerKind()} onChange={(event) => setRunnerKind(event.currentTarget.value as HistoricalOutcomeOfflineDatasetTransformationIsolatedRunnerKind)}>
