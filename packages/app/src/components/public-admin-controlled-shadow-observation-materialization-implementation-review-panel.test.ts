...
const source = readFileSync(
  new URL(
    "./public-admin-controlled-shadow-observation-materialization-implementation-review-panel.tsx",
    import.meta.url,
  ),
  "utf8",
);

describe("Stage 108 observation materialization implementation chain-external review panel", () => {
  it("independently binds implementation, contract, audit, review, registration and specification", () => {
    expect(source).toContain("第 108 阶段");
  ...
  it("keeps approval zero-capability and bounded to Stage 109 specification registration", () => {
    ...
    expect(source).toContain("approval_only_opens_future_isolated_observation_materialization_runner_specification_registration_confirmed");
    expect(source).toContain("Stage 109 runner 规格登记资格");
    expect(source).toContain("仍无 runner、输入读取、观察或交易权限");
  });
});
