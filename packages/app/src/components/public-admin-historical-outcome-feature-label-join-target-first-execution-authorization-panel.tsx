...
import {
  getHistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizations,
  reviewHistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorization,
} from "@/lib/api";
import type {
  HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationRegistry,
  HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationVerdict,
} from "@/lib/types";

const AUTHORIZATION_CHECKS = [
  "已逐项核对精确 runner、实现、两级独立复核、join/target 规范、正式工件与数据集完整哈希链",
  "复核者不属于规范、实现、此前复核或 runner 登记的任何历史角色",
  "已经独立重算 runner 工件 SHA-256，结果与登记摘要完全一致",
  "不可变代码版本可复现，绑定实现与 runner 工件仍可用且未被替换",
  "所有绑定输入和根文件系统只读，不允许读取未绑定数据",
  "进程必须非特权运行并启用 no-new-privileges",
  "输出只允许写入一次性隔离目录，内容寻址、create-once 且必须另行独立校验",
  "运行时身份与单数据集、4096 MiB、300 秒、单进程等资源上限固定",
  "不继承宿主环境，不注入环境变量或密钥",
  "无网络、工具、子进程、生产读写或历史状态修改能力",
  "一对一 fail-closed join、九项原始 f64 目标与 canonical schema/序列化合同未变化",
  "时点可得性、显式缺失、purge/embargo、split 隔离与 sealed holdout 均已独立确认",
  "不得访问通用标签库或训练库，只能使用精确绑定的只读输入",
  "授权只在 24 小时内有效，最多消费一次",
  "授权、claim、执行、输出校验、训练数据准入与训练职责严格分离",
  "不授权 manifest/bundle/join 结果、训练、奖励、影子、订单、券商或交易",
] as const;

export function PublicAdminHistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationPanel() {
  const [registry, setRegistry] =
    createSignal<HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationRegistry>();
  const [selectedRunnerId, setSelectedRunnerId] = createSignal("");
  const [verdict, setVerdict] =
    createSignal<HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationVerdict>(
      "changes_requested",
    );
  ...
  const load = async () => {
    try {
      const next = await getHistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizations();
      setRegistry(next);
      ...
      setError(cause instanceof Error ? cause.message : "join/target 首次执行授权复核读取失败");
    }
  };
  ...
  const approvalSelected = createMemo(
    () => verdict() === "approved_for_one_future_isolated_join_target_invocation",
  );
  ...
  const submit = async () => {
    ...
    try {
      const next =
        await reviewHistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorization(
          runner.isolated_runner_id,
          {
            ...
            expected_implementation_review_contract_sha256:
              implementationReview.review_contract.contract_sha256,
            expected_implementation_independent_audit_sha256:
              implementationReview.independent_audit.audit_sha256,
            expected_specification_review_id: specificationReview.review_id,
            ...
            deterministic_one_to_one_join_nine_target_and_canonical_schema_contract_confirmed:
              confirmed[10],
            point_in_time_missingness_purge_embargo_split_and_sealed_holdout_confirmed:
              confirmed[11],
            no_generic_label_or_training_store_access_confirmed: confirmed[12],
      ...
      setNotice(
        approvalSelected()
          ? "已追加独立批准：24 小时内最多允许一次未来隔离调用；尚未 claim、未执行、未读取标签，也未生成 joined rows。"
          : "已追加复核记录；没有授予未来调用资格。",
      );
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "join/target 首次执行授权复核失败");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <Show when={registry()}>
      {(currentRegistry) => (
        <section class="public-admin-reward-governance" aria-label="join/target 首次执行授权复核">
          <header>
            <strong>第 41 阶段 · join/target 首次执行授权复核</strong>
          ...
          <article class="public-admin-reward-governance">
            ...
            <p>批准只建立短期的一次性未来资格。本页没有 claim 或执行按钮，不启动进程，不读取通用标签库，也不创建输出。</p>
            <p class="public-admin-anchor-boundary">下一门禁只能是另行开发和复核的第 42 阶段单次隔离执行尝试；当前 joined rows、训练数据、奖励、影子组合、订单、券商和交易全部关闭。</p>
          </article>
          ...
          <Show when={currentRegistry().items.length > 0}>
            ...
            <label>
              <span>复核结论</span>
              <select value={verdict()} onChange={(event) => setVerdict(event.currentTarget.value as HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationVerdict)}>
                <option value="changes_requested">要求修改</option>
                <option value="rejected">拒绝</option>
                <option value="approved_for_one_future_isolated_join_target_invocation">批准未来单次隔离调用</option>
              </select>
            </label>
            <label><span>复核依据</span><textarea value={rationale()} onInput={(event) => setRationale(event.currentTarget.value)} placeholder="记录独立重算、上游链、九项目标、PIT/holdout 与沙箱边界" /></label>
          ...
          <For each={currentRegistry().items}>
            {(item) => (
              <article class="public-admin-reward-governance">
                ...
                <p class="public-admin-anchor-boundary">本页不能 claim 或执行；通用标签库、join 输出、目标落库、训练数据、训练、奖励、影子、订单、券商和交易仍全部关闭。</p>
              </article>
            )}
          </For>
        </section>
