...
import {
  ...
  reviewHistoricalOutcomeTrainingFirstExecutionAuthorization,
} from "@/lib/api";
import type {
  HistoricalOutcomeTrainingFirstExecutionAuthorizationRegistry,
  HistoricalOutcomeTrainingFirstExecutionAuthorizationVerdict,
} from "@/lib/types";

const AUTHORIZATION_CHECKS = [
  "已逐项核对精确 runner、Stage 54/53/52/51、实现工件、训练副本、rows 与 targets 完整哈希链",
  "复核者不属于实现登记、实现复核、runner 登记或完整上游的任何历史角色",
  ...
  "固定零预测/岭回归/梯度提升三臂、17/29/43、65 项特征、九项目标与逐目标逐种子指标未变化",
  "train 只拟合、validation 只选模，sealed holdout 对拟合与选择 worker 隐藏",
  "未来只能只读挂载精确 training-store dataset，当前不得读取任何训练或其它数据",
  "授权只在 24 小时内有效，最多消费一次",
  "授权、claim、执行、输出校验、训练数据准入与训练职责严格分离",
  "本复核不读取数据、不训练、不创建模型或指标，也不授权奖励、影子、订单、券商或交易",
] as const;

export function PublicAdminHistoricalOutcomeTrainingFirstExecutionAuthorizationPanel() {
  ...
  const [verdict, setVerdict] =
    createSignal<HistoricalOutcomeTrainingFirstExecutionAuthorizationVerdict>(
      "changes_requested",
    );
  ...
  const load = async () => {
    try {
      const next = await getHistoricalOutcomeTrainingFirstExecutionAuthorizations();
      setRegistry(next);
      ...
      setError(cause instanceof Error ? cause.message : "训练首次执行授权复核读取失败");
    }
  };
  ...
  const approvalSelected = createMemo(
    () => verdict() === "approved_for_one_future_isolated_training_invocation",
  );
  ...
  const submit = async () => {
    ...
    try {
      const next =
        await reviewHistoricalOutcomeTrainingFirstExecutionAuthorization(
          runner.isolated_runner_id,
          {
            ...
            fixed_three_arm_three_seed_sixty_five_feature_nine_target_suite_confirmed:
              confirmed[10],
            train_validation_and_sealed_holdout_isolation_confirmed: confirmed[11],
      ...
      setNotice(
        approvalSelected()
          ? "已追加独立批准：24 小时内最多允许一次未来隔离训练调用；尚未 claim、未读取数据、未训练，也未生成模型或指标。"
          : "已追加复核记录；没有授予未来调用资格。",
      );
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "训练首次执行授权复核失败");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <Show when={registry()}>
      {(currentRegistry) => (
        <section class="public-admin-reward-governance" aria-label="训练首次执行授权复核">
          <header>
            <strong>第 56 阶段 · 训练首次执行授权复核</strong>
          ...
          <article class="public-admin-reward-governance">
            ...
            <p>批准只建立短期的一次性未来资格。本页没有 claim 或执行按钮，不启动进程、不读取训练副本，也不创建模型或指标。</p>
            <p class="public-admin-anchor-boundary">下一门禁只能是另行开发和复核的第 57 阶段单次隔离训练尝试；当前训练、输出校验、奖励、影子组合、订单、券商和交易全部关闭。</p>
          </article>
          ...
          <Show when={currentRegistry().items.length > 0}>
            ...
            <label>
              <span>复核结论</span>
              <select value={verdict()} onChange={(event) => setVerdict(event.currentTarget.value as HistoricalOutcomeTrainingFirstExecutionAuthorizationVerdict)}>
                <option value="changes_requested">要求修改</option>
                <option value="rejected">拒绝</option>
                <option value="approved_for_one_future_isolated_training_invocation">批准未来单次隔离训练调用</option>
              </select>
            </label>
            <label><span>复核依据</span><textarea value={rationale()} onInput={(event) => setRationale(event.currentTarget.value)} placeholder="记录独立重算、完整上游、三臂三种子、训练副本与 holdout 沙箱边界" /></label>
          ...
          <For each={currentRegistry().items}>
            {(item) => (
              <article class="public-admin-reward-governance">
                ...
                <p class="public-admin-anchor-boundary">本页不能 claim 或执行；训练数据读取、训练、模型、指标、输出校验、奖励、影子、订单、券商和交易仍全部关闭。</p>
              </article>
            )}
          </For>
        </section>
