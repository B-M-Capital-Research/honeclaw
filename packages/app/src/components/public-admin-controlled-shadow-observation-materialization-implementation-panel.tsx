...
const CHECKS = [
  "精确绑定 Stage 51–106 当前不可变责任链",
  "登记者不是 Stage 106 复核者或此前完整责任链成员",
  "独立重算 review、registration、specification 与 audit 指纹",
  "只登记零能力契约，不提交源码或可执行制品",
  "未来唯一输入仍是 Stage 104 已准入输出",
  "保留官方交易日、标的/SPY 与三价格口径矩阵",
  "标的缺口显式，SPY 缺口、重复和越界仍失败关闭",
  "分红、拆股和三类价格口径保持分离",
  "保留十进制定标、规范排序、行哈希和内容寻址输出",
  "初始影子分配绑定不重算，也不应用会计状态转换",
  "保守 available_at 与未验证 provider 发布时间限制保持不变",
  "单一信封 create-once，不覆盖、回填、前填、插值或替代",
  "未来输出仍属非可信，必须独立校验",
  "没有入口、runtime、输入挂载/读取、环境、秘密、网络、工具、子进程或生产 I/O",
  "不生成观察、账本、持仓、绩效、模型、训练、奖励、订单、券商或交易事实",
  "隔离 runner 登记前必须先完成 Stage 108 责任链外独立实现复核",
...
export function PublicAdminControlledShadowObservationMaterializationImplementationPanel() {
  const [registry, setRegistry] =
    createSignal<ControlledShadowObservationMaterializationImplementationRegistry>();
  ...
  const load = async () => {
    try {
      const next = await getControlledShadowObservationMaterializationImplementations();
      setRegistry(next);
      ...
      setError(cause instanceof Error ? cause.message : "Stage 107 零能力实现登记表读取失败");
    }
  };
  onMount(() => void load());
  ...
  const submit = async () => {
    ...
    const request: RegisterControlledShadowObservationMaterializationImplementationRequest = {
    ...
    setNotice("");
    try {
      setRegistry(await registerControlledShadowObservationMaterializationImplementationOnce(review.review_id, request));
      setFields(emptyFields());
      setChecks(CHECKS.map(() => false));
      setNotice("Stage 107 零能力实现契约已 create-once 写入；仍须 Stage 108 责任链外独立实现复核。");
      await load();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Stage 107 零能力实现契约登记失败");
      await load();
    } finally {
      setBusy(false);
    }
  ...
  const textFields = [
    ...
    ["deterministic_projection_semantics", "确定性投影语义"],
    ["session_price_basis_and_gap_semantics", "交易日、价格口径与缺口语义"],
    ["corporate_action_decimal_order_and_hash_semantics", "公司行动、十进制、排序与哈希语义"],
    ["initial_allocation_and_availability_semantics", "初始分配与可用时间语义"],
    ["error_and_missing_data_semantics", "错误与缺失数据语义"],
    ["known_limitations", "已知限制"],
    ["future_review_constraints", "Stage 108 独立复核约束"],
  ] as const;

  return <Show when={registry()}>{(current) => (
    <section class="public-admin-reward-governance" aria-label="Stage 107 观察物化零能力实现契约登记">
      <header><strong>第 107 阶段 · 观察物化零能力实现契约登记</strong><span>{current().implementation_status}</span></header>
      <p>{current().scope}</p>
      <div class="public-admin-decision-metrics">
        ...
        <div><span>待 Stage 108 复核</span><strong>{current().independent_implementation_review_eligible_count}</strong></div>
      </div>
      <p class="public-admin-anchor-boundary">这里只冻结纯函数标识、schema、哈希和失败关闭语义；没有源码、可执行入口、runtime、输入读取或观察输出。</p>
      <Show when={current().registration_eligible_count > 0} fallback={<p>当前没有待登记的 Stage 106 独立批准规格。</p>}>
        <label><span>Stage 106 独立批准规格</span><select value={selectedId()} onChange={(event) => setSelectedId(event.currentTarget.value)}><For each={current().items.filter((item) => item.registration_eligible)}>{(item) => <option value={item.specification_review.review_id}>{item.specification_review.review_id.slice(0, 12)}… · {item.specification_registration.specification.subject_symbols.join("、")}</option>}</For></select></label>
