...
const CHECKS = [
  "精确绑定 Stage 51–108 当前不可变责任链",
  "登记人独立于 Stage 108 和完整此前责任链",
  "已复算实现、复核、审计、规格与 Stage 104 准入全链哈希",
  "未来工件身份、代码版本和复现步骤已绑定，但当前工件不存在",
  "八个观察物化函数身份和规范化 schema 保持不变",
  "未来输入只允许 Stage 104 已准入、只读、内容寻址的精确输出",
  ...
  "登记只开放 Stage 110 责任链外首次执行授权复核资格",
  "没有把未确认 Hari/老王观点写成系统规则",
] as const;

const initialFields = () => ({
  runner_name: "自然前向观察物化隔离 runner",
...
export function PublicAdminControlledShadowObservationMaterializationIsolatedRunnerPanel() {
  const [registry, setRegistry] =
    createSignal<ControlledShadowObservationMaterializationIsolatedRunnerRegistry>();
  ...
  const load = async () => {
    try {
      const next = await getControlledShadowObservationMaterializationIsolatedRunners();
      setRegistry(next);
      ...
      setError(cause instanceof Error ? cause.message : "Stage 109 runner 规格登记表读取失败");
    }
  };
  onMount(() => void load());
  ...
  const submit = async () => {
    ...
    const request: RegisterControlledShadowObservationMaterializationIsolatedRunnerRequest = {
    ...
    try {
      setRegistry(
        await registerControlledShadowObservationMaterializationIsolatedRunnerOnce(
      ...
      setNotice("Stage 109 runner 规格已 create-once 登记；当前仍没有工件、入口、runtime 或数据访问能力。");
      await load();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Stage 109 runner 规格登记失败");
      await load();
    } finally {
      setBusy(false);
    }
  ...
  return (
    <Show when={registry()}>
      {(current) => (
        <section class="public-admin-reward-governance" aria-label="Stage 109 观察物化隔离 runner 规格登记">
          <header>
            <strong>第 109 阶段 · 观察物化隔离 runner 规格登记</strong>
          ...
          <div class="public-admin-decision-metrics">
            ...
            <div><span>待 Stage 110</span><strong>{current().first_execution_authorization_review_eligible_count}</strong></div>
          </div>
          <p class="public-admin-anchor-boundary">
            这里只冻结未来 runner 身份与沙箱合同。登记后仍没有观察物化工件、入口、runtime、挂载或准入输入读取权限。
          </p>
          <Show when={current().registration_eligible_count > 0} fallback={<p>当前没有可登记的 Stage 108 独立批准实现。</p>}>
            <label>
              <span>Stage 108 独立批准实现</span>
            ...
            <button type="button" class="public-admin-decision-submit" disabled={disabled()} onClick={() => void submit()}>
              {busy() ? "正在登记…" : "写入 Stage 109 create-once 规格"}
            </button>
          </Show>
          ...
          <For each={current().items}>
            {(item) => (
              <article class="public-admin-decision-card">
                ...
                <p>工件存在：否 · runtime 实例化：否 · 输入读取：否 · 下单/券商/交易：否</p>
              </article>
            )}
          </For>
        </section>
