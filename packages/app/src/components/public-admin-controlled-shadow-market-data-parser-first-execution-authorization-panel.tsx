...
import type {
  ...
  ControlledShadowMarketDataParserFirstExecutionAuthorizationVerdict,
} from "@/lib/types";

const REVIEW_CHECKS = [
  "精确绑定当前 Stage 51–99 完整责任链",
  "复核者独立于 Stage 99 登记人、工件构建者和全部上游角色",
  "服务端已重新读取只读常规工件并计算 SHA-256，摘要和长度完全匹配",
  "自哈希 manifest、代码版本、runtime 与复现步骤摘要完全匹配",
  "工件构建者与 Stage 100 复核者相互分离",
  "八个 parser 函数与 canonical schema 继续由 Stage 99 合同绑定",
  "显式来源、交易日历、公司行动、数值与失败关闭语义保持不变",
  "禁止静默去重、前填、插值、回退或推断公司行动",
  "固定无特权身份、只读根目录、临时工作区和资源上限保持不变",
  "未来输入仅限 Stage 94 已验证、只读且内容寻址的收据载荷",
  "未来输出 create-once、非可信、独立验证且不含市场解释或订单意图",
  "source_available_at 仍未验证，必须由后续独立证据确认",
  "授权 24 小时、仅一次，并与 Stage 101 claim 严格分离",
  "当前无 runtime/entrypoint/挂载/载荷读取/parser 执行或解析行",
  "无环境继承、secret、网络、工具、子进程或生产读写",
  "无观察、账本、持仓、绩效、模型/指标、训练、reward、订单、券商或交易权限",
  "批准只开放未来 Stage 101 claim-first 单次尝试候选",
  "未把未确认 Hari/老王观点写成系统规则",
] as const;

export function PublicAdminControlledShadowMarketDataParserFirstExecutionAuthorizationPanel() {
  const [registry, setRegistry] =
    createSignal<ControlledShadowMarketDataParserFirstExecutionAuthorizationRegistry>();
  const [selectedRunnerId, setSelectedRunnerId] = createSignal("");
  const [verdict, setVerdict] =
    createSignal<ControlledShadowMarketDataParserFirstExecutionAuthorizationVerdict>(
  ...
  const load = async () => {
    try {
      const next = await getControlledShadowMarketDataParserFirstExecutionAuthorizations();
      setRegistry(next);
      ...
      setError(cause instanceof Error ? cause.message : "Stage 100 parser 首次执行授权表读取失败");
    }
  };
  ...
  const submit = async () => {
    ...
    try {
      const next = await reviewControlledShadowMarketDataParserFirstExecutionAuthorizationOnce(
        runner.isolated_runner_id,
        {
          ...
          expected_validation_sha256: implementationContract.validation_sha256,
          expected_receipt_sha256: implementationContract.receipt_sha256,
          expected_claim_sha256: implementationContract.claim_sha256,
          expected_result_sha256: implementationContract.result_sha256,
          ...
          exact_current_stage_51_through_stage_99_binding_confirmed: checks()[0] as boolean,
          reviewer_independent_from_stage_99_builder_and_complete_prior_chain_confirmed: checks()[1] as boolean,
          ...
          all_eight_parser_functions_and_canonical_schemas_remain_bound_confirmed: checks()[5] as boolean,
          strict_source_calendar_action_numeric_and_failure_semantics_preserved_confirmed: checks()[6] as boolean,
          no_deduplication_forward_fill_interpolation_fallback_or_inferred_actions_confirmed: checks()[7] as boolean,
          fixed_unprivileged_identity_read_only_root_ephemeral_workdir_and_resource_limits_confirmed: checks()[8] as boolean,
          future_input_only_stage_94_validated_read_only_content_addressed_receipt_payloads_confirmed: checks()[9] as boolean,
          future_output_create_once_untrusted_independently_validated_no_market_interpretation_or_order_intent_confirmed: checks()[10] as boolean,
          source_available_at_remains_unverified_until_separate_evidence_confirmed: checks()[11] as boolean,
          authorization_single_use_24_hour_expiry_and_stage_101_claim_separation_confirmed: checks()[12] as boolean,
          no_runtime_entrypoint_mount_payload_read_parser_execution_or_parsed_rows_confirmed: checks()[13] as boolean,
          no_environment_secret_network_tool_subprocess_or_production_io_confirmed: checks()[14] as boolean,
          no_observation_ledger_position_performance_model_metric_training_reward_order_broker_or_trading_confirmed: checks()[15] as boolean,
          approval_only_opens_future_stage_101_claim_first_attempt_confirmed: checks()[16] as boolean,
      ...
      setNotice(approving()
        ? "已签发 24 小时内一次性的未来 Stage 101 claim 候选；本次没有执行 parser 或读取行情。"
        : "复核已 append-only 保存；没有开放执行能力。");
      await load();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Stage 100 parser 首次执行授权复核失败");
      await load();
  ...
  return (
    <Show when={registry()}>{(current) => (
      <section class="public-admin-reward-governance" aria-label="行情解析器首次执行授权独立复核">
        <header><strong>第 100 阶段 · 行情解析器首次执行授权独立复核</strong><span>{current().authorization_status}</span></header>
        <p>{current().scope}</p>
        ...
        <article class="public-admin-reward-governance">
          ...
          <p class="public-admin-anchor-boundary">本阶段只审查工件身份和隔离合同，不创建入口、runtime、挂载、解析行或交易权限。</p>
        </article>
        <Show when={current().items.length > 0} fallback={<p>当前没有可进入 Stage 100 的 Stage 99 runner。</p>}>
          <label><span>Stage 99 runner</span><select value={selectedRunnerId()} onChange={(event) => setSelectedRunnerId(event.currentTarget.value)}>
          ...
          <label><span>复核结论</span><select value={verdict()} onChange={(event) => setVerdict(event.currentTarget.value as ControlledShadowMarketDataParserFirstExecutionAuthorizationVerdict)}>
            <option value="changes_requested_rebuild_artifact">要求修改并重建工件</option>
            <option value="rejected">拒绝</option>
            <option value="approved_for_one_future_claim_first_parser_attempt">批准未来一次 claim-first 尝试候选</option>
          </select></label>
          ...
          <button type="button" class="public-admin-decision-submit" disabled={disabled()} onClick={() => void submit()}>{busy() ? "正在保存复核…" : "保存 Stage 100 独立复核"}</button>
        </Show>
        ...
        <For each={current().items}>{(item) => <article class="public-admin-reward-governance">
          ...
          <p class="public-admin-anchor-boundary">{item.future_claim_eligible ? "仅获得未来 Stage 101 一次性 claim-first 候选，尚未执行。" : "当前没有执行资格。"}</p>
        </article>}</For>
      </section>
    )}</Show>
  );
