...
- Current conclusion: HONE exposes the existing USD 199.99/year subscription and the USD 229.99/12-month non-renewing pass, and creates a correct authenticated live fixed-term Checkout. No live payment was submitted. On 2026-08-13 Stripe still reported both wallets `available=false`; correction `b9051301` therefore makes the public offer list server-authoritative and card-only by default instead of advertising pending wallets. Its production deployment and external-Chrome screenshot are the current execution step.
