...
- 2026-07-28: Concurrent `main` work added the Whop activation surface after the first package was built, so production was rebuilt from the new head. That restart exposed a second independent operational defect: the CLI supervisor used the cloud-backed `/api/meta` endpoint for local child readiness, and its loopback `reqwest` client was still eligible for proxy/TUN routing. External probes saw a healthy child while the supervisor timed out and killed it after roughly seventy seconds. Commits `e07eadb3a7af01bc71e1240ddd351c8805313eff` and `f56631072a38f32f8f02efa49c5a268156612219` changed startup readiness to the lightweight local active-run endpoint and force loopback supervisor probes to bypass proxies.
- 2026-07-28: Exact `f56631072a38f32f8f02efa49c5a268156612219` is the final production package at `target/deploy-f5663107`. Its `502` payloads and manifest SHA-256 `b908de852668a47ea350e8f00dfb8ef09c47e7dcfa494a68a24c4994d32428bd` were verified before restart. Web and Feishu remained healthy beyond the prior self-exit window; version `0.15.3`, ports `8077/8088`, authoritative PostgreSQL/R2, zero local durable dependencies, JSON `401` auth boundaries, public routes including `/activate/whop`, at least one established Feishu connection, and zero active chats all passed. The unconfigured Whop webhook returns an intentional JSON `503` and cannot activate entitlement until its secret and sender are configured.
