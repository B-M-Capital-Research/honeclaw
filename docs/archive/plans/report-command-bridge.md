...
- Archived on 2026-04-16 because the plan had gone inactive and its workflow assumptions risk drifting out of date; it should reopen only when the local report bridge work resumes.

## Validation

- Archived without new execution in this cleanup pass. Reopen with a live workflow runner and a fresh validation matrix before implementation resumes.

## Documentation Sync

- Archived from the active index on 2026-04-16.
- If resumed, reopen with a fresh plan and then update `docs/repo-map.md` once the bridge behavior actually lands.
