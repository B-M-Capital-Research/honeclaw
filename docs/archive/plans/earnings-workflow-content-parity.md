...
- 本事故已上升为仓库级生成型/智力型工作流原则：机械门禁测量的是代理指标，模型会针对代理指标优化，可能在形式合规的同时降低真实作品价值。内容评审默认作为真实 canary 的诊断信号；只有客观可判定且误杀/代理优化成本经过真实样本证明可接受的性质，才可成为阻断条件。权限、隔离、路由、安全、幂等和 artifact 闭环仍保持确定性执行。
