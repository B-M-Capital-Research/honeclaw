...
- Focused tests cover isolated tool schemas, control preambles followed by valid decisions, content/empty/error/timeout terminal reasons, Required fallback boundaries, metadata/finish/DONE enforcement, MiniMax-style tool/stop finish plus clean EOF, no-finish EOF, duplicate finish, finish-then-transport-error, literal-DONE deduplication, reasoning removal, empty-tools payload normalization, exact canonical prefix capture, one buffered recovery, recovery mismatch, suffix-only Session publication, and no business replay.
