...
- Bind each persistent Hone logical session to one usable `codex_acp_session_id`. `codex_acp_session_mode=native_turn_v2` and the SHA-256 instruction fingerprint remain audit/migration metadata and never rotate the identity. Only missing ID normally calls `session/new`; the sole replacement exception is a pre-prompt `session/resume` response from the validated adapter whose structured details exactly prove that the same persisted thread ID has no rollout. The new ID plus mode/fingerprint is checkpointed through a narrow host-owned persistence boundary before the first `session/prompt`. Every other resume failure remains fail-closed. Native prompt results are not automatically resent because transport failure does not prove the native thread rejected the turn.
...
- Any nonempty persisted Codex native ID, including a legacy pre-marker value, remains authoritative until an operator explicitly repairs it or the validated adapter proves before prompt that the exact ID has no rollout. Instruction/config changes and all ambiguous failures update nothing and never create a replacement task.
