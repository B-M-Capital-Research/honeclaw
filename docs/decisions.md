...
- Clarification boundary: The dynamic Interactive contract is an observational fact-enhancement and deterministic truth-checking input, not permission to answer. If it cannot be built—because coverage is empty, the provider failed, candidates remain ambiguous, a seed was omitted, or an exact quote/time is missing—the service records the diagnostic and preserves the successful Agent response verbatim. It must never change `success=true` into failure or replace the body with a fixed “cannot safely publish” sentence. The Agent owns the concrete no-coverage, ambiguity, missing-data, or qualitative answer based on its current tool context. A bounded read-only continuation may still recover a silently omitted explicit seed, but failure to obtain an optional contract after that continuation is not a publication ban. Typed scheduled/heartbeat contracts, runner/auth/quota failures, and persistent-side-effect safety retain their separate fail-closed semantics.
- Output boundary: The initial Agent draft is hidden. If the trace builds a dynamic contract, the service prepends the provider-time/canonical-fact block and applies only deterministic checks that do not depend on the Agent's prose organization: resolved entities, same-symbol current quote and provider time, extended-session identity/session/freshness when present, false market-data denial, and the single-answer/session-history/SSE terminal boundary. It does not reclassify the user's wording, require fixed chapter titles, change the Agent's chosen depth, parse arbitrary natural-language financial/news/holding sections, or replace an interactive answer with a service-authored whole-answer template. One read-only repair may correct a deterministic truth violation while preserving the Agent's structure, length, and priorities. Initial and repair tool traces remain merged for audit, and an unknown or persistent repair call fails closed before a revised body can be accepted. If no dynamic contract is built, the untouched successful Agent body follows the same one-answer/one-terminal boundary without a fabricated verified-time prefix. `DeferredUserOutputEmitter` currently publishes that final body once; reducing the resulting first-visible-text latency is a separate follow-up and may not reintroduce draft/reset flicker.
