...
- Agent-loop contract: The main Agent reads the complete query and decides which entities, evidence depth, tools, priorities, and answer shape are relevant without emitting or conforming to a closed intent taxonomy. If securities are named, its first tool round searches every candidate with current-turn `data_fetch(search)`, including explicit ticker codes; after those results enter context, the next round exact-verifies every selected canonical symbol with quote/profile and continues with whichever financials, holdings, news, web, earnings, sector, or extended-hours tools the Agent judges necessary. Those tool results are loaded into the Agent's current context; their presence describes available evidence, not a service-owned depth route, format, or chapter contract. The service reconstructs scope from the first search group plus the same run's actual structured tool trace, and never infers a required tool set from words such as “估值”, “最近”, “安全区间”, or any finite phrase list. High-confidence explicit code seeds are a post-run completeness minimum: each must appear in the search group, but seeds never prevent the Agent from adding other entities found from the full query. A positive same-symbol quote with usable provider timestamp is mandatory for every resolved security. Market/sector scope likewise comes from Agent-selected structured tools and verified benchmark types. Partial/omitted explicit-code coverage, wrong-symbol quotes, stale timestamps, search ordering, scanner seeds alone, or historical context cannot establish current facts.
- Clarification boundary: A current-turn search that genuinely has no authoritative coverage, a provider failure, or an unresolved equal candidate set is a safe Agent-owned clarification; the service preserves the Agent's concrete explanation instead of replacing it with a generic entity-recognition message. A uniquely selected security that still lacks an exact quote/time, or a run that silently omitted a high-confidence explicit seed, is unsafe incomplete execution: one read-only Agent-loop continuation may load the missing evidence, after which numeric security analysis remains fail-closed if the evidence is still absent.
- Output boundary: The initial Agent draft is hidden. If the trace builds a dynamic contract, the service prepends the provider-time/canonical-fact block and applies only deterministic checks that do not depend on the Agent's prose organization: resolved entities, same-symbol current quote and provider time, extended-session identity/session/freshness when present, false market-data denial, and the single-answer/session-history/SSE terminal boundary. It does not reclassify the user's wording, require fixed chapter titles, change the Agent's chosen depth, parse arbitrary natural-language financial/news/holding sections, or replace an interactive answer with a service-authored whole-answer template. One read-only repair may correct a deterministic truth violation while preserving the Agent's structure, length, and priorities. Initial and repair tool traces remain merged for audit, and an unknown or persistent repair call fails closed before a revised body can be accepted. `DeferredUserOutputEmitter` then publishes one final answer and one terminal event, so users never see a draft followed by a second body or reset. If the Agent found no named security and made no search round, the ordinary answer proceeds without a fabricated market-data time line.
