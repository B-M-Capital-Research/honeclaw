...
- Status: Accepted
- Superseded in part by: `D-2026-07-18-01` extends the deferred-output boundary to every nonempty interactive turn because named-security scope is known only after the Agent tool loop. Native streaming remains applicable to typed noninteractive paths that do not use this boundary.
...
- Scope: Guarded typed investment turns trade token-by-token display for validated one-shot display, where correctness and stable rendering take priority over draft latency. Under `D-2026-07-18-01`, nonempty interactive turns also remain behind this boundary until their same-run tool trace proves whether a security contract exists, preventing a streamed draft from being followed by a repaired second answer.
...
- Output and latency: `DeferredUserOutputEmitter` remains in place for this correctness repair so draft deltas, resets, and attempt-local errors cannot create a second-looking response. This preserves one final answer and one terminal event, but it also defers first visible body text until synthesis completes. Safe first-token streaming is a separate follow-up and must solve tool-branch reset semantics before removing the defer.
