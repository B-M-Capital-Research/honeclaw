...
- Production follow-up evidence: The first post-deployment replay of `分析下crwv和nbis的估值` proved the Agent loop and DataFetch path were healthy: it searched, quoted, profiled, loaded financials, and loaded news for both `CRWV` and `NBIS`, then logged `contract_built=true entities=CRWV,NBIS`. The answer was nevertheless rejected by legacy free-prose financial/event parsers inherited from the typed formatter. That failure confirms why arbitrary prose parsing is outside the interactive service boundary; it is not evidence of missing provider data. A second production Web/SSE acceptance replay remains required after the boundary repair.
