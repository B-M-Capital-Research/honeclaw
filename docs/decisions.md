...
- Extended-hours path: `data_fetch extended_hours` exposes one bounded normalized minute bar. Explicit US premarket/postmarket questions use it only after exact entity resolution and a standard quote, and only when the leaf carries the exact symbol, the requested `pre` or `post` session matches its New York timestamp, and the timestamp is no more than 45 minutes old. The 16:00 close remains regular; only 16:01–20:00 is postmarket, and only 04:00–09:29 is premarket. A failed match retains the regular quote with an explicit disclosure instead of calling it a live extended-hours price. Crypto/night-session requests do not enter this US-equity contract.
- Numeric and event boundaries: Common aliases such as “目前价 / 现在价 / 市价 / 市场价 / market price” are current-price claims and cannot coexist with a conflicting quote. Historical/open/close/high/low prices fail closed until the contract has a dedicated verified historical-price field; dates, source domains, inference labels, and coincidentally equal current values cannot authenticate them. Event inference/condition subheadings govern only their following Markdown list items and reset before prose or unknown headings.
- Operations: English and Chinese persistent-operation-plus-analysis requests are execute-once. The safe fallback never justifies replaying a user operation or retaining rejected runner tool metadata.
- Verification: CI-safe tests cover exact lowercase tickers with price/extended aliases, US session boundaries, exact symbol/session/freshness, canonical extended/fallback labels, unsupported historical prices, event heading scope, same-gate deterministic outputs for quote/equity/fund/crypto/market, rejected draft metadata clearing, persistent-operation classification, and single-attempt fallback behavior.
