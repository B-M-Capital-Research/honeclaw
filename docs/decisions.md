...
- Output boundary: The initial Agent draft is hidden. If the trace builds a dynamic contract, the service prepends the provider-time/canonical-fact block and applies truth-only checks for exact entity, quote/time consistency, selected-tool numeric evidence, false market-data denial, and dated event claims. It does not reclassify the user's wording, require fixed chapter titles, change the Agent's chosen depth, or replace an interactive answer with a service-authored whole-answer template. One read-only repair may correct a truth violation while preserving the Agent's structure, length, and priorities. Initial and repair tool traces remain merged for audit, and an unknown or persistent repair call fails closed before a revised body can be accepted. `DeferredUserOutputEmitter` then publishes one final answer and one terminal event, so users never see a draft followed by a second body or reset. If the Agent found no named security and made no search round, the ordinary answer proceeds without a fabricated market-data time line.
