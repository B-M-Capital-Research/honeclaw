...
- Status: Accepted
- Supersedes: The single-security-only scope, text-envelope detection, acronym denylist, and final-stage ticker inference in `D-2026-07-15-03`. Its deep-analysis evidence set, nine-section answer format, one reset/retry, and fail-closed behavior remain active.
...
- Answer contracts: Interactive deep single-security requests retain the nine-section contract. Multi-security comparisons must cover every resolved symbol, state a data timestamp, and include risks or falsification conditions. Scheduled and heartbeat turns use the same entity and quote gate but keep their delivery-specific response shape.
