...
- Prompt layering: `crates/hone-channels/src/prompt.rs`
  ...
  - The guard supports single and mixed-asset turns. Dynamic interactive contracts use truth-only validation for exact entities, current quote/time consistency, evidence-backed financial/holding/event/extended-session numbers, and false real-time-data denial; they do not run the fixed chapter/intent validator. Typed scheduled/heartbeat contracts retain complete single-security/fund/crypto/market/sector or comparison validation. Both paths accumulate forbidden calls across repair and receive the typed origin plus original scheduler task body.
  - `agent_session/emitter.rs` owns the validated-output boundary: guarded runner attempts expose progress/tool status but withhold answer deltas, resets, thoughts, and attempt-local errors. `agent_session/core.rs` renders the server-owned Beijing data-time/entity/quote prefix before the Agent-authored body and validates the same visible content later persisted. Dynamic interactive work never uses a deterministic whole-answer fallback; one truth-only repair must preserve the Agent's chosen organization. Typed noninteractive contracts may still use their deterministic contract-only fallback. `tool_trace.rs` canonicalizes runner/MCP tool aliases and owns the explicit read-only allowlist used by retry/recovery/fallback; unknown tools and persistent actions such as `deep_research` block automatic retry, recovery, repair, and fallback, while a no-trace intent classifier keeps covered portfolio writes, completed trades, and research starts execute-once. Persistent mutations remain execute-once even when hidden runner traces are uncertain. `hone-web-api/routes/chat.rs` treats session `Done` as the sole terminal authority, emits at most one `run_finished`, and ignores late frames after terminal delivery.
...
4. `hone-channels::AgentSession::run()` orchestrates session semantics such as run locking, fast user-message persistence, quota, runner invocation, listener dispatch, compaction trigger, and final persistence. Every nonempty interactive turn appends lightweight Agent discovery guidance and runs once with actor-bound tools. Its draft output is deferred; the service derives a dynamic investment contract from the same run's search/quote and whatever problem-evidence tools the Agent selected, then applies only server time/canonical facts and truth checks before publishing one final body. It neither classifies the wording nor chooses the response format. Portfolio truth is loaded by the Agent's own `portfolio(view)` call, while typed scheduled/heartbeat work may use deterministic securities/portfolio/broad preparation. A truth repair preserves the Agent's structure and the initial/retry traces, and rejects unknown or persistent retry calls before accepting a revised body. Prompt/slash skill turn building lives in `turn_builder.rs`; assistant output cleanup, empty-success fallback, system-prompt/internal-output blocking, and local image marker stabilization live in `response_finalizer.rs`. Keep an explicit distinction between:
