...
- `crates/`
  ...
  - `hone-llm`: model provider abstraction, profile resolver, OpenRouter integration, and generic OpenAI-compatible provider plumbing used by configured LLM routes such as auxiliary/background tasks and selected `multi-agent` stages. Its tool-capable stream contract normalizes content/reasoning/usage plus indexed tool-call fragments; OpenRouter and generic OpenAI-compatible providers consume upstream SSE directly, with key fallback allowed only before a stream starts.
...
- `agents/`
  - `function_calling`: actor-bound function-calling agent core. It assembles fragmented parallel tool calls from the provider stream, hides internal reasoning tags across chunk boundaries, emits final-answer deltas through an observer, and resets any visible tool preamble before continuing the tool loop; the completed response remains the only persisted assistant turn.
