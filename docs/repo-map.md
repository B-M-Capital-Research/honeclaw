...
- `crates/`
  ...
  - `hone-llm`: model provider abstraction, profile resolver, OpenRouter integration, and generic OpenAI-compatible provider plumbing used by configured auxiliary/background routes. Its stream contract normalizes content/reasoning/usage plus indexed tool-call fragments; OpenRouter and generic OpenAI-compatible providers consume upstream SSE directly, with key fallback allowed only before a stream starts.
...
- Runner contract and ACP / Gemini execution layer: `crates/hone-channels/src/runners/`
  ...
  - `gemini_cli.rs`, `codex_acp.rs`, `opencode_acp.rs`, `hone_cloud.rs`: active primary runner implementations; `tool_reasoning.rs` owns the non-user-selectable strict function-calling fallback that keeps non-admin actors off trusted-host CLI/ACP subprocesses
- Prompt layering: `crates/hone-channels/src/prompt.rs`
  ...
  - `security_identifier.rs` is the single lexical/provider-dialect layer for bare codes, cashtags, exchange-qualified symbols, share classes, index prefixes, and crypto pairs. It emits ordered source spans, excludes URLs/email/source paths, consumes unsupported composites without rescanning their suffix as a US ticker, and owns only a closed set of provider aliases. `investment_response_guard.rs` is the entity-first binding/resolution layer. Every nonempty turn first enters `EntityResolutionScope`, whose outcomes are `Securities`, `Portfolio`, `Broad`, `ConfirmedNoEntity`, and `NeedsClarification`. Each `EntityMention` retains provenance, source span, identifier kind, and per-mention numeric market/asset hints. Explicit codes take bounded batch exact-quote probes; company names and genuinely tentative code/name collisions use concurrent semantic search. Tentative reconciliation classifies strong full-name/name-prefix relations separately from product names that merely embed the underlying ticker; the latter cannot override an exact quote, and low-score semantic results are unresolved. Complex names/aliases and incomplete comparisons may use the configured auxiliary LLM, which has a 15-second timeout and fails closed on timeout, provider failure, malformed output, or incomplete extraction. Auxiliary aliases cannot replace explicit lookup keys. Safe-range, margin-of-safety, fair-value, recommendation, and entry-price wording is classified as deep valuation intent before quote-only detection.
  - The `Securities` route resolves to one provider-canonical symbol before any downstream work, reuses verified probe quotes, and concurrently enriches profile metadata when the response contract requires asset routing. Bare numeric symbols use a closed exchange candidate registry with span-local market/asset binding; amounts, years, percentages, target prices, and scheduler cadence are excluded before provider work. The route independently classifies each symbol as equity, ETF/fund, index, or crypto, then chooses company financials, ETF holdings, index, or crypto evidence and forbids inapplicable tools. Provider transport failure, empty exact coverage, ambiguity, and stale quote time remain distinct states. Provider evidence is reduced to symbol-matched typed fields: conflicting profile snapshot prices are removed, annual income-statement metrics retain their real semantics, unsupported cash/debt/FCF/consensus/forward/peer fields remain unavailable, and news is filtered against the resolved entity. Every event fact must match a verified absolute-date/full-domain pair in the same clause. `Broad` resolves representative quotes and exchange-local search dates per requested market.
  - The `Portfolio` route creates the actor-bound tool registry and performs one service-owned read-only `portfolio {"action":"view"}` preflight. That bounded snapshot is the only holding/watchlist membership and cost truth source. A current-price or performance question continues into exact DataFetch entity and quote checks for the explicitly requested ticker, or a bounded portfolio subset when none was named. Snapshot metadata records record/symbol totals, included counts, truncation, and omitted counts so limited coverage cannot masquerade as full-portfolio analysis; the preflight cannot be treated as a requested mutation. `ConfirmedNoEntity` forbids adding an old ticker from history, while `NeedsClarification` stops before security analysis.
  - The guard supports single and mixed-asset turns, applies complete single-security/fund/crypto/market/sector or comparison validation to the canonical sanitized visible response, rejects current-price conflicts and false real-time-data denial, accumulates forbidden calls across repair, and receives typed interactive/scheduled/heartbeat origin plus the original scheduler task body.
  - `agent_session/emitter.rs` owns the investment validated-output boundary: guarded runner attempts expose progress/tool status but withhold answer deltas, resets, thoughts, and attempt-local errors. `agent_session/core.rs` renders the server-owned Beijing data-time/entity/quote prefix before the model-authored body and validates the same visible content later persisted. For supported quote/equity/fund/crypto/market scopes, an incomplete successful draft is replaced without another model call by a deterministic contract-only answer that passes the same sanitizer/validator; comparison and sector scopes keep specialized repair. `tool_trace.rs` canonicalizes runner/MCP tool aliases and owns the explicit read-only allowlist used by retry/recovery/fallback; unknown tools and persistent actions such as `deep_research` block automatic retry, recovery, repair, and fallback, while a no-trace intent classifier keeps covered portfolio writes, completed trades, and research starts execute-once. Persistent mutations remain execute-once even when hidden runner traces are uncertain. `hone-web-api/routes/chat.rs` treats session `Done` as the sole terminal authority, emits at most one `run_finished`, and ignores late frames after terminal delivery.
...
- FMP-backed DataFetch implementation: `crates/hone-tools/src/data_fetch.rs`; every symbol interpolated into a path or query is URL-encoded per symbol while commas remain bounded batch separators. `extended_hours` returns only one normalized latest minute bar and labels `pre / regular / post / closed` using US session boundaries before the investment guard applies exact-symbol/session/freshness checks.
...
- Tavily-backed current-event search: `crates/hone-tools/src/web_search.rs`
  - Supplies dated news, causes, policy, and event context after the security/market scope is known. Investment evidence is treated as untrusted external data until reduced to entity-matching records with an actual event date and normalized full source domain; web query dates cannot masquerade as event dates.
...
4. `hone-channels::AgentSession::run()` orchestrates session semantics such as run locking, fast user-message persistence, quota, runner invocation, listener dispatch, compaction trigger, and final persistence. Before security-specific generation, every nonempty turn passes through the five-scope entity stage. Exact ticker securities use current-turn bounded DataFetch exact-quote probes, named securities use semantic search, portfolio scope reads the actor portfolio and then uses the same canonical market-data contract when analysis needs current facts, broad scope prepares market/sector evidence, confirmed-no-entity continues without an old ticker, and clarification stops unresolved named-security work. Prompt/slash skill turn building lives in `turn_builder.rs`; assistant output cleanup, empty-success fallback, system-prompt/internal-output blocking, and local image marker stabilization live in `response_finalizer.rs`. Keep an explicit distinction between:
...
5. `hone-channels::execution` builds the concrete execution plan for both persistent conversations and transient tasks: prompt audit, tool registry, runner selection, and actor-sandbox-backed `AgentRunnerRequest`. Native CLI/ACP runners are reserved for explicit administrators; non-admin actors fail closed and must use `hone_cloud` instead of a retired in-process fallback.
...
9. Responses are sent back to the originating channel; the Web console streams `run_started / run_progress / assistant_delta / assistant_reset / tool_call / run_error / run_finished` via v2 SSE events. `run_finished` is terminal and unique; the client closes its observer and discards later frames, while refresh recovers the server-owned original start time. Controlled CLI shutdown polls `/api/runtime/active-chat-runs` before terminating the backend.
