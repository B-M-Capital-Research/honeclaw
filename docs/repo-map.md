...
- Attachment ingest / preview helpers: `crates/hone-channels/src/attachments.rs` and `crates/hone-channels/src/attachments/{ingest,vision,vector_store}.rs`
  - Enforces shared attachment gates across channels: 5 MB for generic attachments, 3 MB for images, plus rejection of extreme aspect ratio, resolution, or pixel-count cases. Rejected attachments never enter the prompt. After the attachment has been downloaded and validated, `agent.image_understanding_profile` may route the trusted image bytes—not a user-supplied filesystem path—to an image-capable LLM through `LlmProvider::chat_with_images`; its bounded result enters as `【视觉模型描述】`. On macOS, `attachments/vision.rs` independently performs Apple Vision OCR and appends normalized rows as `【图片文字提取】`. Visual understanding comes first and OCR second, so charts, position, color and shape do not depend on OCR while readable text remains available. A missing/failed visual profile degrades to OCR, and unsupported hosts, empty extraction or provider failure never fabricate image facts.
...
- FMP-backed DataFetch implementation: `crates/hone-tools/src/data_fetch.rs`; its model-facing contract requires the Agent to declare a stable, distinct, case-sensitive `entity_route` for each named security, send one separate identity search per route (parallel calls are allowed), provide call-scoped `identity_match` on every search, and reuse the route verbatim on refinement, quote, profile/snapshot, and later calls. A formal-English-name or canonical-ticker refinement stays on the same route when a Chinese-name/alias lookup is empty; `refines_query` may link that exact empty query, while `supersedes_query` may migrate one earlier search that omitted a route. Both are case-sensitive verbatim metadata, mutually exclusive, not sent to FMP, and never used to parse natural language. Explicit ticker results must match the same code and cannot be replaced by a product whose name merely embeds it. Public `effective_data_fetch_{data_type,target,security_target}` and `validated_data_fetch_{symbols,search_query}` helpers are the single executor/ledger request parser: a present but wrongly typed higher-priority field does not fall through, broad types ignore ticker fields, only ASCII commas delimit a batch, and empty segments, controls, or inputs over 512 bytes are rejected. Every accepted symbol is URL-encoded per symbol. `extended_hours` returns one normalized latest minute bar and labels `pre / regular / post / closed` using US session boundaries before the investment guard applies exact-symbol/session/freshness checks. When configured FMP credentials are absent or unusable, `fmp.official_fallback_enabled` permits the bounded Nasdaq/SEC official-source fallback; tests can disable it to remain hermetic.
...
- Layered current-event search: `crates/hone-tools/src/web_search.rs`
  - Supplies dated news, causes, policy, relationship, and event context after the security/market scope is known. Runtime prefers the bounded Tavily key pool, then automatically degrades to DuckDuckGo's official non-JavaScript HTML results without requiring a second credential; repeated public-provider failures enter a five-minute circuit-breaker cooldown. Both routes remain basic snippet search without answer/raw-page content and responses are locally capped again to three results. A trusted root `hone_search_contract` records the actual provider, snippet-only scope, no full page, and that search order/score is not real-world rank; each result receives locally overwritten `hone_evidence` citation metadata, while a result without an original URL is non-citable. Provider-spoofed fields cannot loosen this envelope. Investment evidence remains untrusted external data until reduced to current entity-matching records; a URL locates its title/content/snippet but does not prove facts absent from that result, and web query dates cannot masquerade as event dates. Current company/financial claims still prefer DataFetch's SEC/Nasdaq/company-source evidence over either search provider.
