...
- Prompt layering: `crates/hone-channels/src/prompt.rs`
  ...
  - `investment_response_guard.rs` is the entity-first investment gate: it extracts structured mentions, resolves every named security through current-turn DataFetch search without first-result guessing, verifies same-symbol quotes, and prepares one contract reused by execution and retry. It supports single and multi-security turns, distinguishes broad no-entity requests from ambiguous/unresolved identities, applies deep nine-section or comparison response validation, and receives typed interactive/scheduled/heartbeat origin plus the original scheduler task body.
