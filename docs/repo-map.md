...
- Prompt layering: `crates/hone-channels/src/prompt.rs`
  ...
  - After an interactive run, `build_agent_discovered_investment` identifies the first assistant DataFetch search group from `context_messages`, requires every high-confidence explicit code seed to appear in that group without treating seeds as a complete set, intersects each search result with later same-run exact quote evidence, and requires a positive price and usable provider timestamp for every resolved entity. The Agent-selected financials, holdings, news, web, earnings, sector, market, and extended-hours results stay in the current tool context for synthesis; their presence does not select a service-owned depth route, chapter set, or prose validator. DataFetch envelopes are never treated as provider rows: a Chinese lookup such as `英伟达` resolves from the returned `NVDA` row, not the wrapper's original query. CRWV search results that also contain CWY can only resolve to CRWV when CRWV itself has the exact quote; quoting only CWY cannot satisfy the request. Omitted explicit seeds or unique entities without exact quote/time are unsafe incomplete and may receive one read-only Agent-loop continuation; actual no-coverage/provider failure/equal-candidate ambiguity preserves the Agent's concrete clarification. A successful trace builds a dynamic `InvestmentResponseContract` for server-owned time/canonical facts and deterministic truth validation only. It does not select a template or answer depth. No search round leaves an ordinary answer unprefixed, while sector/market scope comes from Agent-selected structured tools and verified benchmark types.
