...
1. A channel entrypoint or the Web API receives user input and performs protocol parsing, allowlist checks, and mention-semantic detection on the channel side
2. `hone-channels::ingress` centralizes actor scope, chat mode, deduplication, session serialization, and `IncomingEnvelope`
