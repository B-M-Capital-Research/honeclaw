...
- Prompt layering: `crates/hone-channels/src/prompt.rs`
  ...
  - `investment_response_guard.rs` is the entity-first, asset-aware investment gate: it recognizes contextual bare ticker candidates without waiting for auxiliary JSON, extracts complex names/aliases structurally, resolves every candidate through current-turn DataFetch search without first-result guessing, verifies same-symbol quotes, and prepares one contract reused by execution and retry. Exact profile/search fields route each symbol independently as equity, ETF/fund, or crypto; the route chooses company financials, ETF holdings, or crypto market evidence and forbids inapplicable tools. It supports single and mixed-asset turns, applies asset-specific substantive nine-section or comparison validation to the canonical sanitized visible response rather than raw reasoning, rejects conflicting current prices, accumulates forbidden calls across retries, and receives typed interactive/scheduled/heartbeat origin plus the original scheduler task body.
  - `agent_session/emitter.rs` owns the investment validated-output boundary: guarded runner attempts expose progress/tool status but withhold answer deltas, resets, thoughts, and attempt-local errors. `agent_session/core.rs` publishes one finalized canonical answer only after the guard succeeds; `hone-web-api/routes/chat.rs` maps `Done` to the sole `run_finished` event.
...
9. Responses are sent back to the originating channel; the Web console streams `run_started / run_progress / assistant_delta / assistant_reset / tool_call / run_error / run_finished` via v2 SSE events. Controlled CLI shutdown polls `/api/runtime/active-chat-runs` before terminating the backend.
