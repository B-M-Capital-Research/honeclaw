...
- Prompt layering: `crates/hone-channels/src/prompt.rs`
  ...
  - `investment_response_guard.rs` is the entity-first, asset-aware investment gate. Every nonempty turn first enters `EntityResolutionScope`, whose outcomes are `Securities`, `Portfolio`, `Broad`, `ConfirmedNoEntity`, and `NeedsClarification`. Contextual bare ticker candidates take the deterministic exact-symbol path without waiting for auxiliary JSON; complex names/aliases may use the configured auxiliary LLM, which has a 15-second timeout and fails closed on timeout, provider failure, malformed output, or incomplete extraction. The guard resolves securities through current-turn DataFetch without first-result guessing, verifies same-symbol quotes, selects a fresh exact-symbol extended-hours minute bar for explicit US pre/post-market requests, and prepares one contract reused by execution and final validation.
