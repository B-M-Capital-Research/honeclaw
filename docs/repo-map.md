...
- `packages/`
  - `app`: SolidJS Web console and public user client. Public visual ownership is layered: `src/pages/public-foundation.css` owns HONE tokens and global interaction foundations; `public-site.css` retains the broad public-page legacy/layout rules; `public-polish.css` owns the shared navigation/push component refinement; `public-chat.css` exclusively owns the authenticated chat shell that was formerly embedded in `chat.tsx`. `/chat` startup uses the same full-page `PublicChatStartup` shell from route-module loading through authenticated bootstrap; `/api/public/bootstrap` returns auth/quota plus the newest 20 projected history items, and `/api/public/history?before=<cursor>&limit=20` supplies older pages when the user scrolls upward. Absolute projected-history offsets keep message IDs stable while pages prepend, and the page commits bootstrap state atomically before mounting at the newest message. Each assistant turn is one stable in-thread card across thinking, streaming, completion, recovery, abort, and error states; recovered background runs use a temporary timeline ID that the final history row adopts in place. Restored media and the finance-calendar action row reserve their final layout before images decode progressively. The interactive portrait-calendar preview lives beside its component in `src/components/finance-calendar-mobile-card.css`; uploaded mobile PNGs are authored by `src/lib/finance-calendar-mobile-renderer.ts` through direct Canvas 2D painting at deterministic 2x resolution and a versioned mobile filename, avoiding platform-specific DOM screenshot font metrics.
