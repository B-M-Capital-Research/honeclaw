...
- `packages/`
  - `app`: SolidJS Web console and public user client. Public visual ownership is layered: `src/pages/public-foundation.css` owns HONE tokens and global interaction foundations; `public-site.css` retains the broad public-page legacy/layout rules; `public-polish.css` owns the shared navigation/push component refinement; `public-chat.css` exclusively owns the authenticated chat shell that was formerly embedded in `chat.tsx`. `/chat` startup uses the eager `PublicChatStartup` shell from route-module loading through authenticated bootstrap; `/api/public/bootstrap` returns auth/quota and history in one response, and the page commits both atomically before exposing the ready chat shell. Restored media reserves its layout and decodes progressively. The interactive portrait-calendar preview lives beside its component in `src/components/finance-calendar-mobile-card.css`; uploaded mobile PNGs are authored by `src/lib/finance-calendar-mobile-renderer.ts` through direct Canvas 2D painting at deterministic 2x resolution and a versioned mobile filename, avoiding platform-specific DOM screenshot font metrics.
