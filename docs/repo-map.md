...
- Prompt layering: `crates/hone-channels/src/prompt.rs`
  ...
  - `investment_response_guard.rs` is the entity-first, asset-aware investment gate. Every nonempty turn first enters `EntityResolutionScope`, whose outcomes are `Securities`, `Portfolio`, `Broad`, `ConfirmedNoEntity`, and `NeedsClarification`. Contextual bare ticker candidates and explicit ticker-identity bindings take the deterministic exact-symbol path without waiting for auxiliary JSON merely because surrounding business prose is unfamiliar; complex names/aliases and possible incomplete comparisons may use the configured auxiliary LLM, which has a 15-second timeout and fails closed on timeout, provider failure, malformed output, or incomplete extraction. Exact ticker lookup keys cannot be replaced by auxiliary aliases. A semantic-empty or derivative-only exact search receives one same-symbol profile fallback before normal quote verification. Safe-range, margin-of-safety, fair-value, recommendation, and entry-price wording is classified as deep valuation intent before quote-only detection, so the full asset-specific response contract remains mandatory. The guard otherwise resolves securities through current-turn DataFetch without first-result guessing, verifies same-symbol quotes, selects a fresh exact-symbol extended-hours minute bar for explicit US pre/post-market requests, and prepares one contract reused by execution and final validation.
  - The `Securities` route uses exact profile/search fields to classify each symbol independently as equity, ETF/fund, or crypto, then chooses company financials, ETF holdings, or crypto market evidence and forbids inapplicable tools. Provider evidence is reduced to symbol-matched typed fields: conflicting profile snapshot prices are removed, annual income-statement metrics retain their real semantics, unsupported cash/debt/FCF/consensus/forward/peer fields remain unavailable, and news is filtered against the resolved entity. Every event fact must match a verified absolute-date/full-domain pair in the same clause. `Broad` resolves representative quotes and exchange-local search dates per requested market.
...
- FMP-backed DataFetch implementation: `crates/hone-tools/src/data_fetch.rs`; `extended_hours` returns only one normalized latest minute bar and labels `pre / regular / post / closed` using US session boundaries before the investment guard applies exact-symbol/session/freshness checks.
