...
- `config.yaml` / `data/runtime/`
  ...
  - `cloud.mode=local|cloud|auto` controls storage authority. `local` is the default and preserves JSON / SQLite / filesystem behavior even if PG / OSS env vars are present; `cloud` requires PG + OSS and exposes strict cloud status; `auto` keeps the older development behavior where env presence can enable cloud capabilities. `cloud.postgres` / `cloud.oss` define env-backed PG / OSS settings, including `HONE_POSTGRES_PROXY` and `HONE_OSS_PROXY`.
  - `crates/hone-core/src/cloud_runtime.rs` centralizes runtime role parsing, PG schema / health / document-index helpers, actor-scoped OSS keys, OSS proxy support, `.env` loading, and the cloud-mode local durable dependency report. `hone-cli cloud doctor` and `/api/meta` use this helper layer instead of inferring authority from config presence alone.
  - `bins/hone-cli/src/cloud.rs` provides `hone-cli cloud doctor` and `hone-cli cloud migrate`. The migrator dry-runs local `data/`, uploads recognized durable files to OSS under `users/{actor_storage_key}/documents/...`, and indexes them in PG `cloud_documents`; SQLite files are counted but skipped until structured table import is implemented.
