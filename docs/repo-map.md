...
- Prompt layering: `crates/hone-channels/src/prompt.rs`
  ...
  - `agent_session/emitter.rs` owns the investment validated-output boundary: guarded runner attempts expose progress/tool status but withhold answer deltas, resets, thoughts, and attempt-local errors. `agent_session/core.rs` renders the server-owned Beijing data-time/entity/quote prefix before the model-authored body, validates the same visible content later persisted, and repairs the sanitized draft once without replaying the original operation. Persistent mutations are execute-once even when hidden runner traces are uncertain. `hone-web-api/routes/chat.rs` treats session `Done` as the sole terminal authority, emits at most one `run_finished`, and ignores late frames after terminal delivery.
...
- FMP-backed DataFetch implementation: `crates/hone-tools/src/data_fetch.rs`
