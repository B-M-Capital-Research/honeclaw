...
- `packages/`
  - `app`: SolidJS Web console and public user client. Public visual ownership is layered: `src/pages/public-foundation.css` owns HONE tokens and global interaction foundations; `public-site.css` retains the broad public-page legacy/layout rules; `public-polish.css` owns the shared navigation/push component refinement; `public-chat.css` exclusively owns the authenticated chat shell that was formerly embedded in `chat.tsx`. Generated portrait-calendar composition lives beside its component in `src/components/finance-calendar-mobile-card.css`, and its capture contract uses deterministic 2x rasterization plus a versioned mobile filename.
