...
- Prompt layering: `crates/hone-channels/src/prompt.rs`
  ...
  - `security_identifier.rs` is the single lexical/provider-dialect layer for bare codes, cashtags, exchange-qualified symbols, share classes, index prefixes, and crypto pairs. It emits ordered source spans, excludes URLs/email/source paths, consumes unsupported composites without rescanning their suffix as a US ticker, and owns only a closed set of provider aliases. `investment_response_guard.rs` owns `EntityResolutionScope`: `AgentToolDiscovery`, `Portfolio`, `Securities`, `Broad`, or empty `PassThrough`. Every nonempty interactive turn uses `AgentToolDiscovery`, including portfolio/watchlist wording; lexical `EntityMention` values retain provenance and span hints but are injected only as non-factual seeds. The configured main runner reads the full current query, calls `portfolio(view)` itself when membership matters, and performs current-turn DataFetch discovery, so no phrase grammar, exact-ticker count, comparison connector, broad-market marker, or auxiliary LLM closes the interactive entity set before the Agent runs. Scheduled/heartbeat turns may retain deterministic `Securities` / `Portfolio` / `Broad` preparation under their typed origin.
  - After an interactive run, `build_agent_discovered_investment` consumes every DataFetch search call in the current runner trace. Empty or failed broad/enriched attempts are skipped so later exact-symbol refinements can contribute. Strong verified scope combines exact ticker seeds with uniquely verified name/alias searches grounded in the literal user query. A quote-verified search that is neither seed-bound nor text-grounded makes the optional contract unavailable—rather than silently collapsing a mixed alias request into a ticker-only subset—unless it is a clearly embedded reference product such as CWY for CRWV. Every contracted entity must intersect a positive same-symbol quote with a usable provider timestamp. The Agent-selected financials, holdings, news, web, earnings, sector, market, and extended-hours results stay in current context for synthesis; their presence does not select a service-owned depth route, chapter set, or prose validator. DataFetch envelopes are never treated as provider rows: a Chinese lookup such as `英伟达` resolves from the returned `NVDA` row, not the wrapper's original query. A successful trace builds an optional dynamic `InvestmentResponseContract` for server-owned time/canonical facts and deterministic truth validation. If reconstruction returns none, `AgentSession` logs `contract_built=false answer_preserved=true` and publishes the Agent's successful body unchanged; no fixed refusal or success-to-failure mutation is permitted. Typed scheduled/heartbeat preparation and fail-closed validation remain separate. No search round leaves an ordinary answer unprefixed, while sector/market scope comes from Agent-selected structured tools and verified benchmark types.
  ...
  - The guard supports single and mixed-asset turns. Dynamic interactive contracts validate only deterministic boundaries: exact resolved entities, current quote/provider-time consistency, extended-session identity/session/freshness when present, and false real-time-data denial; arbitrary Agent prose is not divided into financial/holding/news/event/valuation chapters for regex-style fact approval. Session persistence and the Web SSE layer separately enforce one canonical answer and terminal event. Typed scheduled/heartbeat contracts retain complete field-aware single-security/fund/crypto/market/sector or comparison formatting and validation. Both paths accumulate forbidden calls across repair and receive the typed origin plus original scheduler task body. Future interactive per-claim enforcement must consume structured provenance that binds claims to tool-result fields.
  - `agent_session/emitter.rs` owns the validated-output boundary: guarded runner attempts expose progress/tool status but withhold answer deltas, resets, thoughts, and attempt-local errors. `agent_session/core.rs` renders the server-owned Beijing data-time/entity/quote prefix before the Agent-authored body and validates the same visible content later persisted. Dynamic interactive work never uses a deterministic whole-answer fallback or injects the typed fixed-format enforcement block; one deterministic-fact repair must preserve the Agent's chosen organization. Typed noninteractive contracts may still use their deterministic contract-only fallback and strict formatter. `tool_trace.rs` canonicalizes runner/MCP tool aliases and owns the explicit read-only allowlist used by retry/recovery/fallback; unknown tools and persistent actions such as `deep_research` block automatic retry, recovery, repair, and fallback, while a no-trace intent classifier keeps covered portfolio writes, completed trades, and research starts execute-once. Persistent mutations remain execute-once even when hidden runner traces are uncertain. `hone-web-api/routes/chat.rs` treats session `Done` as the sole terminal authority, emits at most one `run_finished`, and ignores late frames after terminal delivery.
...
4. `hone-channels::AgentSession::run()` orchestrates session semantics such as run locking, fast user-message persistence, quota, runner invocation, listener dispatch, compaction trigger, and final persistence. Every nonempty interactive turn appends lightweight Agent discovery guidance and runs once with actor-bound tools. The Agent reads the complete query, identifies all relevant instruments, and uses its first tool rounds to load search plus exact quote/profile facts into the same context before choosing any additional evidence, analysis scope, and answer shape. Its draft output is deferred; the service derives a dynamic investment contract from the same run's structured search/quote trace, then applies only server time/canonical facts and deterministic truth checks before publishing one final body. It neither classifies wording, chooses response format, nor parses arbitrary financial/news prose. Portfolio truth is loaded by the Agent's own `portfolio(view)` call, while typed scheduled/heartbeat work may use deterministic securities/portfolio/broad preparation and strict formatting. A truth repair preserves the Agent's structure and the initial/retry traces, and rejects unknown or persistent retry calls before accepting a revised body. Prompt/slash skill turn building lives in `turn_builder.rs`; assistant output cleanup, empty-success fallback, system-prompt/internal-output blocking, and local image marker stabilization live in `response_finalizer.rs`. Keep an explicit distinction between:
