...
- Prompt layering: `crates/hone-channels/src/prompt.rs`
  ...
  - The guard supports single and mixed-asset turns, but Interactive reconstruction is observational only and performs no post-run answer validation or composition. Session persistence and the Web SSE layer enforce one canonical answer and terminal event. Typed scheduled/heartbeat contracts retain complete field-aware single-security/fund/crypto/market/sector or comparison formatting, validation, repair/fallback safety, typed origin, and original scheduler task body. Future Interactive per-claim enforcement requires a new structured claim/provenance protocol; unconstrained prose or an observed dynamic contract cannot be used as a publication gate.
  - `agent_session/emitter.rs` currently owns the one-output boundary: guarded Interactive runner attempts expose progress/tool status but withhold answer deltas, resets, thoughts, and attempt-local errors until the successful Agent body is ready. The body—including its prompt-required Beijing data-time line—is then published and persisted unchanged; this defer remains temporarily to prevent draft/reset/error flicker, while first-visible-text latency is tracked separately. `agent_session/core.rs` renders and validates server-owned fact prefixes only for caller-supplied typed scheduled/heartbeat contracts. `tool_trace.rs` canonicalizes runner/MCP tool aliases and owns the explicit read-only allowlist used by typed retry/recovery/fallback; unknown tools and persistent actions such as `deep_research` block automatic retry, recovery, repair, and fallback, while a no-trace intent classifier keeps covered portfolio writes, completed trades, and research starts execute-once. Persistent mutations remain execute-once even when hidden runner traces are uncertain. `hone-web-api/routes/chat.rs` treats session `Done` as the sole terminal authority, emits at most one `run_finished`, and ignores late frames after terminal delivery.
...
4. `hone-channels::AgentSession::run()` orchestrates session semantics such as run locking, fast user-message persistence, quota, runner invocation, listener dispatch, compaction trigger, and final persistence. Every nonempty interactive turn appends lightweight Agent discovery guidance and runs once with actor-bound tools. The Agent reads the complete current query, identifies all relevant instruments, loads search plus exact quote/profile facts into the same context, chooses any additional evidence, and owns the complete final answer. Post-run dynamic reconstruction is audit-only and cannot prefix, normalize, retry, repair, rewrite, or reject that successful body. Interactive output is still deferred and published once to prevent draft/reset flicker; first-visible-text latency is a separate follow-up. Portfolio truth is loaded by the Agent's own `portfolio(view)` call, while typed scheduled/heartbeat work may use deterministic securities/portfolio/broad preparation, server-owned prefixes, strict validation, and typed repair/fallback. Before the Interactive runner request is built, prior scheduler/heartbeat/job turn groups and `run_failed=true` groups are removed from runtime context only; Web direct restore remains bounded, while durable history and history pagination remain intact. Prompt/slash skill turn building lives in `turn_builder.rs`; assistant output cleanup, empty-success fallback, system-prompt/internal-output blocking, and local image marker stabilization live in `response_finalizer.rs`. Keep an explicit distinction between:
