...
- Native CLI/ACP runners (`gemini_cli`, `codex_cli`, `codex_acp`, and `opencode_acp`) are trusted-host capabilities because their subprocesses may inspect files or process state outside Hone's actor-bound tool registry. Only explicit channel/CLI administrators may use them; non-admin requests must fail closed and use `hone_cloud` rather than falling back to a retired in-process runner.
...
- Every investment turn must run the entity stage before security-specific planning or generation. Named securities are resolved from current-turn DataFetch search results into structured entities; ambiguous or unresolved candidates fail closed, and no search result may be accepted merely because it is first. Normal bare ticker tokens are lexical candidates even when written in contextual lowercase, but they must pass an exact-symbol DataFetch match and must not depend on auxiliary-LLM JSON success; assignment metadata, reporting periods, industry/metric acronyms, and unrelated words are not entities. A genuinely broad macro/industry request may explicitly produce no security entity. Every resolved symbol requires a positive same-symbol current quote before numeric conclusions. Deep evidence must then follow the verified asset type: equities use an exact company profile plus meaningful financials and news; ETFs/funds use an exact structured fund profile plus holdings and news; crypto uses an exact crypto-market identity plus quote and news, without stock-profile or company-financial requirements. Empty company financials do not by themselves classify an instrument as a fund, and unknown types fail closed. Interactive deep single-security answers retain an asset-specific nine-section, timestamp, fact/inference and action-trigger contract; multi-security answers must give each symbol its own asset-appropriate evidence plus timestamp and risk/falsification conditions. The final validator must inspect the same sanitized visible content sent to users, reject current-price conflicts anywhere in that content, and accumulate forbidden tool calls across internal retries; raw reasoning and tool-protocol blocks cannot satisfy or disrupt the public answer contract. All investment candidate attempts are non-user-visible until validation and finalization succeed: progress/tool status may pass through, but draft deltas, resets, thoughts, and attempt-local errors are withheld, then exactly one canonical answer and one terminal result are emitted. Scheduler and heartbeat use typed turn origins and the original task body, reuse the same prepared contract, and must never infer entities from delivery envelopes.
...
- Native CLI/ACP remains administrator-only. Non-admin actors use the restored in-process function-calling fallback with the actor-bound registry; absence of its LLM configuration fails closed.
