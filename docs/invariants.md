...
- Public user login must be phone number + SMS verification as the primary path. The active, non-revoked admin-created web invite user list is the whitelist source; phones outside that list must fail closed before SMS send/check. Aliyun SMS credentials must come from environment/configuration, never committed source.
