...
- Native CLI/ACP runners (`gemini_cli`, `codex_cli`, `codex_acp`, `opencode_acp`, and `multi-agent`) are trusted-host capabilities because their subprocesses may inspect files or process state outside Hone's actor-bound tool registry. Non-admin actors must be routed to the in-process `function_calling` runner, and the request must fail closed when that safe runner has no configured LLM. Only explicit channel/CLI administrators may retain the configured native runner.
