...
- Deep single-stock/outlook turns are code-enforced: entity plus same-symbol quote must be verified before execution; current profile/financial/news and relevant earnings-calendar evidence are required for the full route. The final answer must pass the nine-section, timestamp, fact/inference, and two-method valuation contract; one reset/retry is allowed, then fail closed.
