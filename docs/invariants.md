...
- Direct, uniquely identified deep single-stock/outlook turns are code-enforced: entity plus exact same-symbol quote must be verified before execution; current profile/financial/news and relevant earnings-calendar evidence are required for the full route. Scheduler/heartbeat envelopes, multi-security comparisons, and generic finance/report acronyms must not enter this single-stock guard. The final answer must pass the nine-section, timestamp, fact/inference, and two-method valuation contract; one reset/retry is allowed, then fail closed.
