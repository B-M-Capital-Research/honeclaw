...
### 2026-08-29 — Hari Invest 0.3.0 与 SNDK 方法迁移成为默认投资内核

- 默认内核明确分为五层：六条 L4 老王确认逻辑、SNDK 通用方法迁移、历史公司研究基线、本轮当前证据、候选/开放知识。层级与调用顺序固定在 `skills/hari-invest/references/kernel-manifest.md`。
- 深度个股统一采用“需求 → 供给/替代 → 公司价值捕获 → 财务兑现 → 悲观/基准/乐观情景 → 反向隐含要求 → 条件化动作”。该链条只规范分析完整性，不能把 SNDK 当时数字、观点或动作迁移为今天的事实。
- `LOG-V0001`—`LOG-V0006` 的内容和版本保持不变；2026-08-13 的完整决策闭环、拥挤度冲突优先级及训练/自主操盘仍未达到 L4。默认提示和 CI 契约会阻止它们被写成老王固定动作。
- `CHANGELOG.md`、产品 eval 集、来源清单、仓库地图和交接文件已经补齐。用户授权范围是提交并推送 `oldwang`，不包括生产部署、自动训练、券商连接或真实交易。
