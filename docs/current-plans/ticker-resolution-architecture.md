...
- 2026-07-18: post-review removed the remaining interactive keyword/depth classifier, fixed-section validator, and deterministic whole-answer fallback. Dynamic contracts derive entity/quote scope from the tools the Agent selected and enforce deterministic truth rather than presentation. Genuine no-coverage or equal-candidate ambiguity preserves the Agent's specific explanation; only omitted explicit seeds or unique entities lacking exact quote/time remain unsafe incomplete. Regression coverage includes deliberately unmodeled wording plus shallow, financial, news, web, no-coverage, and ambiguity traces.
- 2026-07-18: the first exact production replay after deployment confirmed the new entity/tool architecture: the Agent independently searched, quoted, profiled, loaded financials, and loaded news for both `CRWV` and `NBIS`, and runtime logged `contract_built=true entities=CRWV,NBIS`. Delivery still failed because the dynamic repair path indirectly reused the typed fixed comparison instructions and legacy validators parsed the Agent's unconstrained valuation/news prose as if it had fixed chapters. This is an output-boundary regression, not a DataFetch/entity failure. The hotfix removes typed-format injection and arbitrary prose financial/event parsing from interactive validation; its second production Web/SSE replay is still pending.
