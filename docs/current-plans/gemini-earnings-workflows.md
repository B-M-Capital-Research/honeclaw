...
- 以生产 Dify `V2-财报前瞻` 的原始 LLM prompt 为主，恢复 `1.2.3` 对上期财报指引、真实机构评级/建议、机构营收与利润判断、当前股价预期门槛、管理层会议及订单新闻的逐项比较。
- 新闻选择回到公司强相关：公司公告、上次财报/电话会、明确点名公司的机构观点，以及能证明与公司本季收入/利润/产能传导关系的客户、供应链事件。剔除纯股价异动、会议出席、泛行业风险偏好和无法建立公司传导链的新闻。
- `1.3` 每条新闻改成独立自然段，不再输出字段流水账或 Markdown 列表；来源只显示纯文本来源名称，不展示超链接或 URL。
- 将新闻相关性和机构观点对比做成 renderer 可验证的私有审计契约，补齐回归测试、生产技能热更新和 AAOI 真机样片。
