...
- updated_at: 2026-08-06 20:10 CST
- owner: Codex
- related_files:
  - `docs/bugs/README.md`
  - `docs/bugs/*.md`
...
- 2026-08-06 20:10: Closed the active P2 `feishu_scheduler_silent_noop_still_sends_report` at code level. The recent `P0` watchlist-price candidate did not have a safe same-round quote-conflict reproduction after the 2026-08-06 17:55 CST user-feedback recheck, so this run moved to the next scheduler bug with direct live evidence. The actual root cause was heartbeat parsing rather than delivery routing: rich plain-text bodies such as `数据时间 … **状态：noop — 无全新独立重大事件触发。**` bypassed the old short-form noop markers and were classified as `PlainTextTriggered`, which kept sending full reports. `crates/hone-channels/src/scheduler.rs` now treats explicit `状态/检查结论/检查结果/监测结论 = noop` and `无新增触发（noop）/无高权重触发（noop）` rich-text conclusions as `PlainTextNoop`, while preserving a triggered override so mixed historical text cannot hide a real alert. Added `heartbeat_rich_plain_text_noop_status_is_compatible_noop`, re-ran it plus `heartbeat_plain_text_noop_is_compatible_noop`, and passed `cargo check -p hone-channels --tests`. Live services were not restarted in this automation, so the bug doc moved to code-level `Fixed` pending natural runtime revalidation.

- 2026-08-02 23:20: Closed the active P0 `feishu_scheduler_watchlist_stockanalysis_abnormal_prices` at code level. The previous watchlist anchor guard only had enough signal when local hit-zone context was restored, so `Current Status: Price: $1,214.83` and multi-symbol live-price strings such as `LITE/COHR/MU $658.42/$244.47/$103.30` could still bypass the fail-closed path. `crates/hone-channels/src/scheduler.rs` now extracts verified prices from same-turn successful `data_fetch` `quote` / `extended_hours` / `snapshot.data.quote` results and blocks ordinary scheduler plus heartbeat delivery whenever user-visible precise current-price claims materially disagree with those verified quotes. Added 4 focused `scheduler_verified_quote_price_*` regressions, re-ran the existing `watchlist_price_anchor_guard` tests, and passed `cargo check -p hone-channels --tests`. Live services were not restarted in this automation, so the bug doc moved to code-level `Fixed` pending natural runtime revalidation.
- 2026-08-01 11:45: Closed the active P1 `runtime_process_absent_stops_channels_and_scheduler` at code level. The root gap was that source-checkout runtime startup still defaulted to a foreground `hone-cli start --build` shell lifetime, while `restart_hone` already relied on a detached background restart path. `hone-cli start` now supports `--detach`: the foreground parent can finish `--build`, then re-exec a detached hone-cli supervisor that writes `data/logs/hone-cli-start.log` and preserves the existing `data/runtime/current.pid` contract. `scripts/restart_hone.sh` now uses the same `--detach` path, and the source-runtime runbook / repo map now point to the detached contract instead of a long-lived foreground shell. Verified `cargo test -p hone-cli start::tests -- --nocapture`, `cargo check -p hone-cli --tests`, `rustfmt --edition 2024 --check bins/hone-cli/src/start.rs`, `bash -n scripts/restart_hone.sh`, and `git diff --check`. Live services were not restarted in this automation, so the bug doc moved to code-level `Fixed` pending runtime revalidation. The active P0 `feishu_scheduler_watchlist_stockanalysis_abnormal_prices` remains open because its live price-anchor root cause still is not safely closed by this detached-start fix.
- 2026-07-07 03:06: Closed the active P2 `codex_acp_transport_disconnect_request_failure` at code level. Shared `AgentSession` retry handling now gives ACP-style transient failures one lightweight rerun before surfacing the final error, covering `codex/opencode acp session/prompt idle timeout`, `stream disconnected before completion`, `stream closed before response`, `acp stream disconnected`, and `transport disconnected`. Added `retryable_transient_runner_error_text_matches_acp_disconnect_and_idle_timeout` and `transient_runner_failure_retries_once_before_returning_success`, and re-ran `empty_success_with_tool_calls_uses_fallback_after_retries`, `cargo check -p hone-channels --tests`, rustfmt check, and `git diff --check`. Live services were not restarted in this automation, so the bug doc moved to code-level `Fixed` pending runtime revalidation.
- 2026-07-06 03:41: Closed 5 shared-output-boundary P3 bugs at code level through the common `sanitize_user_visible_output(...)` path: relative company-profile write copy, model metadata warning leakage, `data_fetch` / fake public-market source wording, Feishu direct internal runtime progress copy, and Web direct internal skill/local-store wording. Added focused regressions for `Hone data_fetch quote` / `行情数据工具`, `我已经加载单股研究流程` / `本地没有 HPE 画像`, and `已更新 NVDA 公司画像` / `本轮已将 KLAC 建立为长期公司画像` / `长期研究框架沉淀为组合画像`; `cargo test -p hone-channels sanitize_user_visible_output_ --lib -- --nocapture`, `cargo check -p hone-channels --tests`, and `git diff --check` passed. Live services were not restarted in this automation, so the five docs were moved to code-level `Fixed` and remain pending runtime revalidation.
- 2026-07-04 03:06: Closed the active P1 `acp_events_session_new_env_secret_leak` at code level again, this time by hardening the log sanitizer against payload-shape drift instead of assuming only one `mcpServers[].env[]` array form. `acp-events.log` redaction now handles both `mcpServers` arrays and object maps, plus `env` arrays and object maps, so alternate ACP / runner serialization cannot bypass the whitelist-based value redaction. Verified `cargo test -p hone-channels log_acp_payload_redacts_session_new_mcp_env_values -- --nocapture`, `cargo test -p hone-channels log_acp_payload_redacts_session_new_mcp_env_object_map_values -- --nocapture`, `cargo test -p hone-channels parse_error_log_records_bounded_redacted_raw_line_preview -- --nocapture`, `cargo check -p hone-channels --tests`, `rustfmt --edition 2024 --check crates/hone-channels/src/runners/acp_common/log.rs`, and `git diff --check`. Queue now has no active `P0/P1`; remaining items are `P2/P3`. Live runtime was not restarted in this task, so the old `acp-events.log*` evidence remains a follow-up cleanup / revalidation item.
- 2026-06-28 10:00: Closed the active P2 `sessions_sqlite_mirror_stalled_after_successful_direct_replies` at code level. Cloud-authoritative session storage had regressed into “PG only” mode: `SessionStorage::new_cloud(...)` no longer initialized the optional local sqlite mirror, cloud writes never shadowed to `sessions.sqlite3`, and the sqlite mirror path still required a real local source file. `SessionStorage` now keeps PG as the cloud truth source while optionally dual-writing a local sqlite mirror, performs best-effort cloud-session -> sqlite backfill on startup when that mirror is configured, and `SqliteSessionMirror` now accepts synthetic `cloud_sessions/<id>.json` source paths without local file metadata. Verified `cargo test -p hone-memory cloud_runtime_backend_dual_writes_sqlite_shadow --lib -- --nocapture`, `cargo test -p hone-memory upsert_session_accepts_cloud_shadow_source_path_without_local_file --lib -- --nocapture`, `cargo test -p hone-memory shadow_sqlite_backfills_existing_json_on_startup --lib -- --nocapture`, `cargo test -p hone-memory sqlite_runtime_backend_reads_from_sqlite --lib -- --nocapture`, `cargo check -p hone-memory --tests`, `cargo check -p hone-channels --tests`, `cargo check -p hone-web-api --tests`, and `git diff --check`. Queue still has no `P0/P1`; remaining active items are the ACP transport disconnect recovery, heartbeat near-threshold/structured-status drift, LLM audit PG serialization failure, and the three P3 wording leaks.
- 2026-06-25 03:06: Closed 2 active P2 heartbeat reliability bugs at code level: `scheduler_heartbeat_context_window_limit_no_recovery` and `scheduler_heartbeat_iteration_exhaustion_skips_alert`. Instead of further raising the shared heartbeat budget or masking failures, the scheduler now retries once in a narrower “budget recovery” mode when the first transient-task attempt fails with `context window exceeds limit` or `max_iterations_exceeded`. The recovery path uses a shorter heartbeat prompt, drops the delivered-history section, tightens tool usage to `max_tool_calls=2` with `data_fetch<=1` and `web_search<=1`, and requires `noop` when certainty cannot be recovered. Focused `hone-channels` heartbeat tests, `cargo check -p hone-channels --tests`, rustfmt check, and `git diff --check` passed. Active queue still has no `P0/P1`; remaining active bugs are the LLM audit PG serialization failure, ACP transport disconnect recovery, heartbeat near-threshold instability, heartbeat structured-status drift, and the three P3 wording leaks.
- 2026-06-20 03:05: Closed the active P2 `feishu_direct_etf_holding_wrong_entity_write` at code level. Shared finance-domain prompt policy now explicitly blocks deictic/write-side effects such as “this ETF / 这一只” from updating holdings, average cost, cron/heartbeat tasks, or company profiles before the user confirms a unique ticker/entity; multi-agent search-stage guidance mirrors the same write-oriented clarification rule so an older context ticker cannot be treated as sufficient authority for a persistent-state write. Verified `cargo test -p hone-channels build_prompt_bundle_always_includes_finance_domain_policy --lib -- --nocapture`, `cargo test -p hone-channels search_input_guidance_allows_direct_replies_for_greetings --lib -- --nocapture`, and `cargo check -p hone-channels --tests`.
- 2026-06-13 03:06: Closed the reopened active P1 `feishu_direct_answer_idle_timeout_no_reply` at code level. Feishu direct interrupted-session recovery is no longer startup-only: the channel now runs the same recovery scan once at boot and then every 60 seconds, while skipping sessions still marked active in `SessionLockRegistry` so normal long-running turns are not misclassified. This closes the specific “user-only session stays hung until the next restart” gap without requiring a service restart during validation. Targeted `hone-feishu` / `hone-channels` tests, `cargo check -p hone-feishu -p hone-channels --tests`, rustfmt check, and `git diff --check` passed.
