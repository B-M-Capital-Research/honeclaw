...
- Existing session, auth, audit, portfolio, cron, notification prefs, KB, and company profile runtime stores are not fully PG-backed yet. Durable files from the local `data/` snapshot have been uploaded/indexed, quota is PG-backed in cloud mode, and selected runtime file surfaces now write OSS in cloud mode, but the remaining hot-path repositories stay local until follow-up adapters land.
