...
- The first two historical file batches promoted 297 resources: batch one uploaded 186 immutable R2 objects and reused 7; batch two reused/read back all 104 objects after a transient first-attempt upload failure and then atomically updated all 104 PG rows. Both batches completed with zero conflicts. One source `.xls` was independently downloaded twice with identical bytes and verified as a valid OOXML workbook; the backfill permits only this narrow `.xls` metadata → XLSX magic alias, and the client corrects its download name to `.xlsx`.
