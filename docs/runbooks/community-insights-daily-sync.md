...
Track topic text, attachment bytes, and publication separately. Importing complete posts and attachment names makes the timeline metadata current; it does not complete attachment synchronization. A file or image is archived only after its legitimate bytes have passed validation, R2 readback, and promotion to `stored`. A publisher no-op cannot prove that metadata-only attachments are available.
...
## Optional official CLI authorization and discovery

The [Knowledge Planet guide](https://doc.zsxq.com/zsxq-skill-ai-tool-guide.html) identifies `zsxq-cli` and [unnoo/zsxq-skill](https://github.com/unnoo/zsxq-skill) as official tools. On September 5, version `0.5.1` was installed only under the ignored task directory with `npm install --prefix data/community-imports/2026-09-05/tools/zsxq-cli-0.5.1 --ignore-scripts --include=optional --no-audit --no-fund --save-exact --registry=https://registry.npmjs.org zsxq-cli@0.5.1`. The main and `@zsxq/cli-darwin-arm64` tarballs matched their registry SHA512 integrity values, and the installed binary matched the verified platform package. Both packages have no lifecycle scripts. The binary SHA256 is `0c854e2bba3fa966dc267c6d403919ece9fc01abbc089e37116b9f0f181d09fc`; non-secret evidence is in `data/community-imports/2026-09-05/official-zsxq-cli-install-audit.json`. Do not replace this reviewed version with an implicit `latest` installation during a scheduled run.

This CLI uses OAuth device authorization and system Keychain storage. `auth login --no-browser --json` displays authorization information and waits while polling. Give the user only the official verification URI and user code for reviewing the account-level permissions on the official page. The observed confirmation page says approval creates an authorization key. Version `0.5.1` exposes no `--scope` option, so do not claim it has been restricted to read-only OAuth scopes. Leave confirmation to the user unless they explicitly authorize confirmation of the displayed permissions; do not infer approval from elapsed time, export tokens, or obtain browser credentials. `--no-wait` exits before completion; `--device-code` resumes a pending flow. **`auth status` can also resume pending authorization and exchange a token**, so it is not always a purely read-only status check. These observations do not imply that a pending user authorization has completed.

After the user completes authorization, use `api list` to discover the server's actual tools, then `group +topics --group-id 51115212285814 --limit 10 --json` and `topic +detail --topic-id <returned-native-id> --json` for narrowly scoped reads. Review each candidate tool's schema and operation before invoking it. Group membership and any required group Skill enablement still apply. At the time of this audit, **file download/export was not demonstrated**; documented `--files` flags upload attachments. This optional discovery route is not the default sync workflow. Do not invent a download endpoint, invoke source writes, or replay a Chrome-blocked signed URL through it. Adopt it for synchronization only after normal authorization and a reviewed read/download canary establish the actual capability and identity mapping.
...
`metadata_only` means bytes have not been archived. The importer assigns it to every new resource before any download attempt; it is not a source-permission decision. Keep unknown/unattempted files distinct from a download attempt that failed. `protected_in_app` is a historical explicit state and must not be inferred from missing bytes. Only describe a resource as source-restricted when its source UI actually reports that restriction, and preserve the dated observation rather than assuming it remains current forever.

Record browser failures separately from source restrictions. For example, Chrome `ERR_BLOCKED_BY_CLIENT` means that the browser blocked this attempt; it does not prove that Knowledge Planet prohibited the member from downloading. Record the resource ID, failure category, observation time, and needed next step in the private run result. Do not mark all other missing files blocked because one attempt failed. A screen lock or browser connection failure is likewise a local execution condition.

An incomplete attachment phase does not invalidate complete visible topic text or other accessible images. Keep missing bytes metadata-only, retain any established historical state, and continue archiving other legitimate resources while reporting attachments as incomplete. Do not repeatedly trigger the same blocked download or bypass a browser/source restriction. A previously obtained ordinary local file may be reused after its source/topic/attachment identity and bytes are verified; a matching filename alone is insufficient. Preserve prior verified bytes if an attempted replacement is unavailable. A known group homepage may be offered as a group entry, but a content-derived `topic_...` identity must never be turned into an invented original-post URL.
...
Write a small, secret-free `result.json` or `result.md` in each run directory. Retain the production authority fingerprint, source check time, live/source head times, inserted topic/resource-metadata counts, newly archived resource count/bytes, separate `metadata_only` and historical `protected_in_app` counts, actual browser/source failure observations, unattempted scope, append replay result, publication result/prefix, elapsed time, and any recovery entry point. Record text, attachment, and publication completion independently. Keep complete source text and private manifests out of user-facing summaries.

- `CONTENT_SYNCED`: the observed source delta and projection are current, but one or more attachment references still lack verified bytes. State how many attachments were archived and how many remain pending or have a specific observed restriction. Newly imported filenames do not count as archived resources.
- `SYNCED`: the observed source delta and projection are current and every attachment in the verified scope has archived bytes. State that scope; never imply that unexamined historical attachments or a later unseen source update were verified.
...
- `NO_CHANGES`: source and production agree, no resource recovery occurred, and publisher is already a no-op. Retain any pending attachment counts and observations; unchanged metadata-only files do not become complete. Record locally and produce no new user-facing finding.
