...
- `Codex ACP`
  ...
  - Each deterministic Hone logical session owns one native Codex session. The first compatible turn stores the `session/new` ID with `codex_acp_session_mode=native_turn_v2` and an instruction fingerprint; later turns call `session/resume` only when both markers match. This preserves one multi-turn conversation in Codex and leaves compaction to the Codex harness.
  ...
  - A session without the exact v2 mode/fingerprint is treated as legacy or instruction-mismatched. Its next Codex turn creates a new native generation without seeding the old Hone transcript; the previous Codex task is left intact for inspection.
