...
- `Codex ACP`
  ...
  - Hone passes `model`, `variant`, safety settings, extra overrides, and the complete static instruction layer through the adapter-supported `CODEX_CONFIG` environment before ACP startup. It does not call ACP `session/set_model`, whose model-id format is adapter-version-specific.
  - Each deterministic Hone logical session owns one native Codex session. Only a session with no stored native ID calls `session/new`; the returned ID, `codex_acp_session_mode=native_turn_v2`, and instruction fingerprint are checkpointed to authoritative Hone storage before the first `session/prompt`. Every later turn resumes that ID even when instructions, mode metadata, code revision, process, or compaction state changed. Mode/fingerprint are audit facts, not session-rotation triggers.
  - Every ACP `session/prompt`, including the first turn and the first turn after native `contextCompaction`, sends only the normalized current user content (including current time, actual attachments/images, or an explicit slash-skill task). It never embeds Hone's static instructions, restored user/assistant history, tool calls/results, compact summary, session ID, channel-routing metadata, related-skill hints, entity-loop guidance, or answer contracts. Compaction is telemetry only and never schedules a seed/reseed.
  - Enabled bundled/custom Hone skills are linked individually into each trusted persistent actor workspace at `.agents/skills/hone__*`. Codex discovers and reads these as native skills; only live Hone data/action tools remain on MCP. Do not point the actor workspace back at the source checkout or manually edit the generated links.
  ...
  - Existing legacy mode/fingerprint metadata is upgraded in place while resuming its nonempty stored native ID. If an operator intentionally wants a different native task, that is an explicit metadata repair/reset operation; normal turns never fork automatically. Native prompt transport failures are not automatically resent because the original thread may already have accepted the turn.
  - All repository Codex manual probes source `tests/regression/manual/codex_probe_home.sh`, which creates a temporary `CODEX_HOME` and links only the existing authentication file. Probe sessions/logs must stay outside the user's primary Codex Desktop task index and be removed with the temporary directory.
