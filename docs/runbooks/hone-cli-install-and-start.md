...
- `Codex ACP`
  ...
  - The first native prompt sends Hone's static system instructions; the first successful prompt after a native `contextCompaction` sends them once again. An ordinary trusted Interactive resume sends only the current Beijing time and normalized current user content (including actual attachments/images or an explicit slash-skill task). It does not resend Hone history/session ID, channel-routing metadata, related-skill hints, entity-loop guidance, or answer contracts.
