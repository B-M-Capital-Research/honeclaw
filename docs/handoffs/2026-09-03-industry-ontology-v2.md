...
- ont3 没接上的三题都是公司研究稿（soul.md 的「1. 结论 / 2. 公司是什么 / 3. 护城河 / 4. 行业位置与关键对手 …」
  大纲），估值稿的对账表槽位对它们不起作用。soul.md 不动，改注入头部：公司研究稿落在「行业位置与关键对手」
  一节的第一句，估值稿落在对账表行 / 增长来源 / 基准档第一项输入；每处都从那条最近动作写起，再写传导闸门，
  最后用更新一季覆盖。
- 发布：commit `a2d76ea4`（只改 prompt.rs 一句），镜像
  `…@sha256:c356d9a78b2536108ce3d70010e96e7855f421c3f6d8f9df42621f0c64ab1e7d`，13:4x UTC 切换
  （current → a2d76ea4，previous → 1950b5b8，91e78e74 已清；NRestarts=0，无 error）。harness 不变（35a422cd）。
- 复测 `ont4`（同 8 题，13:45–13:53 UTC）：**6/8**（脚本口径：提及 + 最近财报/指引 + 带日期）。成员公司 7 家里
  7 家都写进了英伟达截至 2026-08-26 的 FY27Q2 动作：MU 6/6 个数字、SNDK 6/6、AAOI / VST / COHR / NBIS 各 3 个；
  CRDO 走的是「更新一季覆盖」——用本轮 `earnings_outlook` 取到的总收入 $96.2B / +106% / Q3 指引 $108.7B 写需求源头，
  数字比本体那条新，但同样带日期。COHR 有数字、脚本没认出日期写法。RKLB 不在树里（对照，始终 0）。
  四轮：ont0 0 → ont2 0（事实进提示）→ ont3 4（估值模板槽位）→ ont4 6（公司研究稿点名落位）。
- 再往后的候选：在 pre-turn enrichment 里对成员公司预取一次上游的 `earnings_outlook`，让「更新一季」不靠模型
  自觉；把 `latest` 的季度更新做成定时任务（英伟达财报后自动生成草稿，管理员确认）。
