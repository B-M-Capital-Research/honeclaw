...
## 上线后复测（同一批 20 题，同一份评分卡）

| | before | after | Δ |
|---|---:|---:|---:|
| **平均分** | **6.25** | **6.65** | **+0.40** |
| C1 结论落到数 | 0.60（0 分 60%） | 0.95（0 分 37%） | +0.35 |
| C2 胜负手 | 1.16 | 1.58 | +0.42 |
| C3 稀缺度与差异化 | 0.82（0 分 18%） | 1.11（0 分 6%） | +0.29 |
| C4 估值先定类型 | 0.65（0 分 47%） | 0.72（0 分 33%） | +0.08 |
| C5 Bull/Bear 量化 | 0.72（0 分 39%） | 1.11（0 分 26%） | +0.38 |
| C6 可执行 | 1.45 | 1.60 | +0.15 |

路由这一层的改变最直接：**加载了场景/视角 skill 的题从 7/20 变成 17/19**。
5 道「帮我分析一下 X」现在每一道都加载了 `stock_research` + `valuation-audit`；
`aaoi为什么突然大跌` 第一次加载了 `market_analysis`；q09 加载了 `moat` + `scarcity-differentiation`；
两道控制题（新闻、知识）仍然不加载场景 skill，没有被误伤。

两条能直接对上老王原话的：

- VST（他原话「没有对稀缺度、差异化的评价…他的稀缺度会拉高估值」）现在写的是
  「稀缺度/差异化把 VST 的基准倍数从传统独立发电商历史中枢（约 8.0x）上调至 9.0x~10.0x EV/EBITDA」——
  稀缺度显式抬倍数，且用了 EV/EBITDA 而不是他批评的单一 PE。
- QQQ（他要的第 4、5 步）现在逐家算出 forward EPS × 倍数 → 每家合理价 → 按权重加权 →
  ETF 合理价区间 $779.33–$912.09 → 现价 $716.43 的位置。这两步之前都是 0 分。

### 但复测暴露了一个新的失败形态，已经修掉

**模型现在会算三情景，也会给"合理价值区间"，但两者对不上。**

- MU：三情景 $366.84 / $910.00 / $1,265.00，结论段却宣布 $675–$1,207.50；
  而且基准档 $910.00 已经**低于现价 $932.86**（=基准下略高估），被"48.4% 分位、持有观望"盖过去。
- GEV：同一篇里出现三套互不相等的悲观估值（$397.93 / $733.33 / 结论下沿 $750）。
- 泡泡玛特：三情景 112.56 / 167.20 / 213.94，结论段写 140~185，把 Bear 的 −29% 悄悄收窄成 −12%。
  这条从 9.0 掉到 6.5，是本轮唯一一处真实回退。

修法（`95af942d`）：把**三情景表定为全文唯一的价格来源**——区间下沿逐字取悲观那行、上沿逐字取乐观那行，
Bull/Bear 段里的价位必须是同样这三个数；基准档低于现价时必须直说"基准情景下已略高估"，
不许用更宽的区间盖住。同时补一条：**说了做交叉验证就要拿出第二个数**
（打分里反复出现"宣称 EV/EBITDA 交叉验证但全文零 EBITDA 数值"，这正是 C4 几乎没动的原因）。

### 关于分数噪声的说明

before / after 由两组独立的评分 agent 打，10 分制的折算有 ±0.5~1.0 的噪声。
q03 前后**逐维度得分完全相同**（C1:2 C2:2 C3:1 C4:1 C5:2 C6:2）却报 8.5 → 8；
q11 维度上还涨了一档（C6 1→2）却报 6 → 5。所以**单题 1 分以内的差不要当结论看**，
维度均值和 0 分占比才是可比的。真实回退只有泡泡玛特那一条，原因已定位并修复。

### 异动题（q15）：根因修掉了一半

`aaoi为什么突然大跌` 从 0.5 变成 1.0，仍然是模板兜底，但**失败的位置往后挪了**：

- before：`target_date=None, eligible_source_count=0, cause_evidence_missing=true` → 连重试都不给，直接兜底。
- after：`target_date=Some(2026-08-28), eligible_source_count=1, cause_evidence_missing=false` → **重试了 3 次**。
  日期锚定和取证都做到了，日期+URL 那条违规在第一次重试后就消失了。

卡住它的是另外两条检查，都复现了是误判：

1. 收盘价声明正则 `(?:收报|收于|收在|收)\s*[$¥€£]?\s*[0-9]` —— 裸的 `收` 会命中
   **营收 6.5 亿 / 回收 3 亿 / 吸收 5 家**，任何量化了营收的草稿都被判成"把 quote price 写成收盘价"。
2. 涨跌幅对账把**同行 ticker 后 80 字内的任意百分比**都要求等于 `hone_change_basis.pct`。
   而异动题按老王的要求本来就要量化事件影响，于是
   `AAOI 的毛利率从 22.1%`、`对 AAOI 的影响：合理价下移约 6.6%` 全被判成写错了涨跌幅。

修法（`351c4f2c`）：前者要求真正的收盘动词；后者只在前导文字没有点名别的指标时才对账，
原来那条"provider changesPercentage 顶替服务端口径"的用例仍然被 `market_move_quote_evidence_prefers_server_change_basis` 拦住。
两条都补了回归测试。**这两处修好后 q15 还没有复测过**，需要下一轮验证。
