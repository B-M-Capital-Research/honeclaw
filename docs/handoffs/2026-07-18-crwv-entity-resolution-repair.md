...
The repair stays at the generic provider adapter boundary. `normalize_clean_eof_after_finish` emits one internal `Done` only when the lower stream reaches error-free EOF after exactly one typed finish and no earlier explicit Done. Literal `[DONE]` terminates once; EOF without finish, duplicate finish, and parser/transport error—including an error after finish—never synthesize completion. `FunctionCallingAgent::require_complete_stream` remains strict, and the dedicated OpenRouter adapter keeps its explicit-sentinel behavior. Regression coverage includes the exact MiniMax fragmented tool/finish shape, normal stop completion, literal-DONE deduplication, no-finish EOF, duplicate finish, and finish-then-transport-error. Focused `hone-llm` tests pass 32/32 and the architecture contract passes 33/33. Full exact-commit gates, redeployment, three production canaries, and final health evidence are still required before this phase is complete.
