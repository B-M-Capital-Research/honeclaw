...
- related_docs:
  - docs/archive/plans/public-community-assets-navigation-fix.md
...
- The completed source audit leaves 113 file attachments explicitly protected by the source platform and one unresolved metadata row (`resource_id=834`). They intentionally remain metadata-only; the other 651 originals are stored and previewable/downloadable through the authenticated resource route.
...
Done. Code, content reconciliation, all legitimately retrievable originals, service restart, frontend production deployment, data audit, and queue cleanup are complete.
...
- The screenshot PDF plus every other legitimately retrievable historical attachment were downloaded through the visible source UI, validated, uploaded, and promoted. The final file ledger is 651 stored originals, 113 explicitly source-protected rows, and one unresolved metadata row (`resource_id=834`), totaling 765 file resources.
- Ten logical attachment batches promoted all 651 originals. The files total 2,614,811,800 bytes: 642 PDF, 7 DOCX, 1 PPTX, and 1 XLSX. All 651 passed magic, byte-size, and SHA-256 verification; all nine OOXML packages passed ZIP and internal-structure checks. Every successful apply read its R2 object back and completed with zero final conflicts. The 104-object second batch correctly held PG at zero updates after one transient first-attempt failure, then completed 104/104 on retry.
- Resource 951 remains the ambiguous old protected Starbucks shadow row; the independently reconciled duplicate source position is stored as resource 1016. No bytes were guessed or aliased across those rows.
...
- Backend source runtime was rebuilt and restarted on the current `0.14.1` codebase; no release tag was created. Commits `879e9722`, `af3cb605`, and `7ab36682` were pushed to `main`, and Cloudflare Pages published `assets/index-BB8Wrwbl.js`.
...
- Rust: 118 `hone-core` tests; 106 `hone-web-api` tests passed with two live-credential tests ignored; 67 `hone-cli` tests; package checks and changed-file formatting passed.
- Web: 242 tests, TypeScript check, and public production build passed.
...
- Historical attachment capture is complete. Every file row is final as stored, visibly App-only protected, or the single unresolved resource 834 after independent deep visible-UI search. Signed source URLs were never logged or persisted; the final audit reports no URL, credential, duplicate-ID, or local/manifest mismatch.
