...
- Verification: 13 LLM tests, 7 function-calling tests, 500 channel tests, 101 Web API tests with two credentialed tests ignored, 218 frontend tests, frontend typecheck/public production build, workspace all-target check excluding Apple clients, production `assistant_reset` bundle fingerprint, local/origin/Worker health, and restarted backend/Feishu/Discord processes. Chrome had no authenticated HONE cookie, so no production session token was copied or fabricated for a synthetic send; the next normal signed-in user turn is the remaining hands-on observation point.
