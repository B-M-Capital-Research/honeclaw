...
- `data/sessions/*.json` / `data/runtime/logs/acp-events.log`
  - 巡检时间窗：2026-06-17 15:17-19:01 CST；`data/sessions.sqlite3` 的 `session_messages` 未追平该窗口，因此本轮改用 JSON 会话源与 ACP 事件核对用户可见 final。
  - `data/sessions/Actor_feishu__direct__ou_5f62439dbed2b381c0023e70a381dbd768.json` 在 15:30 CST 更新，末尾 assistant final 对应 `美股盘前要闻、持仓评级与机会观察`，用户可见文本仍写出 `本轮沙盒内未读取到 data/portfolio/holdings.json`，并以“会话中最后确认过的持仓版本”为回退口径生成分析。
  - 同一窗口 `data/runtime/logs/acp-events.log` 仍在 15:17-18:33 CST 记录 Feishu ACP session 启动、工具调用与 `stopReason=end_turn`，说明真实会话链路仍有收口；异常集中在旧 live 进程读取持久化 portfolio 数据根时仍会按沙盒空目录判断。
  - 该样本与 07:00 CST 样本同根同链路，且仍发生在 03:10 CST 代码级修复之后、live 服务尚未确认重启/部署的窗口；因此继续作为“代码级 `Fixed` 但 live 未生效”证据保留，不把状态从 `Fixed` 回退为 `New`。
- `data/portfolio/`
  ...
  - 2026-06-17 15:30 CST JSON 会话样本对应的 `portfolio_feishu__direct__ou_5f62439dbed2b381c0023e70a381dbd768.json` 仍真实存在且非空，包含 LITE、COHR 等持仓及多条 tracking-only 标的；因此 `data/portfolio/holdings.json` 为空不是用户真实没有持仓。
...
- `2026-06-17 19:01 CST` live 旧进程补证：
  - `data/sessions.sqlite3` 在本轮未追平 15:17-19:01 CST，会话证据来自 JSON 源与 ACP 日志。
  - 15:30 CST `美股盘前要闻、持仓评级与机会观察` JSON final 仍写出未读取到沙盒 `data/portfolio/holdings.json`，对应 actor 的 portfolio 文件真实存在且非空。
  - 该样本仍按“代码级 `Fixed`、等待 live 重启/部署后复核”处理，不重新登记为 `New`；已有 GitHub Issue [#49](https://github.com/B-M-Capital-Research/honeclaw/issues/49) 不重复创建。
