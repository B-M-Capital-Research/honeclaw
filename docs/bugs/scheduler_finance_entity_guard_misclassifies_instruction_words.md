...
- `2026-07-30 18:00-22:03 CST` 真实运行态继续复发，状态维持 `New/P2`：
  - `data/sessions.sqlite3` / `session_messages` / `cron_job_runs`
    - 20:00 CST Feishu scheduler `每日名人基金美股操作跟踪` 的任务正文要求跟踪 Nancy Pelosi / Cathie Wood / ARK Invest / Berkshire / Li Lu / Dan Bin 等公开披露和美股相关操作。
    - 20:01 assistant final 只返回“已识别证券代码 `13F`，但当前数据供应商没有返回同代码行情覆盖”，未生成名人基金 / 公开披露追踪主体内容。
    - `cron_job_runs` 同条记录落成 `heartbeat=0`、`execution_status=execution_failed`、`message_send_status=sent`、`should_deliver=1`、`delivered=1`，说明失败提示已作为用户可见 scheduler 结果发送。
  - 判断：
    - 最新样本仍是 scheduler 任务正文中的披露表单 / 金融语境词进入证券实体 guard / resolver 后误抽为 ticker，导致业务正文被阻断；与既有 `13F` / `NAND` / `ARK` / `Nancy` / `PCE` / `SEC` 误抽同根，不新建重复缺陷。
    - 严重等级维持 `P2`：它直接阻断部分 scheduler 正文生成并发送失败提示，但同窗 direct / scheduler 仍有正常收口，未见全渠道停摆、错投、敏感信息泄露或持久化数据破坏，因此不是 `P1`，不创建 GitHub Issue。
...
## 运行态复核（2026-07-29 10:02 CST）

- 本轮 2026-07-29 06:01-10:02 CST 真实运行态继续复发，状态维持 `New/P2`：
  - `data/sessions.sqlite3` / `cron_job_runs`
    - 08:30 CST Feishu scheduler `闪迪(SNDK)每日行情与行业简报` 的任务正文明确要求调取 `SNDK` 行情、华尔街评级变化及 `NAND Flash` 存储行业动态。
    - assistant final 只返回“已识别证券代码 `NAND`，但当前数据供应商没有返回同代码行情覆盖”，未生成 SNDK 简报主体。
    - `cron_job_runs` 同条记录为 `heartbeat=0`、`execution_status=execution_failed`、`message_send_status=sent`、`should_deliver=1`、`delivered=1`，说明失败提示已作为用户可见 scheduler 结果发送。
  - 判断：
    - 最新样本仍是 scheduler 任务正文中的行业词进入证券实体 guard / resolver 后误抽为 ticker，导致业务正文被阻断；与既有 `NAND` / `PCE` / `ARK` / `Nancy` / `SEC` 误抽同根，不新建重复缺陷。
    - 严重等级维持 `P2`：它直接阻断部分 scheduler 正文生成并发送失败提示，但同窗 direct / scheduler 仍有正常收口，未见全渠道停摆、错投、敏感信息泄露或持久化数据破坏，因此不是 `P1`，不创建 GitHub Issue。
...
- 本轮 2026-07-27 07:02-11:02 CST 真实运行态再次复发，状态从代码级 `Fixed` 回退为 `New/P2`：
  - `data/sessions.sqlite3` / `cron_job_runs`
    - 08:30 CST Feishu scheduler `闪迪(SNDK)每日行情与行业简报` 的任务正文要求调取 SNDK 行情、评级变化及 NAND Flash 存储行业动态；assistant final 只返回“已识别证券代码 `NAND`，但当前数据供应商没有返回同代码行情覆盖”，未生成 SNDK 简报主体。
    - `cron_job_runs.run_id=48684` 记录 `heartbeat=0`、`execution_status=execution_failed`、`message_send_status=sent`、`should_deliver=1`、`delivered=1`，说明失败提示已作为用户可见 scheduler 结果发送。
    - 09:30 CST Feishu scheduler `持仓股收盘复盘与美股新闻整理` 又把正文里的 `Nasdaq` 落成多个证券实体候选，返回要求补交易所后缀或公司全名；`run_id=48723` 同样为 `execution_failed + sent + delivered=1`。
  - 判断：
    - 最新样本晚于 2026-07-26 代码级修复，仍是 scheduler 任务正文中的行业词、指数名或上下文词进入证券实体 guard / resolver 后误抽、误拦，导致业务正文被阻断；与既有 `PCE` / `ARK` / `Nancy` / `SEC` / `NAND` 误抽同根，不新建重复缺陷。
    - 严重等级维持 `P2`：它直接阻断部分 scheduler 正文生成并发送失败提示，但同窗 direct / scheduler 仍有正常收口，未见全渠道停摆、错投、敏感信息泄露或持久化数据破坏，因此不是 `P1`，不创建 GitHub Issue。
...
- 本轮 2026-07-25 15:01-19:02 CST 真实运行态继续复发，状态维持 `New/P2`：
  - `data/runtime/logs/web.log.2026-07-25`
    ...
    - 15:30 / 16:00 / 16:30 / 17:00 / 17:30 / 18:00 / 18:30 / 19:00 CST `ORCL 大事件监控` 继续因 Oracle 多上市地候选落成 `runner_error`，要求补交易所后缀或公司全名。
    - 同窗整体仍有 `runner_error / execution_failed=44`，实体误抽 / 多候选 fail-closed 仍是 heartbeat 失败来源之一。
  - `data/sessions.sqlite3`
    - 同窗新增 8 条 user / 5 条 assistant / 2 条 system compact，覆盖 2 个更新 session；普通 Feishu direct 与 Web scheduler 仍有 assistant 收口，未见全渠道停摆、错投或敏感信息泄露。
  - 判断：最新样本仍是 scheduler / heartbeat 任务正文、监管缩写或上市地候选进入实体 guard / resolver 后误抽、误拦或多候选拦截；与既有缺陷同根，不新建重复缺陷。
  - 严重等级维持 `P2`：它直接阻断部分 scheduler / heartbeat 正文生成，但同窗其它 direct / scheduler 仍正常收口，未见全渠道停摆、错投、敏感信息泄露或数据破坏，因此不是 `P1`，不创建 GitHub Issue。

- 本轮 2026-07-25 07:02-11:02 CST 真实运行态继续复发，状态维持 `New/P2`：
  - `data/runtime/logs/web.log.2026-07-25`
    - 10:30 / 11:00 CST `AAOI 1.6T 光模块心跳检测` 继续把任务上下文里的 `SEC` 当证券代码，因数据供应商没有同代码行情覆盖而落成 `runner_error` / 不发送。
    - 10:30 / 11:00 CST `ORCL 大事件监控` 继续因 Oracle 多上市地候选落成 `runner_error`，要求补交易所后缀或公司全名。
    - 同窗整体仍有 `runner_error=28`，实体误抽 / 多候选 fail-closed 仍是 heartbeat 失败来源之一。
  - `data/sessions.sqlite3`
    - 同窗新增 38 条 user / 23 条 assistant / 10 条 system compact，覆盖 13 个更新 session；多条 direct / scheduler 仍有 assistant 收口，未见全渠道停摆、错投或敏感信息泄露。
  - 判断：最新样本仍是 scheduler / heartbeat 任务正文、监管缩写或上市地候选进入实体 guard / resolver 后误抽、误拦或多候选拦截；与既有缺陷同根，不新建重复缺陷。
  - 严重等级维持 `P2`：它直接阻断部分 scheduler / heartbeat 正文生成，但同窗其它 direct / scheduler 仍正常收口，未见全渠道停摆、错投、敏感信息泄露或数据破坏，因此不是 `P1`，不创建 GitHub Issue。

- 本轮 2026-07-25 03:01-07:02 CST 真实运行态继续复发，状态维持 `New/P2`：
  - `data/sessions.sqlite3`
    - 05:00 CST Web scheduler `盘后美股复盘与SNDK/MU存储产业链日报` 任务正文要求复盘宏观数据与政策事件，assistant 先返回“已识别证券代码‘PCE’，但当前数据供应商没有返回同代码行情覆盖”，随后写入用户可见 `定时任务...执行出错，请稍后重试。`
  - `data/runtime/logs/web.log.2026-07-24`
    - 03:30-07:00 CST `AAOI 1.6T 光模块心跳检测` 继续把任务上下文里的 `SEC` 当证券代码，因数据供应商没有同代码行情覆盖而落成 `runner_error` / 不发送。
    - 03:30-07:00 CST `ORCL 大事件监控` 继续因 Oracle 多上市地候选落成 `runner_error`，要求补交易所后缀或公司全名。
    - 同窗整体仍有 `runner_error=36`，实体误抽 / 多候选 fail-closed 仍是 heartbeat 失败主要来源之一。
  - 判断：最新样本仍是 scheduler / heartbeat 任务正文、宏观词、监管缩写或上市地候选进入实体 guard / resolver 后误抽、误拦或多候选拦截；与既有缺陷同根，不新建重复缺陷。
  - 严重等级维持 `P2`：它直接阻断部分 scheduler / heartbeat 正文生成，但同窗 direct / scheduler 用户可见 final 仍有正常收口，未见全渠道停摆、错投、敏感信息泄露或持久化数据破坏，因此不是 `P1`，不创建 GitHub Issue。

## 运行态复核（2026-07-24 19:01 CST）

- 本轮 2026-07-24 23:02-2026-07-25 03:02 CST 真实运行态继续复发，状态维持 `New/P2`：
  - `data/runtime/logs/web.log.2026-07-24`
    - 00:00 / 03:00 CST `AAOI 1.6T 光模块心跳检测` 继续把任务上下文里的 `SEC` 当证券代码，因数据供应商没有同代码行情覆盖而落成 `runner_error` / 不发送。
    - 00:00 / 02:30 / 03:00 CST `ORCL 大事件监控` 继续因 Oracle 多上市地候选落成 `runner_error`，要求补交易所后缀或公司全名。
    - 同窗整体仍有 `runner_error=34`，实体误抽 / 多候选 fail-closed 仍是 heartbeat 失败主要来源之一。
  - `data/sessions.sqlite3`
    - 同窗新增 16 条 user / 9 条 assistant / 6 条 system compact，覆盖 5 个更新 session；近期 direct / scheduler 仍有 assistant 收口，未见全渠道停摆、错投或敏感信息泄露。
  - 判断：最新样本仍是 scheduler / heartbeat 任务正文、监管缩写或上市地候选进入实体 guard / resolver 后误抽、误拦或多候选拦截；与既有缺陷同根，不新建重复缺陷。
  - 严重等级维持 `P2`：它直接阻断部分 scheduler / heartbeat 正文生成，但同窗 direct / scheduler 用户可见 final 仍有正常收口，未见全渠道停摆、错投、敏感信息泄露或持久化数据破坏，因此不是 `P1`，不创建 GitHub Issue。
...
- 本轮 2026-07-24 15:01-19:01 CST 真实运行态继续复发，状态维持 `New/P2`：
  - `data/runtime/logs/web.log.2026-07-24`
    ...
    - 15:30-19:00 CST `ORCL 大事件监控` 每半小时继续因 Oracle 多上市地候选落成 `runner_error`，要求补交易所后缀或公司全名。
  - `data/sessions.sqlite3`
    - 同窗新增 11 条 user / 3 条 assistant / 6 条 system compact，覆盖 3 个更新 session；近期 direct / scheduler 仍有 assistant 收口，未见全渠道停摆、错投或敏感信息泄露。
...
- 本轮 2026-07-24 03:02-07:01 CST 真实运行态继续复发，状态维持 `New/P2`：
  - `data/sessions.sqlite3`
    ...
    - 05:00 CST Web scheduler session `Actor_web__direct__web-user-afc1cabadbf8` 的 `盘后美股复盘与SNDK/MU存储产业链日报` 任务正文要求复盘宏观数据与政策事件，包含 `PCE`；assistant 先返回“已识别证券代码‘PCE’，但当前数据供应商没有返回同代码行情覆盖”，随后用户侧只看到 `定时任务「盘后美股复盘与SNDK/MU存储产业链日报」执行出错，请稍后重试。`
    - 06:31 前其它已收口 ordinary assistant final 未见全渠道停摆、错投、空回复、本机路径、provider 原始错误或 panic。
  - `data/runtime/logs/web.log.2026-07-23`
    - 05:00 / 07:00 CST `AAOI 1.6T 光模块心跳检测` 继续把任务上下文里的 `SEC` 当证券代码，因数据供应商没有同代码行情覆盖而落成 `runner_error` / 不发送。
    - 05:00 / 07:00 CST `ORCL 大事件监控` 继续因 Oracle 多上市地候选落成 `runner_error`，要求补交易所后缀或公司全名。
  - 判断：最新样本仍是 scheduler / heartbeat 任务正文、宏观词、监管缩写或上市地候选进入实体 guard / resolver 后误抽或多候选拦截；与既有缺陷同根，不新建重复缺陷。
  - 严重等级维持 `P2`：它直接阻断部分 scheduler / heartbeat 正文生成，但同窗 direct / scheduler 已收口样本仍有正常 assistant final，未见全渠道停摆、错投、敏感信息泄露或持久化数据破坏，因此不是 `P1`，不创建 GitHub Issue。

- 本轮 2026-07-23 23:02-2026-07-24 03:02 CST 真实运行态继续复发，状态维持 `New/P2`：
  - `data/runtime/logs/web.log.2026-07-23`
    - 23:30-03:00 CST `AAOI 1.6T 光模块心跳检测` 每半小时继续把任务上下文里的 `SEC` 当证券代码，因数据供应商没有同代码行情覆盖而落成 `runner_error` / 不发送。
    - 23:30-03:00 CST `ORCL 大事件监控` 每半小时继续因 Oracle 多上市地候选落成 `runner_error`，要求补交易所后缀或公司全名。
  - `data/sessions.sqlite3` 同窗新增 6 条 user / 6 条 assistant，覆盖 4 个更新 session；近期 direct / scheduler 均有 assistant 收口，未见全渠道停摆、错投或敏感信息泄露。
  - 判断：最新样本仍是 scheduler / heartbeat 任务正文、监管缩写或上市地候选进入实体 guard / resolver 后误抽或多候选拦截；与既有缺陷同根，不新建重复缺陷。
  - 严重等级维持 `P2`：它直接阻断部分 scheduler / heartbeat 正文生成，但同窗 direct / scheduler 用户可见 final 正常收口，未见全渠道停摆、错投、敏感信息泄露或持久化数据破坏，因此不是 `P1`，不创建 GitHub Issue。

- 本轮 2026-07-23 15:02-19:02 CST 真实运行态继续复发，状态维持 `New/P2`：
  - `data/runtime/logs/web.log.2026-07-23`
    - 15:30-19:00 CST `AAOI 1.6T 光模块心跳检测` 每半小时继续把任务上下文里的 `SEC` 当证券代码，因数据供应商没有同代码行情覆盖而落成 `runner_error` / 不发送。
    - 15:30-19:00 CST `ORCL 大事件监控` 每半小时继续因 Oracle 多上市地候选落成 `runner_error`，要求补交易所后缀或公司全名。
  - `data/sessions.sqlite3` 同窗新增 3 条 user / 3 条 assistant，覆盖 3 个更新 session；Feishu direct 与 Web scheduler 均有 assistant 收口，未见全渠道停摆、错投或敏感信息泄露。
  - 判断：最新样本仍是 scheduler / heartbeat 任务正文、监管缩写或上市地候选进入实体 guard / resolver 后误抽或多候选拦截；与既有缺陷同根，不新建重复缺陷。
  - 严重等级维持 `P2`：它直接阻断部分 scheduler / heartbeat 正文生成，但同窗 direct / scheduler 用户可见 final 正常收口，未见全渠道停摆、错投、敏感信息泄露或持久化数据破坏，因此不是 `P1`，不创建 GitHub Issue。
...
- `data/sessions.sqlite3`
  ...
  - 20:00 CST Feishu scheduler 任务 `每日美股大盘晚报` 之前的同 actor final 返回“已识别证券代码 `Nancy`，但当前数据供应商没有返回同代码行情覆盖”，把人物名误当证券代码。
  - 20:01 CST Feishu scheduler 任务 `每日名人基金美股操作跟踪` final 返回“已识别证券代码 `ARK`，但当前数据供应商没有返回同代码行情覆盖”，把机构 / ETF 语境词抢先当证券代码，未生成基金操作跟踪正文。
  - 21:00 CST Web scheduler `盘前美股要闻与SNDK/MU存储产业链日报` 继续把宏观指标 `PCE` 当证券代码，随后写入用户可见 `定时任务...执行出错，请稍后重试。`
- `data/runtime/logs/web.log.2026-07-23`
  - 19:30-23:00 CST `AAOI 1.6T 光模块心跳检测` 每半小时继续把任务正文中的 `SEC` 当证券代码，并因数据供应商无覆盖落成 `runner_error` / 本轮不发送。
  - 同窗 `ORCL 大事件监控` 每半小时继续因 Oracle 多上市地候选 fail-closed，要求补充交易所后缀或公司全名。
- 本轮判断
...
- `data/sessions.sqlite3` -> `cron_job_runs`
  ...
  - `run_id=50000` / Feishu scheduler `每日名人基金美股操作跟踪` 在 20:00 落成 `execution_failed/sent/delivered=1`，用户可见内容只说已识别证券代码 `13F`，但数据供应商没有同代码行情覆盖，要求检查交易所后缀或稍后重试。
  - 任务本意是跟踪名人基金 / 13F 持仓操作，`13F` 是监管披露表单语境，不是本轮要查询行情的证券代码。
- 本轮判断
  - 最新样本仍是 scheduler / heartbeat 任务正文、披露表单名、行业词或宏观指标被实体 guard 当作证券实体的同根问题，不新建重复缺陷。
  - 影响是目标报告没有生成，用户收到误导性失败提示；同窗仍有其它 scheduler / direct 正常收口，维持功能性 `P2 / New`，非 P1。
