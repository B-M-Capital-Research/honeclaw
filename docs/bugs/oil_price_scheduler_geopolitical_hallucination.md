...
系统在外部搜索出现额度失败告警的同一轮，仍将高风险地缘叙述当作事实发送给用户，并把它们作为油价变化的主因。09:01 和 12:00 复核显示，即使输出不再复述 06:02 的完整战争/停火叙事，也仍继续把单一政治讲话、布伦特冲高、OPEC+ 与地缘风险包装成确定性驱动并送达。该任务并非“美伊战争专题”，而是原油价格播报；当前输出把未经足够验证的宏观事件链混入投资相关定时提醒。
