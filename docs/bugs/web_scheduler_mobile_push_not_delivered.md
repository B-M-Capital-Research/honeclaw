...
- `2026-08-17 10:02 CST` 运行态继续复发，状态维持 `New/P2`：
  ...
  - `data/sessions.sqlite3`
    - `web_push_messages.max(created_at)=2026-07-19T13:30:44.965959+08:00`，06:02 CST 后 `web_push_messages` 增量仍无法从本地镜像确认推进。
  - 判断：本轮样本规模较小，但再次证明 Web push channel closed 后会退到 dryrun sink，且本地 Web push 台账不推进；维持功能性 `P2 / New`，非 P1。
...
- `2026-08-10 10:02 CST` 运行态继续复发，状态维持 `New/P2`：
  - `data/logs/hone-console-page-source.log`
    ...
    - 08:31 CST event-engine digest 出站记录 13 条 `web push broadcast failed: channel closed channel=web user=...`，随后进入 25 条 `[dryrun sink]` fallback；代表样本覆盖多个 Web actor 的盘前摘要。
  - `data/sessions.sqlite3`
    - `web_push_messages.max(created_at)=2026-07-19T13:30:44.965959+08:00`，06:00 CST 后 `web_push_messages` 增量为 0。
...
- `2026-08-09 22:03 CST` 运行态继续复发，状态维持 `New/P2`：
  - `data/logs/hone-console-page-source.log`
    ...
    - 20:30 CST event-engine digest 出站记录 2 条 `web push broadcast failed: channel closed channel=web user=...`，随后进入 3 条 `[dryrun sink]` fallback；代表样本覆盖 `web-user-879a3b18fce2` 与 `web-user-be13e1f84d14` 的盘前摘要。
  - `data/sessions.sqlite3`
