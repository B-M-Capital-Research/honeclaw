...
- `OWALERT_PreMarket` 与 `Oil_Price_Monitor_Premarket` 在最近一小时连续两次失败。
- 两次失败都发生在相同用户、相同手机号目标、相同 scheduler 送达链路。
...
- 由于同一目标在 `20:03` 仍有一条 `run_id=1976` 成功送达，说明并非该用户或该目标整体不可用，更像是某类盘前任务在发送 payload 形态上触发了 400。
