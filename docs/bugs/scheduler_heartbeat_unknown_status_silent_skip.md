...
- **证据来源**:
  ...
  - 最近一小时同一任务连续异常：
    - `run_id=1753`，`job_id=j_ab7e8fb1`，`job_name=Monitor_Watchlist_11`，`executed_at=2026-04-15T14:00:26.726229+08:00`，`execution_status=noop`，`message_send_status=skipped_noop`，`delivered=0`，`detail_json.parse_kind=JsonUnknownStatus`
    - `run_id=1750`，`executed_at=2026-04-15T13:30:25.883729+08:00`，`execution_status=noop`，`message_send_status=skipped_noop`，`delivered=0`，`detail_json.parse_kind=JsonUnknownStatus`
    - `run_id=1747`，`executed_at=2026-04-15T13:00:25.468082+08:00`，`execution_status=noop`，`message_send_status=skipped_noop`，`delivered=0`，`detail_json.parse_kind=JsonUnknownStatus`
  - 对比同一小时其他 heartbeat 任务：
    - `j_38745baf`、`j_654aef9b` 同时段均为 `JsonNoop`
  - 24 小时聚合：
    - `j_ab7e8fb1` 共运行 55 次，其中 25 次为 `JsonUnknownStatus`
  - 生命周期聚合：
    - `j_ab7e8fb1` 自 `2026-04-04T21:30:31.191391+08:00` 起累计运行 452 次，仅 3 次 `completed` / `delivered`
...
- 最近一小时内，`Monitor_Watchlist_11` 连续 3 次运行都被记为 `JsonUnknownStatus`，但最终状态仍是 `noop / skipped_noop`。
- 同一时间窗内其他 heartbeat 任务能正常落到 `JsonNoop`，说明问题不是“heartbeat 全局都无结果”，而是该链路的输出契约或解析兼容性异常。
