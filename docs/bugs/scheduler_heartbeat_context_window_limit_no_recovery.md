...
- 这表明当前问题不是“单个任务配置写坏后永久失败”，而是 heartbeat 任务集合中存在不稳定的上下文预算失控，故障会在相似任务之间漂移。
...
- 结合 `15:00` 的新样本看，问题也不完全等同于“新建任务首轮继承了过长历史”；因为同根因已经从 `TEM/AAOI` 漂移到 `RKLB`，更像是 heartbeat prompt 预算在不同任务之间缺少稳定上限控制。
