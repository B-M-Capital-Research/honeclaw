...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-27 04:00 最新窗口仍未恢复：最近一小时唯一新增直聊已正常完成，但 04:00 heartbeat 仍在 `JsonEmptyStatus`、`PlainTextSuppressed` 与 `JsonNoop` 间漂移，其中 `TEM/RKLB/Watchlist/持仓重大事件/TEM破位` 已显式落成 `execution_failed + skipped_error` | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
