...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-18 19:01、19:31、20:01 `Monitor_Watchlist_11` 已连续三轮 `execution_failed + skipped_error`；其余 heartbeat 同窗多为 `noop`，说明故障当前收敛成单条 watchlist 模板持续掉线 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
