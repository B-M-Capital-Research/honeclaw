...
| Feishu 直聊任务治理 / 定时汇总请求在搜索阶段耗尽迭代后整轮无回复 | P1 | Fixing | 2026-04-17 21:01 的 `OWALERT_PreMarket` 又在 7 次 `data_fetch` 后耗尽 8 轮；`cron_job_runs` 这次改记 `execution_failed + send_failed`，但真实会话仍无 assistant 回复 | [feishu_direct_cron_job_iteration_exhaustion_no_reply.md](./feishu_direct_cron_job_iteration_exhaustion_no_reply.md) |
...
| 深度分析链路持续访问不存在的 `company_profiles` 相对路径，长期画像记忆被静默跳过 | P3 | New | 2026-04-17 23:54 `UNH` 新话题会话仍先命中 `company_profiles` 不存在，并与 `context_overflow_recovery` 叠加放大长链路降级 | [company_profiles_relative_path_misses_actor_sandbox.md](./company_profiles_relative_path_misses_actor_sandbox.md) |
...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-18 03:01 最新窗口继续呈现相邻轮次抖动：`小米破位预警` 已从 `02:01` 的 `JsonUnknownStatus + execution_failed` 自恢复为 `03:01` `JsonNoop`，但 `raw_preview` 仍保留 `<think>` 前缀，说明协议仍未稳定 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
