...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-27 05:30 与 06:00 最新两轮仍未恢复：11 个 heartbeat 中分别有 7 个与 5 个落成 `execution_failed + skipped_error`，其余继续在 `JsonEmptyStatus` / `JsonNoop` 伪装成 `noop + skipped_noop` | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
