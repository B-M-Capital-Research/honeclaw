...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-26 22:00 最新窗口仍未恢复：`持仓重大事件心跳检测` 再次落成 `JsonMalformed + parse failure escalated`，`ORCL` 继续 `JsonEmptyStatus`，`TEM/ASTS/Monitor_Watchlist_11` 与多条破位预警继续输出 `<think>` + 纯文本后被吸收成 noop | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
...
| Telegram update listener 持续不可用，近一个月没有新消息入库 | P2 | New | 2026-04-26 21:03 bundled runtime 再次尝试拉起 Telegram，但旧进程仍占启动锁，`desktop.log` 继续记 `managed channel telegram skipped because it exited during startup`；最近 Telegram 会话仍停留在 2026-03-18 | [telegram_update_listener_connection_refused.md](./telegram_update_listener_connection_refused.md) |
