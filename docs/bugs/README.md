...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-20 00:31-01:01 `Monitor_Watchlist_11` 已连续两个窗口落成 `execution_failed + skipped_error`；同批 `noop` 任务仍依赖 `<think>...JSON` 尾部提取 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
| ASTS 发射链路把预告与停牌前行情误报成已发射后的实时结果 | P2 | New | 2026-04-19 21:30-22:00 最新窗口里 ASTS heartbeat 仍在“已发射升空”与“今日按计划发射”间摇摆，并继续捆绑停牌前 `ASTS $85.53 / -5.95%` | [asts_launch_schedule_misread_as_completed_event.md](./asts_launch_schedule_misread_as_completed_event.md) |
