...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | Fixing | 2026-04-24 17:31-18:01 最新两批 live heartbeat 仍统一 `starts_with_json=false`；`Monitor_Watchlist_11` 继续落成 `JsonEmptyStatus`，`持仓重大事件心跳检测` 虽触发送达但正文仍来自 `<think>...JSON` 尾部抽取 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
