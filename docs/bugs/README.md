...
| 会话压缩摘要仍以 `role=user` 的 `Compact Summary` 回灌真实 transcript，修复结论已回归失效 | P1 | Fixing | 2026-04-21 16:05 `Actor_feishu__direct__ou_5f1fdf...` 再次在 compact 后写入 `role=user` 的股票关注表 summary，随后继续回答“美股亚川”新问题；生产链路仍未收口 | [session_compact_summary_report_hallucination.md](./session_compact_summary_report_hallucination.md) |
...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-21 17:00 `小米破位预警` 与 `Monitor_Watchlist_11` 又因 `JsonUnknownStatus` 落成 `execution_failed + skipped_error`，问题继续在破位和 watchlist 模板间漂移 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
