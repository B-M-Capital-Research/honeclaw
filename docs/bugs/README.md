...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-19 04:00 最新窗口又漂移到 `Monitor_Watchlist_11` 失败；同窗 `全天原油价格3小时播报` 虽回到 `noop`，但仍输出长段 `<think>` 推理后才侥幸收口 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
