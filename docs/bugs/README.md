...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-27 16:00 最新窗口仍未恢复：15:30/16:00 两窗 22 条 heartbeat 样本全部 `delivered=0`，其中 14 条落成 `execution_failed + skipped_error`、8 条继续漂成伪 `noop`；`ASTS` 甚至退化成纯 `<think> + <minimax:tool_call>` 空壳输出 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
