...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | Fixing | 2026-04-26 05:00 最新 10 条 heartbeat 再次全部 `noop + skipped_noop`；`RKLB异动监控`、`TEM大事件心跳监控` 仍是 `JsonEmptyStatus`，`持仓重大事件心跳检测` 等任务则把完整自然语言结论吞进 `PlainTextSuppressed` | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
