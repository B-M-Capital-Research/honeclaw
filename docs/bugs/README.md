...
| Heartbeat 定时任务结构化状态退化在静默跳过与误发失败提示之间漂移 | P2 | Fixing | 2026-04-28 00:00 跨日窗口仍无一条恢复成合法单段 JSON；`小米/CAI/TEM/ORCL/RKLB/持仓监控/原油` 等样本继续把 `<think>`、自然语言 `noop` 或 fenced JSON 代码块当最终输出 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
