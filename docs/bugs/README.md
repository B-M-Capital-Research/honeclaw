...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-27 15:00 最新窗口仍未恢复：`全天原油价格3小时播报`、`ASTS 重大异动心跳监控`、`持仓重大事件心跳检测` 等继续把 `<think>` 后的自然语言总结落成 `execution_failed + skipped_error`，其余任务仍混出 `noop + skipped_noop`；14:30/15:00 两窗 22 条样本全部 `delivered=0` | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
