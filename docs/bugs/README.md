...
| 会话压缩摘要再次以 `Compact Summary` 回灌为 `role=user`，真实 transcript 与修复结论重新冲突 | P1 | Fixing | 2026-04-19 09:00 定时任务 `特斯拉与火箭实验室新闻日报` 在 auto compact 后再次把 `TSLA/RKLB` 摘要写成 `role=user`，随后仍成功送达最终日报；说明污染仍活跃于 scheduler 生产链路 | [session_compact_summary_report_hallucination.md](./session_compact_summary_report_hallucination.md) |
...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-19 10:00 最新窗口再次漂移到 `RKLB异动监控`、`ORCL 大事件监控` 与 `Monitor_Watchlist_11` 的 `execution_failed + skipped_error`；同窗其余 heartbeat 仍依赖 `<think>` 污染输出尾部侥幸提取 `noop` | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
| Heartbeat 已触发事件在无新增增量时跨窗口重复提醒，同一催化会在半小时轮询里反复送达 | P3 | New | 2026-04-19 08:30-10:00 `ASTS 重大异动心跳监控` 围绕同一 `BlueBird 7` 发射事件连续 4 个窗口 `completed + sent`，未见新增事实却反复推送 | [scheduler_heartbeat_retrigger_duplicate_alerts.md](./scheduler_heartbeat_retrigger_duplicate_alerts.md) |
