...
| Heartbeat 定时任务遇到 `JsonUnknownStatus` 时静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-17 00:31 最新窗口显示问题已收缩：`小米30港元破位预警`、`存储板块加仓信号监控` 恢复为 `JsonNoop`，但 `Monitor_Watchlist_11` 仍连续落成 `execution_failed + skipped_error` | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
...
| 会话压缩摘要仍以 `Compact Summary` 回灌为 `role=user`，导致 scheduler 任务串入上一轮待办与结论 | P1 | New | 2026-04-17 00:03 与 00:36 再次复现；摘要仍在 scheduler 串行任务与普通直聊澄清中以用户消息注回上下文 | [session_compact_summary_report_hallucination.md](./session_compact_summary_report_hallucination.md) |
