...
| Feishu 直聊已成功 `persist_assistant + reply.send`，但 `sessions.sqlite3` 会话镜像整体仍停留在前一日下午 | P2 | New | 2026-04-28 01:08 `sessions` / `session_messages` 最新时间仍卡在 `2026-04-27 16:54:20+08:00`；同窗 `00:36`、`00:39`、`00:45` 的 Feishu 直聊都已在 `sidecar.log` 记录 `session.persist_assistant` 与 `reply.send` 成功 | [sessions_sqlite_mirror_stalled_after_successful_direct_replies.md](./sessions_sqlite_mirror_stalled_after_successful_direct_replies.md) |
| Heartbeat 定时任务结构化状态退化在静默跳过与误发失败提示之间漂移 | P2 | Fixing | 2026-04-28 01:00 最新窗口仍无一条恢复成合法单段 JSON；`小米/CAI/TEM/ORCL/RKLB/ASTS/原油` 等样本继续把 `<think>`、自然语言 `noop` 或 fenced JSON 代码块当最终输出 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
