...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-19 05:00 最新窗口又漂移到 `Monitor_Watchlist_11` 与 `TEM大事件心跳监控`；同窗 `RKLB异动监控` 已恢复送达，说明故障仍在不同 heartbeat 模板间活跃抖动 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
