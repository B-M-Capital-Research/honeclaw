...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | Fixing | 2026-04-24 已补 `strip_internal_reasoning_blocks`（heartbeat parser 扫 JSON 前剥 `<think>`）+ prompt 规则 6a（整条回复必须单段 JSON，首字符 `{`）；LLM 冒烟走**生产辅助通道 MiniMax**（非 mock 也非 OpenRouter 旁路），3 case raw 全 `<think>` 开头但 parse_kind 全落合法契约，contract_ok=true。注：14:30-15:01 heartbeat 整批 skip 的样本发生在策略生效前 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
