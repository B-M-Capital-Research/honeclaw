...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-18 07:01 最新窗口仍未收口：`Monitor_Watchlist_11` 从 `06:31` 失败短暂恢复为 `noop`，但同根因已漂移到 `小米破位预警`，同轮再次落成 `execution_failed + skipped_error` | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
