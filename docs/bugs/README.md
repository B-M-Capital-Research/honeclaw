...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-18 04:01 最新窗口继续呈现跨任务漂移抖动：`Monitor_Watchlist_11` 在 `03:31` 再次回落为 `JsonUnknownStatus + execution_failed`，`小米30港元破位预警` 又在 `04:01` 接力失败 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
