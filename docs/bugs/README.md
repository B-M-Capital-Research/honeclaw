...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-27 12:00 最新窗口仍未恢复：`CAI破位预警` 明确吐出带 `<think>` 的未知状态文本并落成 `execution_failed + skipped_error`，`持仓重大事件心跳检测`/`TEM大事件心跳监控` 继续伪装成 `noop + skipped_noop`，当轮 heartbeat 样本仍全部 `delivered=0` | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
