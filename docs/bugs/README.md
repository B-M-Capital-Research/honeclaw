...
| 会话压缩摘要曾以 `role=user` 的 `Compact Summary` 回灌真实 transcript，且压缩标记会进入最终可见文本 | P1 | Fixing | 2026-04-22 05:00 `科技成长赛道大盘极值与情绪监控` runner 输入仍带入 2026-04-20 的 `role=user` `Compact Summary`；存量污染继续进入 prompt，新生成 summary 角色已有收敛迹象但未完成迁移/隔离闭环 | [session_compact_summary_report_hallucination.md](./session_compact_summary_report_hallucination.md) |
| 原油定时播报把未核验地缘叙述当作油价事实送达用户 | P2 | New | 2026-04-22 06:02 `全天原油价格3小时播报` 已 `completed + sent`，正文把“美伊战争于2月28日爆发 / 霍尔木兹重新开放 / 停火到期”等高风险叙述当作油价原因发送；同轮 Tavily key 多次配额失败，市场播报缺少来源质量门禁 | [oil_price_scheduler_geopolitical_hallucination.md](./oil_price_scheduler_geopolitical_hallucination.md) |
...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-22 07:31-08:01 `ASTS 重大异动心跳监控` 的 `run_id=4355/4362` 再次 `JsonEmptyStatus + skipped_noop`，且 raw preview 已完成判断但仍以 `<think>` 自由文本开头；结构化契约未恢复 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
