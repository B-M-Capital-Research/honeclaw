...
| Feishu 每日动态监控遇到 `codex acp stream closed before response` 后台账仍记为已发送 | P2 | New | 2026-04-20 00:02 `RKLB 每日动态监控` 在 `agent.run` 启动 1 秒后断流失败；真实会话无 assistant，但 `cron_job_runs` 记成 `execution_failed + sent + delivered=1` | [feishu_scheduler_codex_acp_stream_closed_false_sent.md](./feishu_scheduler_codex_acp_stream_closed_false_sent.md) |
...
| Feishu 直达定时任务已生成最终播报，但发送阶段持续返回 `HTTP 400 Bad Request` 导致用户收不到提醒 | P1 | Fixing | 2026-04-20 04:32 `OWALERT_PostMarket` 紧接 `04:02 Oil_Price_Monitor_Closing` 后再次落成 `completed + send_failed`；两轮都拿到 `code=99992361 / open_id cross app`，说明故障仍停留在 Feishu 目标/应用绑定链路 | [feishu_scheduler_send_failed_http_400_after_generation.md](./feishu_scheduler_send_failed_http_400_after_generation.md) |
| 会话压缩摘要再次以 `Compact Summary` 回灌为 `role=user`，真实 transcript 与修复结论重新冲突 | P1 | Fixing | 2026-04-19 23:37-23:47 最近一小时两条不同 Feishu 直聊都再次在 auto compact 后写回 `role=user` 的结构化 `Compact Summary`；问题仍是当前线上活跃缺陷，不是旧脏数据残留 | [session_compact_summary_report_hallucination.md](./session_compact_summary_report_hallucination.md) |
...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-20 04:31-05:01 最新窗口里 `RKLB异动监控`、`Monitor_Watchlist_11` 连续两轮仍落成 `execution_failed + skipped_error`；同批其余 heartbeat 继续依赖 `<think>...JSON` 尾部提取，而 `ASTS/TEM` 还能同时误触发送达 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
| ASTS 发射链路把预告与停牌前行情误报成已发射后的实时结果 | P2 | New | 2026-04-20 04:31-05:01 最新窗口里 ASTS heartbeat 连续两轮再次围绕同一 `BlueBird 7` 低轨事件送达，仍继续捆绑停牌前 `ASTS $85.53 / -5.95%` 作为事件背景 | [asts_launch_schedule_misread_as_completed_event.md](./asts_launch_schedule_misread_as_completed_event.md) |
| Heartbeat 已触发事件在无新增增量时跨窗口重复提醒，同一催化会在半小时轮询里反复送达 | P3 | New | 2026-04-20 04:31-05:01 `ASTS 重大异动心跳监控` 连续两轮再次围绕同一 `BlueBird 7` 低轨事件送达；同窗 `TEM大事件心跳监控` 也从 `noop` 回摆成围绕同一 `AACR 2026` 事件的 `triggered + sent` | [scheduler_heartbeat_retrigger_duplicate_alerts.md](./scheduler_heartbeat_retrigger_duplicate_alerts.md) |
