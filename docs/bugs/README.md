...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-27 11:00 最新窗口仍未恢复：`ORCL 大事件监控` 已回落 `execution_failed + skipped_error`，`持仓重大事件心跳检测` 退化成 `ContextOverflowNoop`，`ASTS 重大异动心跳监控` 继续伪装成 `noop + skipped_noop`，当轮样本仍全部 `delivered=0` | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
