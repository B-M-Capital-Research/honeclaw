...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-22 03:00 最新 CAI/TEM/小米/Watchlist/RKLB/ASTS heartbeat 仍以 `<think>` 自由文本开头再尾部输出 JSON，`starts_with_json=false`；多数被解析成 `noop`，但契约仍依赖脆弱尾部提取 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
