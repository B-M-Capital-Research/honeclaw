...
| 深度分析链路持续访问不存在的 `company_profiles` 相对路径，长期画像记忆被静默跳过 | P3 | New | 2026-04-17 21:01 `OWALERT_PreMarket` 与 `FORM` 深度分析两条新链路再次命中 `company_profiles` 不存在；主链路仍会继续运行，但长期画像继续被静默跳过 | [company_profiles_relative_path_misses_actor_sandbox.md](./company_profiles_relative_path_misses_actor_sandbox.md) |
...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-17 20:31 `Monitor_Watchlist_11` 再次命中 `JsonUnknownStatus + parse failure escalated`，21:01 又恢复 `JsonNoop`；说明 heartbeat 仍在相邻轮次间抖动 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
