...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-27 00:30/01:00 两个最新窗口仍未恢复：同批 heartbeat 继续在 `JsonEmptyStatus`、`JsonNoop`、`PlainTextSuppressed` 之间漂移，并在 `skipped_noop` 与 `skipped_error` 间摆动 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
