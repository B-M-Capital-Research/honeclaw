...
| 会话压缩摘要再次以 `Compact Summary` 回灌为 `role=user`，真实 transcript 与修复结论重新冲突 | P1 | Fixing | 2026-04-19 12:44 新的 `900943` 直聊提问前又写回 `TEM/RKLB` 持仓表摘要；正式回答随后直接带入“你的美股科技主线”与旧持仓建议，说明污染已继续改写新问题答案 | [session_compact_summary_report_hallucination.md](./session_compact_summary_report_hallucination.md) |
...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-19 15:30-16:00 最新窗口仍未收口：`RKLB` 在 `15:30`、`16:00` 连续失败，`Monitor_Watchlist_11` 刚在 `15:30` 暂时恢复，`16:00` 又回落失败，`CAI` 也新漂移进失败队列 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
| ASTS 发射链路把预告与停牌前行情误报成已发射后的实时结果 | P2 | New | 2026-04-19 14:23 直聊先把停牌前收盘价说成发射后下跌；15:00 heartbeat 在北京时间晚间发射窗口前报成“已成功发射”后，15:30 又继续按同一错误条件重复送达 | [asts_launch_schedule_misread_as_completed_event.md](./asts_launch_schedule_misread_as_completed_event.md) |
