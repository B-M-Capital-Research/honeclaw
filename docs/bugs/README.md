...
| Feishu 定时任务在 Codex ACP 未完成搜索工具时集中失败，只发通用抱歉且不回写会话 | P1 | Fixed | 2026-04-27 已在 `execute_scheduler_event` 失败分支追加 `session_storage.add_message` 将失败提示回写会话，同时新增 `failure_kind` 元数据区分 `unfinished_tool` 与通用失败 | [feishu_scheduler_codex_acp_unfinished_tool_generic_failure_unpersisted.md](./feishu_scheduler_codex_acp_unfinished_tool_generic_failure_unpersisted.md) |
