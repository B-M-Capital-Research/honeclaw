...
| 会话压缩摘要曾以 `role=user` 的 `Compact Summary` 回灌真实 transcript，且压缩标记会进入最终可见文本 | P1 | Fixing | 2026-04-22 08:33 `美股AI产业链盘后报告` 已 `completed + sent`，最终正文直接以 `Context compacted` 开头；压缩标记仍可穿透到用户可见消息，新生成 summary 角色已有收敛迹象但未完成输出净化/存量迁移闭环 | [session_compact_summary_report_hallucination.md](./session_compact_summary_report_hallucination.md) |
...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-22 08:31 `ORCL 大事件监控` 与 `Monitor_Watchlist_11` 再次 `JsonEmptyStatus + skipped_noop`，`ASTS 重大异动心跳监控` 仍为 `starts_with_json=false + JsonNoop`；raw preview 已完成判断但仍以 `<think>` 自由文本开头，结构化契约未恢复 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
