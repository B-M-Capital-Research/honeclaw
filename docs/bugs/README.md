...
| Feishu 直达定时任务已生成最终播报，但发送阶段持续返回 `HTTP 400 Bad Request` 导致用户收不到提醒 | P1 | Fixing | 2026-04-20 04:02 `Oil_Price_Monitor_Closing` 再次落成 `completed + send_failed`，这次日志已明确返回 `code=99992361 / open_id cross app`，说明故障仍停留在 Feishu 目标/应用绑定链路 | [feishu_scheduler_send_failed_http_400_after_generation.md](./feishu_scheduler_send_failed_http_400_after_generation.md) |
...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-20 03:31-04:01 最新窗口里 `RKLB异动监控` 连续两轮、`小米破位预警` 新一轮再次落成 `execution_failed + skipped_error`；同批 `TEM破位 / CAI / ORCL / 小米30港元 / Monitor_Watchlist_11` 即使记成 `noop`，仍依赖 `<think>...JSON` 尾部提取 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
| ASTS 发射链路把预告与停牌前行情误报成已发射后的实时结果 | P2 | New | 2026-04-20 01:31-02:02 最新窗口里 ASTS heartbeat 先发出“已成功发射”提醒，30 分钟后又回落成 `noop`，仍继续捆绑停牌前 `ASTS $85.53 / -5.95%` | [asts_launch_schedule_misread_as_completed_event.md](./asts_launch_schedule_misread_as_completed_event.md) |
| Heartbeat 已触发事件在无新增增量时跨窗口重复提醒，同一催化会在半小时轮询里反复送达 | P3 | New | 2026-04-20 03:31-04:01 `ASTS 重大异动心跳监控` 连续两轮再次围绕同一 `BlueBird 7` 低轨事件送达；两轮之间未见新的独立公告、轨道修正或价格阈值跨越 | [scheduler_heartbeat_retrigger_duplicate_alerts.md](./scheduler_heartbeat_retrigger_duplicate_alerts.md) |
