...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-19 02:01 最新窗口再次漂移到 `TEM大事件心跳监控`、`Monitor_Watchlist_11`、`ORCL 大事件监控` 失败；这些任务在 01:31 刚恢复为 `noop/sent`，30 分钟后又重新跌回 `JsonUnknownStatus` | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
