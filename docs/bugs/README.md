...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-19 19:31-20:01 最新窗口里失败对象从 `小米/ORCL/CAI` 漂移到 `RKLB`；同批 `noop` 任务仍继续依赖 `<think>...尾部 JSON` 侥幸提取状态 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
| ASTS 发射链路把预告与停牌前行情误报成已发射后的实时结果 | P2 | New | 2026-04-19 19:31-20:01 heartbeat 先短暂回落 `noop`，随后又把 BlueBird 7 改写成“已成功发射”，并继续把停牌前旧价格包装成事件后反应 | [asts_launch_schedule_misread_as_completed_event.md](./asts_launch_schedule_misread_as_completed_event.md) |
| Heartbeat 已触发事件在无新增增量时跨窗口重复提醒，同一催化会在半小时轮询里反复送达 | P3 | New | 2026-04-19 19:31-20:01 最新窗口里 TEM 与 ASTS 都呈现“上一轮 `noop`，下一轮又对同一旧事件重新 `triggered`”的重复提醒抖动 | [scheduler_heartbeat_retrigger_duplicate_alerts.md](./scheduler_heartbeat_retrigger_duplicate_alerts.md) |
