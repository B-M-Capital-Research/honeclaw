...
| Heartbeat 定时任务遇到 `JsonUnknownStatus` 时仍被静默记为 `noop`，监控提醒可能长期失效 | P2 | New | 修复结论需撤回；2026-04-16 10:30 `Monitor_Watchlist_11` 与 11:00 `存储板块加仓信号监控` 仍落成 `JsonUnknownStatus -> noop / skipped_noop` | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
