...
| 会话压缩摘要曾以 `role=user` 的 `Compact Summary` 回灌真实 transcript，且压缩标记会进入最终可见文本 | P1 | Fixing | 2026-04-23 21:37 `科技核心股池 · 晚间击球区快报` 再次 `completed + sent + delivered=1`，`run_id=5199` 的 response preview 与 assistant final 都仍以 `Context compacted` 开头；说明 21:02 后并未稳定收口 | [session_compact_summary_report_hallucination.md](./session_compact_summary_report_hallucination.md) |
...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-23 21:30-22:00 两批 heartbeat 继续主要落成 `noop + skipped_noop`；但 `sidecar.log` 仍连续记录 `starts_with_json=false`、`<think>` 前缀与 `JsonNoop/JsonTriggered`，说明解析兜底仍在吸收漂移，上游 JSON 契约未恢复 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
