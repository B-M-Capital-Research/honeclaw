...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-19 20:31-21:01 最新窗口里失败对象从 `Monitor_Watchlist_11` 漂移到 `小米破位预警/RKLB异动监控`；同批 `noop` 任务仍继续依赖 `<think>...尾部 JSON` 侥幸提取状态 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
| ASTS 发射链路把预告与停牌前行情误报成已发射后的实时结果 | P2 | New | 2026-04-19 20:31-21:02 heartbeat 连续两轮都把 BlueBird 7 写成“已成功发射”，并继续把停牌前旧价格包装成事件后反应 | [asts_launch_schedule_misread_as_completed_event.md](./asts_launch_schedule_misread_as_completed_event.md) |
| Heartbeat 已触发事件在无新增增量时跨窗口重复提醒，同一催化会在半小时轮询里反复送达 | P3 | New | 2026-04-19 20:31-21:02 最新窗口里 ASTS 又跨半小时重复送达同一 BlueBird 7 旧事件，TEM 虽暂回 `noop` 但整体去重抖动仍未收口 | [scheduler_heartbeat_retrigger_duplicate_alerts.md](./scheduler_heartbeat_retrigger_duplicate_alerts.md) |
