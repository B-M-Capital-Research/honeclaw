...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-18 21:01 最新窗口已同时击中 `Monitor_Watchlist_11`、`ASTS 重大异动心跳监控`、`小米30港元破位预警` 三条任务；影响从单条 watchlist 持续掉线重新扩散为同窗多任务漏提醒 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
