...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-22 14:01 同批 10 条 heartbeat 全部 `noop + skipped_noop`，但日志仍全部 `starts_with_json=false`；`RKLB异动监控` 仍落成 `JsonEmptyStatus`，继续依赖 `<think>...尾部 JSON` 提取，结构化契约未恢复 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
