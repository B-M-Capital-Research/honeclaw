...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-17 22:01 故障又从 `Monitor_Watchlist_11` 漂移到 `全天原油价格3小时播报` 与 `小米破位预警`；同轮 `Monitor_Watchlist_11` 反而恢复 `JsonNoop`，说明公共协议抖动仍在继续 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
