...
| Heartbeat 定时任务遇到 `JsonUnknownStatus` 时静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-16 22:01 与 22:31 `Monitor_Watchlist_11` 继续失败，23:01 又短暂回到 noop，说明未知状态仍在相邻轮次抖动 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
...
| 会话压缩摘要仍以 `Compact Summary` 回灌为 `role=user`，导致 scheduler 任务串入上一轮待办与结论 | P1 | New | 2026-04-16 20:31 与 20:46 两条 scheduler 会话复现；此前 `Fixed` 结论已撤回 | [session_compact_summary_report_hallucination.md](./session_compact_summary_report_hallucination.md) |
