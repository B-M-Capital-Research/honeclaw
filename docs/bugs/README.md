...
| Feishu 直达定时任务已生成最终播报，但发送阶段持续返回 `HTTP 400 Bad Request` 导致用户收不到提醒 | P1 | Fixing | 2026-04-20 20:00 `GEV earnings reminder` 再次落成 `completed + send_failed`，错误体仍是 `code=99992361 / open_id cross app`；同一目标已从盘前/盘后/早报继续扩散到财报提醒 | [feishu_scheduler_send_failed_http_400_after_generation.md](./feishu_scheduler_send_failed_http_400_after_generation.md) |
| Feishu 直聊在处理中遭遇 runtime 重启风暴，placeholder 发出后整轮无最终回复 | P1 | New | 2026-04-20 17:35 `分析一下SNDK` 已进入 `agent.run` 并执行多次工具；17:37 起 bundled runtime 连续重启，最终既无 assistant 落库，也无失败文案 | [feishu_direct_runtime_restart_interrupts_inflight_reply.md](./feishu_direct_runtime_restart_interrupts_inflight_reply.md) |
...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-20 20:00 `Monitor_Watchlist_11` 与 `TEM大事件心跳监控` 同轮再次跌回 `JsonUnknownStatus`；失败对象已不再只是上一小时的单模板漂移 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
