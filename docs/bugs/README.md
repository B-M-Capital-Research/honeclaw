...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-20 14:00 `TEM大事件心跳监控` 一度跌回 `execution_failed + skipped_error`，到 14:30 与 15:00 又回到 `noop`；但 `detail_json.raw_preview` 仍是 `<think>...{"status":"noop"}`，说明只是症状回摆，结构化契约未恢复 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
| ASTS 发射链路把预告与停牌前行情误报成已发射后的实时结果 | P2 | New | 2026-04-20 14:30 与 15:00 连续两轮仍 `completed + sent`，继续把同一 `BlueBird 7` 轨道异常配上停牌前 `ASTS $85.53 / -5.95%` 旧价格当作“当前行情”；14:00 那轮还一度扩写成“卫星已报废” | [asts_launch_schedule_misread_as_completed_event.md](./asts_launch_schedule_misread_as_completed_event.md) |
