...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | Fixing | 2026-04-24 19:01 最新整点 heartbeat 仍统一 `starts_with_json=false`；`ORCL 大事件监控`、`Monitor_Watchlist_11` 与 `持仓重大事件心跳检测` 再次全部落成 `JsonNoop`，没有任何样本恢复为纯 JSON 首包 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
