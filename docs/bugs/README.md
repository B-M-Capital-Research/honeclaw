...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-17 14:00 `Monitor_Watchlist_11` 再次从 13:30 的 `JsonNoop` 回落到 `JsonUnknownStatus + execution_failed`；同批 `原油播报` 与 `小米预警` 仍为 `JsonNoop`，协议抖动持续 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
