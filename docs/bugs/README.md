...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-22 18:31-19:01 heartbeat 仍全部 `noop + skipped_noop`，原油/小米等任务继续出现 `JsonEmptyStatus`，且全部 `starts_with_json=false`，继续依赖 `<think>...尾部 JSON` 提取 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
