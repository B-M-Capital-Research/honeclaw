...
| Bug | 严重等级 | 状态 | 修复情况 | 入口 |
| --- | --- | --- | --- | --- |
| Feishu 每日动态监控在“无新增催化应跳过”时仍照常推送长文 | P3 | New | 2026-04-20 00:01-00:02 `AAOI/TEM 每日动态监控` 两轮正文都明确写着“按规则应跳过正式推送”，但台账与会话都记成 `completed + sent` | [feishu_scheduler_daily_monitor_skip_rule_broken.md](./feishu_scheduler_daily_monitor_skip_rule_broken.md) |
...
| Feishu 直聊在工具尚未跑完时提前把过渡句当成最终答复发送，组合评估请求只收到半成品回复 | P3 | New | 2026-04-20 20:00 `GEV earnings reminder` 在 `web_search` 启动前就先把 72 字计划句落成 assistant；同根因已从直聊分析扩散到定时注入链路 | [feishu_direct_partial_reply_before_tool_completion.md](./feishu_direct_partial_reply_before_tool_completion.md) |
...
| 会话压缩摘要以 `Compact Summary` 回灌为 `role=user`，污染后续真实 transcript | P1 | Fixed | 2026-04-20 已将摘要写入 `role=system`，并在 `restore_context` 中跳过 compact_summary 消息（新旧数据双分支） | [session_compact_summary_report_hallucination.md](./session_compact_summary_report_hallucination.md) |
| 深度分析链路持续访问不存在的 `company_profiles` 相对路径，长期画像记忆被静默跳过 | P3 | Fixed | 2026-04-20 已在 `ensure_actor_sandbox` 中预创建 `company_profiles/` 目录，工具返回空列表而非 not-found | [company_profiles_relative_path_misses_actor_sandbox.md](./company_profiles_relative_path_misses_actor_sandbox.md) |
...
| Feishu 直达定时任务已生成最终播报，但发送阶段持续返回 `HTTP 400 / open_id cross app` | P1 | Fixed | 2026-04-20 已移除 `scheduler_receive_id_for_target` 短路逻辑，始终通过 Feishu API (`resolve_receive_id`) 获取当前 app 绑定的 open_id | [feishu_scheduler_send_failed_http_400_after_generation.md](./feishu_scheduler_send_failed_http_400_after_generation.md) |
...
| Heartbeat 定时任务命中 MiniMax HTTP 发送失败后缺少自动重试与降级，提醒整轮失败 | P2 | Fixed | 2026-04-20 同上，`openai_compatible.rs` provider 级重试已覆盖 heartbeat 链路 | [scheduler_heartbeat_minimax_http_transport_failure_no_retry.md](./scheduler_heartbeat_minimax_http_transport_failure_no_retry.md) |
...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | Fixed | 2026-04-20 空 status JSON `{}` 改为 JsonEmptyStatus（noop），不再升级为 execution_failed | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
| Heartbeat 重大事件监控触发 `已达最大迭代次数 6` 后整轮跳过，用户收不到应发提醒 | P2 | Fixed | 2026-04-20 max_iterations 错误改为 MaxIterationsNoop（skipped_noop），不再记为 execution_failed | [scheduler_heartbeat_iteration_exhaustion_skips_alert.md](./scheduler_heartbeat_iteration_exhaustion_skips_alert.md) |
