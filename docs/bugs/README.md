...
| Heartbeat 定时任务遇到 `JsonUnknownStatus` 时静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-16 16:31 到 17:01 继续抖动；`Monitor_Watchlist_11` 与 `AAOI_动态监控` 之间仍在 `execution_failed`/`noop` 之间漂移 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
| Heartbeat 监控任务触发 `context window exceeds limit` 后缺少恢复，故障会在不同任务间漂移复现 | P2 | New | 2026-04-16 16:31 再次命中 `AAOI_动态监控`；同批 `RKLB_动态监控` 到 17:01 已恢复，说明仍是跨任务漂移而非单点坏任务 | [scheduler_heartbeat_context_window_limit_no_recovery.md](./scheduler_heartbeat_context_window_limit_no_recovery.md) |
