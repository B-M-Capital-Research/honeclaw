...
| Feishu 直聊任务配置请求在搜索阶段反复调用 `cron_job` 后耗尽迭代并整轮无回复 | P1 | Fixing | 2026-04-17 已补 handler panic 监护、`session.run` 边界日志与 placeholder 更新失败回退；`max_iterations` 失败至少不应再静默，待真实任务治理样本复核 | [feishu_direct_cron_job_iteration_exhaustion_no_reply.md](./feishu_direct_cron_job_iteration_exhaustion_no_reply.md) |
...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-17 09:30 再次扩散到 `全天原油价格3小时播报` 与 `Monitor_Watchlist_11`；10:00 `Monitor_Watchlist_11` 虽恢复为 `noop`，但协议抖动仍持续 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
