...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-22 06:00-06:31 最新 heartbeat 仍统一 `starts_with_json=false`；`run_id=4332/4333/4334`（RKLB / Watchlist / ORCL）继续 `JsonEmptyStatus + skipped_noop`，同批原油触发型输出也依赖 `<think>...尾部 JSON` | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
