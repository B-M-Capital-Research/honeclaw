...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-19 03:01 最新窗口又漂移到 `小米30港元破位预警`、`RKLB异动监控` 失败；这些任务在 02:31 刚恢复为 `noop`，30 分钟后再次跌回 `JsonUnknownStatus`，同窗 `ASTS` 仍能成功送达 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
