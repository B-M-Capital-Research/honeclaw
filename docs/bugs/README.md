...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-24 06:30-07:01 heartbeat 仍普遍以 `starts_with_json=false`、`<think>` 前缀和 `JsonNoop/JsonEmptyStatus` 被兜底解析；`RKLB异动监控`、`ASTS 重大异动心跳监控` 与 07:01 的 `小米破位预警` 仍在漂移 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
...
| Heartbeat 已触发事件在无新增增量时跨窗口重复提醒，同一催化会在半小时轮询里反复送达 | P3 | New | 2026-04-24 06:31 `持仓重大事件心跳检测` 又把已被多轮消化的 ASTS `BlueBird 7/FCC` 旧事实包装成 `双触发` 送达；不同 heartbeat 之间仍未共享去重基线 | [scheduler_heartbeat_retrigger_duplicate_alerts.md](./scheduler_heartbeat_retrigger_duplicate_alerts.md) |
