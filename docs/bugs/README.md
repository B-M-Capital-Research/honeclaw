...
| Heartbeat 定时任务遇到 `JsonUnknownStatus` 时静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-16 23:31 与 2026-04-17 00:01 再次复现，`存储板块加仓信号监控`、`小米30港元破位预警`、`Monitor_Watchlist_11` 连续落成 `execution_failed + skipped_error` | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
...
| 会话压缩摘要仍以 `Compact Summary` 回灌为 `role=user`，导致 scheduler 任务串入上一轮待办与结论 | P1 | New | 2026-04-16 23:45 与 23:57 两条真实直聊再次复现；摘要仍以用户消息注入，并继续影响后续问答方向 | [session_compact_summary_report_hallucination.md](./session_compact_summary_report_hallucination.md) |
