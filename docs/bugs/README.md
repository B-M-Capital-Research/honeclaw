...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-18 00:01 `Monitor_Watchlist_11` 在 `23:01`、`23:31` 两轮恢复 `JsonNoop` 后又回落到 `JsonUnknownStatus + execution_failed`，说明协议抖动仍未收敛 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
