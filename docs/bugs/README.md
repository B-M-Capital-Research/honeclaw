...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-18 12:31-13:00 `RKLB异动监控` 连续两轮 `skipped_error`，`Monitor_Watchlist_11` 也从 `JsonNoop` 再次回落到 `JsonUnknownStatus` | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
