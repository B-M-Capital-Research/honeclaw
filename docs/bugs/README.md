...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-20 02:31-03:01 最新窗口里 `小米破位 / RKLB / Monitor_Watchlist_11 / TEM大事件` 从上一轮 `noop` 集体回摆成 `execution_failed + skipped_error`；同批 `noop` 任务仍依赖 `<think>...JSON` 尾部提取 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
...
| Heartbeat 已触发事件在无新增增量时跨窗口重复提醒，同一催化会在半小时轮询里反复送达 | P3 | New | 2026-04-20 02:31-03:01 `ASTS 重大异动心跳监控` 在上一轮 `noop` 后又把同一 `BlueBird 7` 旧事件重新送达；两轮之间无新增独立催化 | [scheduler_heartbeat_retrigger_duplicate_alerts.md](./scheduler_heartbeat_retrigger_duplicate_alerts.md) |
