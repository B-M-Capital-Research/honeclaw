...
| 会话压缩摘要曾以 `role=user` 的 `Compact Summary` 回灌真实 transcript，且压缩标记会进入最终可见文本 | P1 | Fixing | 2026-04-22 09:03 `早9点市场复盘(XME及加密ETF)` 已 `completed + sent`，最终正文仍直接以 `Context compacted` 开头；压缩标记连续穿透到用户可见消息，新生成 summary 角色已有收敛迹象但输出净化/存量迁移仍未闭环 | [session_compact_summary_report_hallucination.md](./session_compact_summary_report_hallucination.md) |
...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-22 12:00 `ORCL 大事件监控`、`Monitor_Watchlist_11` 仍落成 `JsonEmptyStatus + starts_with_json=false`；`TEM` / `ASTS` 虽能解析为 `JsonNoop`，但 raw preview 仍以 `<think>` 自由文本开头，结构化契约未恢复 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
