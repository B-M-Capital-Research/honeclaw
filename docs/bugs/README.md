...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-19 18:31-19:01 最新窗口里 `全天原油价格3小时播报` 从上一轮 `noop` 再次漂移成 `JsonUnknownStatus + execution_failed`；同批其它任务仍继续依赖尾部侥幸提取状态 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
| ASTS 发射链路把预告与停牌前行情误报成已发射后的实时结果 | P2 | New | 2026-04-19 18:31-19:01 heartbeat 仍在“计划于今日发射”与“发射已完成”之间来回改写 BlueBird 7，同一旧价格快照继续被包装成事件背景 | [asts_launch_schedule_misread_as_completed_event.md](./asts_launch_schedule_misread_as_completed_event.md) |
| Heartbeat 已触发事件在无新增增量时跨窗口重复提醒，同一催化会在半小时轮询里反复送达 | P3 | New | 2026-04-19 18:31-19:01 最新窗口确认 ASTS 旧事件仍连续两轮送达，只是从“今日发射”改写成“已完成”；上一窗口 TEM 的 AACR 进行中事件也仍连续重报 | [scheduler_heartbeat_retrigger_duplicate_alerts.md](./scheduler_heartbeat_retrigger_duplicate_alerts.md) |
