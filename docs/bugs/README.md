...
| Feishu / Discord 成功会话已完成 `persist_* + reply.send`，但 `sessions.sqlite3` 会话镜像整体仍停留在前一日下午 | P2 | New | 2026-04-28 10:03 再次复核 `sessions` / `session_messages` 仍卡在 `2026-04-27 16:54:20+08:00`；但同库 `cron_job_runs` 已写到 `2026-04-28 10:00:43+08:00`，且 `09:32-09:33` 又有 Discord 成功 turn 完成 `persist_assistant + success=true` | [sessions_sqlite_mirror_stalled_after_successful_direct_replies.md](./sessions_sqlite_mirror_stalled_after_successful_direct_replies.md) |
| Heartbeat 定时任务结构化状态退化在静默跳过与误发失败提示之间漂移 | P2 | Fixing | 2026-04-28 10:00 窗口仍无一条恢复成合法单段 JSON；最近两窗 22 条 heartbeat 全部异常，坏态已扩散到 `PlainTextSuppressed` / `JsonEmptyStatus` / `ContextOverflowNoop` 混合漂移 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
