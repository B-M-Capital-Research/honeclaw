...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-17 05:00-07:00 连续五轮未再复现 `JsonUnknownStatus`；但 `07:00` 的 `Monitor_Watchlist_11` 原始输出仍带 `<think>` 前缀，需继续观察是否真正稳定 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
