...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-18 01:01 最近两轮已扩散到 `TEM破位预警`、`小米30港元破位预警`、`全天原油价格3小时播报`、`Monitor_Watchlist_11`；自由文本已写明应 `noop` 仍批量回落 `JsonUnknownStatus + execution_failed` | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
