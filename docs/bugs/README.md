...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-20 01:31-02:02 最新窗口里失败对象再次漂移到 `全天原油价格3小时播报 / ORCL / TEM大事件 / Monitor_Watchlist_11`；同批 `noop` 任务仍依赖 `<think>...JSON` 尾部提取 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
...
| Heartbeat 已触发事件在无新增增量时跨窗口重复提醒，同一催化会在半小时轮询里反复送达 | P3 | New | 2026-04-20 00:31-01:01 `TEM大事件心跳监控` 又把同一 AACR 会议进行中事件从 `noop` 回摆成 `triggered + sent`，无新增独立催化 | [scheduler_heartbeat_retrigger_duplicate_alerts.md](./scheduler_heartbeat_retrigger_duplicate_alerts.md) |
