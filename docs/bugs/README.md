...
| Feishu 定时任务在 Codex ACP 未完成搜索工具时集中失败，只发通用抱歉且不回写会话 | P1 | New | 2026-04-27 20:30/21:00 两个窗口至少 6 条常规定时任务统一落成 `execution_failed + sent + delivered=1`；已登记 GitHub issue `#22`，`sidecar.log` 根因是 `codex acp prompt ended before tool completion`，但对应 direct session 仍停在本轮前旧消息 | [feishu_scheduler_codex_acp_unfinished_tool_generic_failure_unpersisted.md](./feishu_scheduler_codex_acp_unfinished_tool_generic_failure_unpersisted.md) |
...
| Heartbeat 定时任务结构化状态退化在静默跳过与误发失败提示之间漂移 | P2 | Fixing | 2026-04-27 21:00 又继续混出 `execution_failed + skipped_error` 与伪 `noop`；失败样本原始输出仍覆盖“系统正常”“条件未触发”“无法核验”“完整原油播报”等非结构化内容 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
