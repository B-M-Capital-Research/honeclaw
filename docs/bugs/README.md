...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | Fixing | 2026-04-26 06:00 最新 10 条 heartbeat 再次全部 `noop + skipped_noop`；`CAI破位预警`、`ASTS 重大异动心跳监控`、`持仓重大事件心跳检测` 回到 `JsonEmptyStatus`，`RKLB异动监控`、`ORCL 大事件监控` 仍会把整段自然语言分析吞进 `PlainTextSuppressed` | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
