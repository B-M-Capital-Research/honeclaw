...
| 原油定时播报把未核验地缘叙述当作油价事实送达用户 | P2 | New | 2026-04-23 09:01 `全天原油价格3小时播报` 再次 `completed + sent + delivered=1`，把伊朗停火协议扩展不确定性和中东供应扰动写成涨价原因；市场播报仍缺来源质量与置信度门禁 | [oil_price_scheduler_geopolitical_hallucination.md](./oil_price_scheduler_geopolitical_hallucination.md) |
...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-23 09:01-10:01 多个 heartbeat 仍 `starts_with_json=false`；`全天原油价格3小时播报` 与 `ASTS 重大异动心跳监控` 触发型输出也先吐 `<think>` 再由解析器提取 JSON，结构化契约未恢复 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
...
| Heartbeat 已触发事件在无新增增量时跨窗口重复提醒，同一催化会在半小时轮询里反复送达 | P3 | New | 2026-04-23 10:01 `ASTS 重大异动心跳监控` 再次把已在 08:01 送达过的 ASTS/FCC/BlueBird 旧事件投递为 `completed + sent + delivered=1`；正文自认未触发 8% 价格阈值，仍以旧基本面事件触发 | [scheduler_heartbeat_retrigger_duplicate_alerts.md](./scheduler_heartbeat_retrigger_duplicate_alerts.md) |
