...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-23 07:31/08:01 `Monitor_Watchlist_11` 再次落成 `JsonEmptyStatus + skipped_noop`；模型已逐项比较触发价但仍输出 `<think>` 自由文本，依赖解析器兜底为 noop，结构化契约未恢复 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
