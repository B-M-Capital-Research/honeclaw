...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-27 09:30 与 10:00 最新两轮仍未恢复：22 条已结束 heartbeat 全部 `delivered=0`，其中 13 条落成 `execution_failed + skipped_error`，其余继续在 `JsonEmptyStatus` / `JsonUnknownStatus` 伪装成 `noop + skipped_noop` | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
