...
| Feishu 直聊已成功 `persist_assistant + reply.send`，但 `sessions.sqlite3` 会话镜像整体仍停留在前一日下午 | P2 | New | 2026-04-28 02:01 再次复核 `sessions` / `session_messages` 仍卡在 `2026-04-27 16:54:20+08:00`；但同库 `cron_job_runs` 已继续写到 `02:00` 窗口，说明停滞仍只集中在会话镜像链路 | [sessions_sqlite_mirror_stalled_after_successful_direct_replies.md](./sessions_sqlite_mirror_stalled_after_successful_direct_replies.md) |
| Heartbeat 定时任务结构化状态退化在静默跳过与误发失败提示之间漂移 | P2 | Fixing | 2026-04-28 02:00 最新两轮窗口仍无一条恢复成合法单段 JSON；`持仓重大事件心跳检测` 甚至退化成 “alive / no user query” 探针口径，`原油/ORCL/ASTS` 等样本继续输出 `<think>`、自然语言 `noop` 或 fenced JSON | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
