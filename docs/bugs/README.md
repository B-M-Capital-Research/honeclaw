...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-20 08:31 `ORCL 大事件监控` 又落成 `execution_failed + skipped_error`；09:01 虽局部回摆成 `noop`，但日志仍显示成功任务依赖 `<think>...JSON` 尾部提取，协议未真正恢复 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
