...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-24 10:30-11:00 heartbeat 仍普遍以 `starts_with_json=false`、`<think>` 前缀和 `JsonNoop/JsonEmptyStatus` 被兜底解析；`10:30` 与 `11:00` 两批除个别触发型任务外继续整批 `noop + skipped_noop`，仍未恢复纯 JSON 首包 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
