...
| Heartbeat 定时任务结构化状态退化后被静默跳过，监控提醒可能长期失效 | P2 | New | 2026-04-18 05:01 最新窗口仍在跨任务漂移：`CAI破位预警` 于 `04:31` 回落后，`TEM破位预警` 与 `小米破位预警` 又在 `05:01` 接力失败，仍无稳定收口 | [scheduler_heartbeat_unknown_status_silent_skip.md](./scheduler_heartbeat_unknown_status_silent_skip.md) |
