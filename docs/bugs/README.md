...
| 会话压缩摘要曾以 `role=user` 的 `Compact Summary` 回灌真实 transcript，且压缩标记会进入最终可见文本 | P1 | Fixing | 2026-04-23 21:02 `OWALERT_PreMarket` 最新播报仍以 `Context compacted` 开头后再发正文；说明 20:01/20:02 的局部止血未覆盖 21:00 新样本，compact 外泄仍活跃 | [session_compact_summary_report_hallucination.md](./session_compact_summary_report_hallucination.md) |
