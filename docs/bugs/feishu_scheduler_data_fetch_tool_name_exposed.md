...
- 2026-08-15 18:02-22:01 CST 运行态继续复发，状态维持 `New`：
  - `data/sessions.sqlite3` 在本轮窗口仍未追入真实运行，用户可见证据改从 `data/logs/hone-console-page-source.log` 的 heartbeat deliver preview 复核。
  - source log 同窗有多条用户可见候选继续外露内部行情工具 / 字段口径：`data_fetch` 命中 321 条、`hone_quote_time` 命中 11 条、`工具调用` 命中 14 条；代表样本包括 20:00-22:00 CST 多条 heartbeat 写出 `hone_quote_time.beijing`、`data_fetch quote`、`quote 工具调用已达上限`、`本轮工具调用已达上限` 或“行情未完成核验”。
...
- 2026-08-15 14:02-18:03 CST 运行态继续复发，状态维持 `New`：
  - `data/sessions.sqlite3` 在本轮窗口仍未追入真实运行，用户可见证据改从 `data/logs/hone-console-page-source.log` 的 heartbeat deliver preview 复核。
  - source log 同窗有多条用户可见候选继续外露内部行情工具 / 字段口径：`hone_quote_time` 命中 11 条、`data_fetch` 命中 259 条；代表样本包括 15:00 / 15:30 / 16:30 / 18:00 CST 多条 heartbeat 写出 `data_fetch 工具额度已耗尽`、`本轮 data_fetch quote`、`hone_quote_time.beijing`、`工具调用已达上限` 或“本轮行情未完成核验”。
  - 这些 heartbeat 消息业务主体已执行并进入送达候选，没有投递失败、空回复、错投、原始工具 JSON 或数据安全证据；问题仍只影响用户可见来源 / 工具口径边界和产品感，不影响主功能链路，因此维持质量性 `P3 / New`，非 P1，不创建 GitHub Issue。
...
- 2026-07-27 15:03-19:04 CST 运行态继续复发，状态维持 `New`：
  ...
  - 16:30 CST `TSLA 正负触发条件心跳监控` preview 写出 `hon_quote_time: 2026-07-25 04:00:01 +08:00`；19:00 CST 同任务 `run_id=48928` 又写出 `hone_quote_time: 2026-07-25 04:00:01 +08:00`。
  - 这些 heartbeat 消息业务主体已送达或进入出站候选，没有投递失败、空回复、错投、原始工具 JSON 或数据安全证据；问题仍只影响用户可见来源 / 工具口径边界和产品感，不影响主功能链路，因此维持质量性 `P3 / New`，非 P1，不创建 GitHub Issue。
...
- 2026-07-26 07:05 CST 运行态继续复发，状态维持 `New`：
  ...
  - 06:02 CST Feishu scheduler / direct actor session `Actor_feishu__direct__ou_5f11da38ad70c47cf87c0b106b6408b190` 的 `每日美股盘后收盘复盘` 正常收口，但 final 开头写出 `Now I have all necessary data. Let me compile the final analysis.`，随后把 `quote/gainers_losers`、`hone_quote_time.beijing`、`quote_short` 等内部工具或字段名作为用户态行情口径。
  - 该消息业务主体已完成指数、板块、VIX、美债收益率、美元指数和科技股复盘输出并送达，没有投递失败、空回复、错投、原始工具 JSON 或数据安全证据；问题仍只影响用户可见执行过程 / 工具字段口径和产品感，不影响主功能链路，因此维持质量性 `P3 / New`，非 P1，不创建 GitHub Issue。

- 2026-07-25 23:05 CST 运行态继续复发，状态维持 `New`：
  - 19:01-23:05 CST `data/sessions.sqlite3` 按真实 `timestamp` 新增 21 条 user / 20 条 assistant / 4 条 system compact，覆盖 13 个更新 session；近期 Web / Feishu direct 和 scheduler 会话均以 assistant 收口。
  - assistant final 污染扫描未命中空回复、`<think>`、本机路径、provider 原始错误、panic、原始工具 JSON 或 fenced JSON；本轮命中的是 scheduler 用户可见内部工具 / 中间进度句。
  - 20:03 CST Feishu scheduler `每日美股大盘风险简报` final 正常收口，但开头直接写出 `数据完整。现在生成最终复盘。`，随后又把 `quote` 字段、`hone_quote_time.beijing` 等实现口径作为行情说明。
  - 21:45 CST Feishu scheduler `每日美股大盘风控简报` final 写出 `数据校验（并行拉取中）`、`核验中`、`搜索中` 等中间执行状态表，最终正文仍继续给出盘中风控结论。
  - 这些消息业务主体已完成并送达，没有投递失败、空回复、错投、原始工具 JSON 或数据安全证据；问题仍只影响用户可见来源 / 工具口径边界和产品感，不影响主功能链路，因此维持质量性 `P3 / New`，非 P1，不创建 GitHub Issue。
...
- 2026-07-21 07:03 CST 运行态继续复发，状态维持 `New`：
  ...
  - 05:11 CST Feishu scheduler / direct actor session `Actor_feishu__direct__ou_5fea712445d905e8418bde07dbcf2cbfb2` 的 `美股收盘资金流向简报` 正常收口，但 final 开头写出 `现在并行核验主要标的收盘行情`，随后展示 `核验中 / 搜索中` 状态表，又写出 `数据加载完成后，正在补充搜索今日最新市场驱动。`
  - 该消息业务主体已完成资金流向、指数、板块和市场驱动输出并送达，没有投递失败、空回复、错投、原始工具 JSON 或数据安全证据；问题仍只影响用户可见执行过程 / 中间稿边界和产品感，不影响主功能链路，因此维持质量性 `P3 / New`，非 P1，不创建 GitHub Issue。
...
- 2026-07-13 23:02 CST 运行态继续复发，状态维持 `New`：
  ...
  - 21:46 CST Feishu direct / scheduler actor session `Actor_feishu__direct__ou_5fea712445d905e8418bde07dbcf2cbfb2` 的金融市场状态 final 正常收口，但事实段写出 `data_fetch 实时报价`，把内部行情工具名作为用户态来源说明。
  - 该样本业务主体已完成 QQQ、SPY、NVDA、MU 等市场状态分析，没有投递失败、空回复、错投、原始工具 JSON 或数据安全证据；问题仍只影响用户可见来源 / 工具口径边界和产品感，不影响主功能链路，因此维持质量性 `P3 / New`，非 P1，不创建 GitHub Issue。
...
- 2026-07-09 03:02 CST 运行态继续复发，状态维持 `New`：
  ...
  - assistant final 污染扫描未命中空回复、`reasoning_content`、`<think>`、本机绝对路径、`company_profiles/`、provider 原始错误、`mcpServers`、`Param Incorrect`、panic、资源耗尽、ACP 断连或原始工具 JSON。
  - 23:01 CST Feishu scheduler / direct actor session `Actor_feishu__direct__ou_5f2ccd43e67b89664af3a72e13f9d48773` 的 `核心观察股池晚间快报` final 正常收口，但开头继续写出 `本轮最新价格来自 data_fetch quote_short；该口径未返回逐笔时间戳和盘前/盘后字段`。
  - 该样本业务主体正常完成并送达，没有投递失败、空回复、错投、原始工具 JSON 或数据安全证据；问题仍只影响用户可见来源 / 工具口径边界和产品感，不影响主功能链路，因此维持质量性 `P3 / New`，非 P1，不创建 GitHub Issue。
...
- 2026-07-08 19:03 CST 运行态继续复发，状态维持 `New`：
  ...
  - 15:44 CST Feishu direct session `Actor_feishu__direct__ou_5f49e2e252460a05eee0ff98f685cf9f16` 的天承科技估值 final 正常收口，但估值口径段继续写出 `StockAnalysis口径显示`，把内部/实现侧行情来源口径作为用户态来源说明。
  - 该样本业务主体正常完成并送达，没有投递失败、空回复、错投、原始工具 JSON 或数据安全证据；问题仍只影响用户可见来源 / 工具口径边界和产品感，不影响主功能链路，因此维持质量性 `P3 / New`，非 P1，不创建 GitHub Issue。

- 2026-07-08 11:03 CST 运行态继续复发，状态维持 `New`：
  - 07:00-11:03 CST `data/sessions.sqlite3` 新增 23 个 user turn 与 23 条 assistant final；Feishu / Discord direct 与普通 scheduler 均以 assistant 收口，普通 scheduler 18 条均为 `completed + sent + delivered=1`。
  - assistant final 污染扫描未命中空回复、`reasoning_content`、`<think>`、本机绝对路径、`company_profiles/`、provider 原始错误、`mcpServers`、`Param Incorrect`、panic、资源耗尽或 ACP 断连。
  - 09:01 CST Feishu scheduler / direct actor session `Actor_feishu__direct__ou_5f2ccd43e67b89664af3a72e13f9d48773` 的 `核心观察池早间简报` final 正常收口，但开头继续写出 `本轮最新价格来自 data_fetch quote_short`，把内部行情工具名作为用户态来源说明。
  - 该样本业务主体正常完成并送达，没有投递失败、空回复、错投、原始工具 JSON 或数据安全证据；问题仍只影响用户可见来源 / 工具口径边界和产品感，不影响主功能链路，因此维持质量性 `P3 / New`，非 P1，不创建 GitHub Issue。

- 2026-07-07 23:02 CST 运行态继续复发，状态维持 `New`：
  - 19:02-23:02 CST `data/sessions.sqlite3` 新增 43 个 user turn 与 43 条 assistant final；Feishu / Web direct 与普通 scheduler 均以 assistant 收口，普通 scheduler 31 条均为 `completed + sent + delivered=1`。
  - assistant final 污染扫描未命中空回复、`reasoning_content`、`<think>`、本机绝对路径、`company_profiles/`、provider 原始错误、`mcpServers`、`Param Incorrect`、panic 或资源耗尽。
  - 20:03 CST Feishu scheduler / direct actor session `Actor_feishu__direct__ou_5f62439dbed2b381c0023e70a381dbd768` 的 `美股盘前要闻、持仓评级与机会观察` final 正常收口，但来源段继续写出 `LITE / COHR / QCOM 行情与新闻：本轮 data_fetch quote/news 口径`。
  - 21:35 CST session `Actor_feishu__direct__ou_5f2ccd43e67b89664af3a72e13f9d48773` 的 `科技核心股池 · 晚间击球区快报` final 开头继续写出 `本轮 quote_short 已校验 25 支最新可得价格`，数据待确认段继续写 `价格来自 data_fetch quote_short`。
  - 两个样本报告主体均正常完成并送达，没有投递失败、空回复、错投、原始工具 JSON 或数据安全证据；问题仍只影响用户可见来源 / 工具口径边界和产品感，不影响主功能链路，因此维持质量性 `P3 / New`，非 P1，不创建 GitHub Issue。
...
- `data/sessions.sqlite3` -> `cron_job_runs`
  ...
  - `TSLA 正负触发条件心跳监控` 在 `06:30`、`07:00`、`08:00`、`08:30` 多次 `completed/sent/delivered=1`，用户可见 preview 的行情口径写出 `hone_quote_time: 2026-07-28 04:00:00 +08:00`。
- 本轮判断
...
- `data/sessions.sqlite3` -> `cron_job_runs`
  ...
  - `TSLA 正负触发条件心跳监控` 在 `13:00` 与 `14:00` 多次 `completed/sent/delivered=1`，用户可见 preview 的行情口径继续写出 `hone_quote_time: 2026-07-28 04:00:00 +08:00`。
- 本轮判断
  - `hone_quote_time` 是内部行情字段名，不应作为用户可见口径外露；这是既有内部工具 / provider 字段外露缺陷复发。
  - 为何不影响功能链路，因此定级为 P3：消息正常生成并送达，主体仍包含可读监控内容；问题集中在文案边界和产品感。状态维持 `P3 / New`，非 P1。
...
- `data/sessions.sqlite3`
  ...
  - 09:59 Feishu direct 用户问 `纳斯达克100当前估值水平`，assistant final 正常收口，但行情口径写出 `hone_quote_time: 2026-07-29 04:00 +08:00`。
- 本轮判断
  - `hone_quote_time` 是内部行情字段名，不应作为用户可见口径外露；这是既有内部工具 / provider 字段外露缺陷复发。
  - 为何不影响功能链路，因此定级为 P3：直聊生成和回复完成，用户仍收到估值分析主体；问题集中在文案边界和产品感。状态维持 `P3 / New`，非 P1。

## 最新运行态复核（2026-07-29 14:02 CST）

- `data/sessions.sqlite3`
  - 巡检窗口：2026-07-29 10:01-14:02 CST。
  - 10:34 Feishu direct `托伦斯公司分析` 正常收口，但行情口径写出“本轮 DataFetch search 已确认 301583.SZ 标准实体”，把内部工具名 / 工具动作作为用户可见来源口径。
  - 12:01 Feishu scheduler `每日公司资讯与分析总结` 用户可见 final 写出“报价源时间：北京时间 2026-07-29 04:00 至 2026-07-29 05:15”，仍暴露 provider / quote implementation 口径而非产品化时间说明。
- 本轮判断
  - 这是既有 Feishu scheduler / direct 内部工具名、provider 字段和行情实现口径外露的同根复发，不新建重复缺陷。
...
- `data/logs/hone-console-page-source.log`
  ...
  - 多条 heartbeat deliver 候选继续写出内部行情字段或工具口径：23:00 / 23:30 / 00:30 / 01:00 / 01:30 CST `存储板块关键事件心跳提醒`、`光模块板块关键事件心跳提醒`、`持仓财报与重大新闻心跳提醒` 明写 `hone_quote_time.beijing`、`data_fetch quote`、`工具额度已耗尽`、`工具调用已达上限`、`本轮行情未完成核验` 等用户不应看到的实现口径。
  - 这些样本均进入 heartbeat deliver 或 duplicate suppression 候选，说明业务主体仍在生成 / 投递路径中，只是文案边界退化。
- 本轮判断
  - 这是既有 scheduler / heartbeat 内部工具名、provider 字段和行情实现口径外露的同根复发，不新建重复缺陷。
  - 为何不影响功能链路，因此定级为 P3：调度生成和出站候选仍完成，用户仍能看到监控内容；问题主要是用户可见文案边界、产品感和可信度退化。状态维持质量性 `P3 / New`，非 P1。
