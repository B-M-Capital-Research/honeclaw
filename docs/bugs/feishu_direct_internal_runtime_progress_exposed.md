...
- 2026-06-23 23:02 CST 运行态复发，状态从 `Fixed` 回退为 `New`：
  ...
  - 20:31-20:35 CST Feishu direct session `Actor_feishu__direct__ou_5f9e9e0bfe7deb3f65197e75892a377e21` 的 GSIT 投研 final 正常完成实体识别、财务、估值、风险和结论，但开头继续写出“本地没有发现 GSIT 的既有长期画像”，并说明会把判断“沉淀成 GSIT 长期画像”，结尾又写出“本轮已将 GSIT 建立为长期公司画像”。
  - 该样本晚于 2026-06-23 03:04 CST 共享净化修复记录；回复主体正常 `end_turn` 收口，不影响投研主链路、落库或投递，因此仍按质量性 `P3 / New` 处理。非 P1，不创建 GitHub Issue。

- 2026-06-23 03:04 CST 代码级修复，状态更新为 `Fixed`：
  - 共享 `sanitize_user_visible_output(...)` 新增 `本地没有 python 命令可用`、`shell 环境里可用的解释器`、`本地观察池配置`、`我没有找到本地已有的 ... 画像`、`我会新建 ... 公司画像`、`准备建立 ... 公司画像并记录框架`、`把长期可复用的 ... 沉淀到公司画像` 等复发表达收口。
  - 新增回归 `sanitize_user_visible_output_strips_watchlist_runtime_progress_variants` 与 `sanitize_user_visible_output_strips_profile_creation_progress_variants`，并复用既有 `sanitize_user_visible_output_` 套件覆盖同类 Feishu direct 样本。
...
- 2026-06-20 11:02 CST 补充同根复发证据，状态保持 `New`：
  ...
  - 07:22 CST Feishu direct session `Actor_feishu__direct__ou_5f49e2e252460a05eee0ff98f685cf9f16` 对 AI 材料 / A 股映射问题完成材料线拆解、A 股公司映射、投资判断和风险提示，但开头外露 `本地没有可复用画像`。
  - 08:30 CST Feishu direct session `Actor_feishu__direct__ou_5fba037d8699a7194dfe01a1fda5ced052` 的 OKLO 每日检查 final 外露 `本地画像显示...`、`沉淀到本地画像日期` 等内部画像动作；业务主体仍完成官方 / NRC / SEC 口径核验与无新增结论。
  - 上述样本均正常 `end_turn` 收口，业务主体可用；问题仍限定在用户可见文案边界，不影响主功能链路，因此保持质量性 `P3 / New`，非 P1，不创建 GitHub Issue。
...
- 2026-06-18 23:03 CST 补充同根复发证据，状态保持 `New`：
  ...
  - 23:01 CST Feishu direct / scheduler actor session `Actor_feishu__direct__ou_5f2ccd43e67b89664af3a72e13f9d48773` 的观察池快报外露 `主行情工具返回额度限制，无法完成 data_fetch 校验`，把内部工具名与工具失败状态写进 final。
  - 两个样本均以 `stopReason=end_turn` 收口，业务主体可用，没有空回复、错投、投递失败、原始工具 JSON、token 或本机绝对路径；问题仍限定在用户可见文案边界，按质量性 `P3 / New`，非 P1，不创建 GitHub Issue。
...
- `data/runtime/logs/acp-events.log`
  ...
  - `Actor_feishu__direct__ou_5f49e2e252460a05eee0ff98f685cf9f16` 在 07:22 CST 的 AI 材料映射 final 外露本地画像缺失口径；回复主体完成材料层级、A 股映射和风险提示。
  - `Actor_feishu__direct__ou_5fba037d8699a7194dfe01a1fda5ced052` 在 08:30 CST 的 OKLO 每日检查 final 外露本地画像参考和沉淀动作；回复主体完成无新增官方变化结论。
  - 两个样本没有本机绝对路径、token、原始工具 JSON 或思维痕迹；问题仍限定在用户可见文案边界。
- `data/runtime/logs/acp-events.log`
...
- `data/runtime/logs/acp-events.log`
  ...
  - `Actor_feishu__direct__ou_5f10f4be9ab6fe3b7fca71f37c8cdbc2c8` 在 19:36 CST 附近的 HPE 投研 final 外露 `本地还没有HPE画像` 与画像沉淀动作；回复主体完整分析 HPE 的 AI 服务器、Juniper 网络业务、财报和风险。
  - `Actor_feishu__direct__ou_5f2ccd43e67b89664af3a72e13f9d48773` 在 23:01 CST 附近的核心股池快报 final 外露 `data_fetch` 与主行情工具额度限制；回复主体仍输出观察池价格边界、击球区和来源口径。
- `data/runtime/logs/acp-events.log`
