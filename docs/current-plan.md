...
- **跨市场 ticker 解析架构修复**
  ...
  - 摘要：系统继续按 ADR 0004 / D-2026-07-19-05 / D-2026-07-19-06 收口跨市场 ticker、Agent 研究循环和 Web 单次提交。Interactive 发布否决、固定拒答和答案回写保持移除；服务端不按自然语言分隔符、大小写、长度或 ticker 形状猜实体。主 Agent 从完整原话识别全部点名标的，为每个标的声明稳定 `entity_route` 和 call-scoped `identity_match`，首轮只并行 search；finance 激活后的工具轮也只继续真实工具，结构 route 覆盖后继续工具或进入逻辑上的 finish-only 结构化 `finish_research` 交接。若 provider 复制 finish 控制调用，使用第一个可解析 handoff；无法验证时，只允许从第一个 JSON 可读的 `facts[*].evidence[*]` 固定路径恢复 Agent 明确选中的 Web result 或 DataFetch 对象，语法损坏/截断则无 fallback evidence，不再调用研究模型。DataFetch search、缺失真实 invocation、未选择的 NBIS/CWY、相邻批量标的/期间、数组、null、root 和 error 路径均不能进入 terminal。混合业务工具与 finish 时忽略所有 finish，仅执行真实业务工具。同一 Agent 的无工具 terminal 只接收当前 system contract、当前 user request、精确 Session 时间和 resolved/scoped-fallback evidence，不接收任何更早对话轮次、原始工具 transcript 或 reasoning；服务端绝不语义审查、改写、拒绝最终答案，完整 DirectFinal 仍原样接受。`3d2474b5` 生产探针已证明 FMP/Tavily、单流和无后处理拒答链路健康，但空 finish 仍让 terminal 扩写出无证据关系结论；本轮同时把浏览器自动 retry 限于 GET/HEAD、chat POST 改为 one-shot、SSE 断连改为 recovery-first，并用 sync/send 双 generation 阻止上一轮 bootstrap 覆盖下一轮 optimistic 消息。完整门禁已通过，精确提交部署、健康检查和 relationship/valuation/non-finance canary 仍待执行，之后再处理 scheduler 800G/NAND/AST/SEC P2
