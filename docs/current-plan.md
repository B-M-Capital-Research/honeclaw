...
- **ACP 对齐的 Agent Runtime 全栈重构**
  ...
  - 摘要：ACP runners 已接入 Hone MCP bridge；runner timeout 已收敛到顶层 `step=3 分钟 / overall=20 分钟` 两档。2026-08-01 会话所有权收敛为显式 `NativePersistent / StructuredReplay / EphemeralCompiledPrompt` 策略与对应输入类型：Codex ACP 通过 `CODEX_CONFIG.developer_instructions` 接收指令，每个 `session/prompt` 无论新建、续轮或 compact 后都只有当前北京时间与当前用户/附件内容，不再保留任何 seed/reseed、历史对话或工具结果拼装路径；v2 模式与指令指纹决定是否可安全 resume。Codex ACP `1.1.7` 与 OpenCode `1.18.11` 采用独立版本化流式方言，尽可能保留各自回答、思考、工具、进度和 usage 细节而不要求跨 adapter 逐字一致。2026-08-02 已收口 Codex ACP 的工具状态投影：取消的 MCP startup watcher 只保留原始诊断，不进入用户进度或业务工具计数；真实 MCP 展示有界业务参数摘要，真实 shell 在开始和完成阶段使用一致的无参数、无密钥安全类别。Discord、Telegram、飞书直聊/群聊共享 Full/Compact 渲染，iMessage 控制台接收同一安全开始态。session 持久化仍使用 `version=4 + user/assistant + content[] + status`，OpenCode 的 fresh-session replay 契约保持独立，Hone skills 继续由 Codex 原生发现和按需读取
