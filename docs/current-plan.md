...
- **跨市场 ticker 解析架构修复**
  ...
  - 2026-07-26 涨跌归因跟进：真实 Web 样本“美股股价下跌原因”已完成 quote/Web 取证却被旧生产 build 的 committed-prefix mismatch 覆盖成通用失败；Feishu 样本“周五暴跌”则把用户指定的周五静默改答成波动更大的周四。Interactive Agent 现先锁定对象/范围和目标时段、核验波动，再用同对象同绝对市场本地日期解释原因；latest quote、宽基、板块、个股和不同交易日不得互相替代。首轮精确提交 `27ea2f53` 已部署且健康，但 fresh `美股为什么大跌` canary 暴露“模型输出精确 canonical header 后继续只读取证”被误判为 persistent tool error；D-2026-07-26-02 已改为保留首行并让完整批次走注册/结构/只读校验，Agent 125/125、Channels 680/680、Web 281/281、Worker 45/45、财经契约 41/41 与完整门禁已通过，替换部署和四类 fresh actor 线上样本仍待完成
