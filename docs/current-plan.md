...
- **公开用户端管理员与会员白名单管理**
  - 状态：`in_progress`
  - 计划：`docs/current-plans/public-user-admin-whitelist-management.md`
  - 摘要：本地实现与回归已完成，生产 PostgreSQL 已确认 `13871396421` 为唯一、有效管理员；移动端与 PC 端共享的 `/me` 仅向管理员显示“管理”，提供会员白名单列表、新增与禁用。后端按管理员和北京时间自然日原子限制每天最多成功新增 5 人，并保留审计、禁止自我/其他管理员禁用、清理目标登录态及普通用户越权访问边界。当前尚未提交、推送或部署，等待正式上线与线上授权验收后归档
...
- **ACP 对齐的 Agent Runtime 全栈重构**
  ...
  - 摘要：ACP runners 已接入 Hone MCP bridge；runner timeout 已收敛到顶层 `step=3 分钟 / overall=20 分钟` 两档，当前继续收口 ACP transcript 边界、compact 防泄漏、system prompt reseed 语义，以及 ACP 子进程 / `hone-mcp` 生命周期泄漏修复。session 持久化已切到 `version=4 + user/assistant + content[] + status` 统一模型，codex/opencode 可互相切换恢复，`codex_cli` 也纳入同一 normalized 持久化契约；旧 `function_calling` 与 `multi-agent` 已退休，`gemini_acp` 已禁用并给出迁移提示。2026-07-29 管理员 Codex ACP 图片消息已跳过冗余 Apple Vision OCR 预提取；2026-07-30 本机 CLI 升级到 Codex `0.146.0` / Codex ACP `1.1.7`，model/effort 改为进程配置，并由真实 Discord 双轮验证把一个 Hone 逻辑会话固定到同一个原生 Codex session，通过 `session/resume` 连续两轮且由 Codex harness 自主管理历史与压缩；2026-07-31 静态系统提示词改为只在原生会话首轮发送，并在检测到 native `contextCompaction` 后仅补发一次，普通可信 Codex Interactive 续轮进一步收敛为“当前北京时间 + 当前用户/附件内容”，不再重复 Session 元数据、技能提示、实体循环与回答契约；已启用 Hone skills 同步为 actor workspace `.agents/skills` 下的独立 symlink，由 Codex 原生发现和按需读取，技能加载类 MCP 工具不再暴露给该路径
