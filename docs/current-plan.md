...
- **跨市场 ticker 解析架构修复**
  ...
  - 2026-07-26 涨跌归因跟进：真实 Web/飞书样本先后暴露通用失败、把用户指定周五改答周四、日期星期错误与无同日来源的“已核验原因”。`27ea2f53`、`cd78375`、`f0281adb` 与 `e46a6bf4` 已逐层把目标日期、已提交标题后的只读续跑、正文机械一致性、quote 锚定事实和确定性证据缺口收紧。最新 `e46a6bf4` canary 又精确复现两处入口死锁：OpenAI-compatible 模型把精确 SPY/QQQ 留在 `query`，且成功宽基行情仍被普通个股 identity floor 拦截。D-2026-07-26-05 现只在 `identity_match=exact_symbol` 时兼容该形态，并只为服务端锚定的宽美股涨跌问题在两个不同代表组行情尝试后开放终稿；普通证券检索规则不变。Focused Hone Tools `1/1`、Agent `131/131`、财经契约 `42/42` 已通过，完整门禁、替换部署和四类 fresh actor 线上样本仍待完成。当前 Discord token 被网关拒绝，Web/飞书使用同一精确 build 隔离运行，避免失效渠道拖掉服务
