...
- **P0 体验问题核心能力治理**
  - 状态：`in_progress`
  - 计划：`docs/current-plans/p0-experience-core-capabilities.md`
  - 摘要：以用户体验问题工作簿中的 19 条记录为输入，按身份/意图、会话与任务可靠性、证据和数值一致性、跨渠道结构化呈现、移动端信息架构五个能力域统一治理；不为单一问法、公司或截图加例外分支。已修项复核真实运行态，未修项只有能形成低副作用系统能力时才实施；最终交付带状态与总览的新 Excel，提交推送后执行不可变构建、受控重启和全服务健康验收。
