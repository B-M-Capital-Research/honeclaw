...
if contains 'fn chat_active_business_tools(' "$FUNCTION_AGENT" && contains 'ActiveBusinessStreamOutcome::DirectFinal' "$FUNCTION_AGENT" && contains 'require_complete_stream(' "$FUNCTION_AGENT" && contains 'active business stream ended before Done' "$AGENT_TESTS" && contains 'eligible_direct_final_is_preserved_without_terminal_or_second_generation' "$FUNCTION_AGENT" && contains 'finance direct-final reasoning must not persist into a later turn' "$FUNCTION_AGENT" && contains 'active_empty_retries_once_then_fails_without_terminal' "$FUNCTION_AGENT" && contains 'active_timeout_fails_immediately_without_terminal_or_visible_draft' "$FUNCTION_AGENT" && contains 'active_provider_error_fails_immediately_without_terminal' "$FUNCTION_AGENT" && contains 'iteration_limit_fails_without_terminal_call' "$FUNCTION_AGENT" && contains 'incomplete_natural_direct_final_never_commits_a_header_or_runs_terminal_recovery' "$AGENT_TESTS" && contains 'incomplete_natural_direct_final_never_persists_a_partial_terminal_prefix' "$AGENT_TESTS" && contains 'agent_owned_required_round_never_forwards_final_deltas' "$FUNCTION_AGENT" && contains 'agent_owned_tool_round_resets_an_uncommitted_final_candidate' "$FUNCTION_AGENT" && contains 'committed_finance_header_rejects_a_late_tool_before_execution' "$FUNCTION_AGENT" && contains 'visible_chunks.is_empty()' "$AGENT_TESTS" && contains 'DeferredUserOutputEmitter' "$AGENT_CORE" && contains 'AgentRunnerEvent::StreamReset' "$AGENT_EMITTER" && contains 'AgentRunnerEvent::Error' "$AGENT_EMITTER" && ! contains 'with_finish_research_terminal_synthesis' "$TOOL_REASONING_RUNNER"; then
  record success "31.natural-final-failure-boundary" "speculative and wrong-prefix output stays hidden; only an exact canonical header may commit, incomplete/error streams cannot reset it or gain terminal recovery, and a late tool cannot execute"
else
  record fail "31.natural-final-failure-boundary" "an incomplete natural final can leak a draft, trigger a reset/partial publication, or regain a terminal recovery generation"
fi
