...
if contains 'fn chat_active_business_tools(' "$FUNCTION_AGENT" && contains 'ActiveBusinessStreamOutcome::DirectFinal' "$FUNCTION_AGENT" && contains 'require_complete_stream(' "$FUNCTION_AGENT" && contains 'active business stream ended before Done' "$AGENT_TESTS" && contains 'eligible_direct_final_is_preserved_without_terminal_or_second_generation' "$FUNCTION_AGENT" && contains 'finance direct-final reasoning must not persist into a later turn' "$FUNCTION_AGENT" && contains 'active_empty_retries_once_then_fails_without_terminal' "$FUNCTION_AGENT" && contains 'active_timeout_fails_immediately_without_terminal_or_visible_draft' "$FUNCTION_AGENT" && contains 'active_provider_error_fails_immediately_without_terminal' "$FUNCTION_AGENT" && contains 'iteration_limit_fails_without_terminal_call' "$FUNCTION_AGENT" && contains 'incomplete_natural_direct_final_after_t0_ack_persists_failure_partial_without_retry' "$AGENT_TESTS" && contains 'incomplete_natural_direct_final_ignores_queued_recovery_after_t0_ack' "$AGENT_TESTS" && contains 'failed_service_prefix_run_appends_and_persists_an_explicit_failure_tail' "$AGENT_TESTS" && contains 'service_prefix_and_final_tail_are_visible_and_persisted_byte_identically' "$AGENT_TESTS" && contains 'committed_prefix_alignment_strips_only_provider_leading_whitespace' "$AGENT_TESTS" && contains 'align_response_to_committed_prefix' "$AGENT_CORE" && contains 'SERVICE_OWNED_PREFIX_FAILURE_SUFFIX' "$AGENT_CORE" && contains 'agent_owned_required_round_never_forwards_final_deltas' "$FUNCTION_AGENT" && contains 'agent_owned_tool_round_resets_an_uncommitted_final_candidate' "$FUNCTION_AGENT" && contains 'committed_finance_header_rejects_a_late_tool_before_execution' "$FUNCTION_AGENT" && contains 'DeferredUserOutputEmitter' "$AGENT_CORE" && contains 'AgentRunnerEvent::StreamReset' "$AGENT_EMITTER" && contains 'AgentRunnerEvent::Error' "$AGENT_EMITTER" && ! contains 'with_finish_research_terminal_synthesis' "$TOOL_REASONING_RUNNER"; then
  record success "31.natural-final-failure-boundary" "speculative and wrong-prefix model output stays hidden; an ACKed typed prefix survives incomplete/error streams only with an explicit persisted failure suffix, no terminal recovery generation, and no reset/error flash"
else
  record fail "31.natural-final-failure-boundary" "an incomplete natural final can leak a model draft, lose or mispersist an ACKed prefix, flash reset/error, or regain a terminal recovery generation"
fi
...
if contains 'exact_symbol_constraint: Option<String>' "$FUNCTION_AGENT" \
  ...
  && contains 'precommitted_prefix_rejects_unregistered_mcp_datafetch_before_frames_or_execution' "$FUNCTION_AGENT" \
