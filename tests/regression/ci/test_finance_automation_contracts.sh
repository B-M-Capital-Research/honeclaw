...
if contains 'fn current_turn_research_source_catalog(' "$FUNCTION_AGENT" && contains 'fn finish_research_tool_schema(sources: &[ResearchEvidenceSource])' "$FUNCTION_AGENT" && contains '["tool_call_id"]["enum"]' "$FUNCTION_AGENT" && contains 'round_tools.push(finish_research_tool_schema(&research_sources));' "$FUNCTION_AGENT" && contains 'research_source_catalog_prompt(&research_sources)' "$FUNCTION_AGENT" && contains 'let actionable_tool_calls = tcs' "$FUNCTION_AGENT" && contains 'let finish_calls = tcs' "$FUNCTION_AGENT" && contains '!finish_calls.is_empty()' "$FUNCTION_AGENT" && contains 'finish_calls.iter().find_map' "$FUNCTION_AGENT" && contains 'parse_finish_research_handoff(finish_call)' "$FUNCTION_AGENT" && contains 'fallback_scope_from_finish_calls(&finish_calls)' "$FUNCTION_AGENT" && contains 'fallback_research_handoff(' "$FUNCTION_AGENT" && contains 'handoff.unresolved_reference_count > 0' "$FUNCTION_AGENT" && contains 'MAX_INVALID_FINISH_CORRECTIONS' "$FUNCTION_AGENT" && contains 'finish_research_evidence_locators_repeatedly_unresolvable' "$FUNCTION_AGENT" && contains 'fn terminal_synthesis_prompt(' "$FUNCTION_AGENT" && contains 'handoff: &ValidatedResearchHandoff' "$FUNCTION_AGENT" && [[ "$(fixed_count '.run_terminal_synthesis(' "$FUNCTION_AGENT")" == "1" ]] && contains 'sole_finish_research_runs_one_tool_free_terminal_stream_in_the_same_agent_run' "$FUNCTION_AGENT" && contains 'duplicate_finish_calls_use_the_first_parseable_handoff_without_a_model_retry' "$FUNCTION_AGENT" && contains 'malformed_finish_gets_one_hidden_same_agent_locator_correction' "$FUNCTION_AGENT" && contains 'generic_finish_ids_are_corrected_before_one_grounded_terminal' "$FUNCTION_AGENT" && contains 'repeatedly_invalid_finish_ids_never_authorize_empty_evidence_terminal' "$FUNCTION_AGENT" && contains 'repeated_gaps_only_finish_with_citable_sources_never_enters_empty_terminal' "$FUNCTION_AGENT" && contains 'chat_terminal_streaming' "$FUNCTION_AGENT" && contains 'on_final_content_delta' "$FUNCTION_AGENT" && contains 'with_finish_research_terminal_synthesis' "$TOOL_REASONING_RUNNER" && contains 'TerminalStreamPolicy::CanonicalInvestmentHeader' "$TOOL_REASONING_RUNNER" && contains 'CommittedStreamDelta' "$RUN_EVENT" && contains 'CommittedStreamDelta' "$AGENT_EMITTER" && contains 'committed_visible_prefix.is_some()' "$AGENT_CORE" && contains 'committed_visible_prefix.is_none()' "$AGENT_CORE" && contains 'committed_terminal_prefix_makes_runner_attempt_irreversible_and_suppresses_retry' "$AGENT_TESTS" && contains 'the early committed header plus the terminal tail must exactly equal the persisted answer' "$AGENT_TESTS" && contains 'remove_tool_fields_without_tools' "$OPENROUTER" && contains 'remove_tool_fields_without_tools' "$OPENAI_COMPATIBLE" && ! contains 'TerminalReason' "$FUNCTION_AGENT" && ! contains 'DEGRADED' "$FUNCTION_AGENT" && ! contains 'degraded_terminal' "$FUNCTION_AGENT" && ! contains 'CONTINUE_RESEARCH_TOOL_NAME' "$FUNCTION_AGENT" && ! contains 'chat_research_control_decision' "$FUNCTION_AGENT"; then
  record success "28.finish-only-authorized-terminal-stream" "the same Agent binds finish-only handoffs to an exact current-turn source catalog, corrects invalid locators once before generation, and authorizes one committed terminal only with resolved evidence or explicit gaps"
else
  record fail "28.finish-only-authorized-terminal-stream" "finish handoffs can again escape current-turn source binding, enter an empty-evidence terminal, expose a speculative draft, rewrite a prefix, or retry after visible output"
fi
