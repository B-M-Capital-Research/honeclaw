...
if contains '交互式投研由同一主 Agent 在标准工具循环内完成' "$SOUL" && contains '仅完整 `Stop + Done` 终稿一次发送并原样持久化' "$SOUL" && contains '工具结果原样留在当前上下文中' "$INVESTMENT_GUARD" && contains '完整 Stop + Done 自然终稿一次发送并原样持久化' "$INVESTMENT_GUARD" && contains '必须由主 Agent 自己把“数据时间：北京时间' "$PROMPT_FILE" && contains 'pub answer_time_beijing: String' "$PROMPT_FILE" && contains 'answer_time_beijing: now.format("%Y-%m-%d %H:%M").to_string()' "$PROMPT_FILE" && [[ "$AGENT_DISCOVERY_CONTEXT_IMPL" == *'answer_time: &str'* ]] && [[ "$AGENT_DISCOVERY_CONTEXT_IMPL" != *'hone_core::beijing_now()'* ]] && [[ "$AGENT_DISCOVERY_CONTEXT_IMPL" == *'与上方 Session 上下文来自同一次时钟读取'* ]] && [[ "$AGENT_DISCOVERY_CONTEXT_IMPL" == *'【本轮最终回答契约：由主 Agent 一次完成】'* ]] && [[ "$AGENT_DISCOVERY_CONTEXT_IMPL" == *'第一可见字符必须是“数”'* ]] && [[ "$AGENT_DISCOVERY_CONTEXT_IMPL" == *'禁止在该行之前输出 `---`、Markdown 标题'* ]] && [[ "$AGENT_DISCOVERY_CONTEXT_IMPL" == *'否则忽略本节格式，正常回答用户原问题'* ]] && contains 'return one complete natural `Stop + Done` final answer' "$STOCK_RESEARCH" && contains 'Time anchor first and Interactive answer ownership' "$MARKET_ANALYSIS" && contains 'first visible line' "$MARKET_ANALYSIS" && ! contains 'server-provided' "$STOCK_RESEARCH" && ! contains 'server-owned' "$STOCK_RESEARCH" && ! contains 'server-provided' "$MARKET_ANALYSIS" && ! contains 'server-owned' "$MARKET_ANALYSIS"; then
...
if contains 'pub(crate) async fn extract_image_text' "$ATTACHMENT_VISION" \
  ...
  && contains '必须由主 Agent 自己把“数据时间：北京时间' "$PROMPT_FILE" \
  && contains 'precommitted_service_prefix_allows_non_executable_image_skill_loading' "$FUNCTION_AGENT" \
