...
if contains 'struct ResearchEvidenceLedger' "$FUNCTION_AGENT" && contains 'identity_only_attempts: u32' "$FUNCTION_AGENT" && contains 'post_identity_attempts: u32' "$FUNCTION_AGENT" && contains 'fn is_identity_only_search_call' "$FUNCTION_AGENT" && contains 'data_type.trim().eq_ignore_ascii_case("search")' "$FUNCTION_AGENT" && contains '.observe_business_call(tool_call, active_business_round);' "$FUNCTION_AGENT" && contains 'let finish_research_available' "$FUNCTION_AGENT" && contains 'active_business_round && research_evidence.post_identity_attempts > 0;' "$FUNCTION_AGENT" && contains 'POST_IDENTITY_EVIDENCE_SYSTEM_INSTRUCTION' "$FUNCTION_AGENT" && contains 'ACTIVE_RESEARCH_SYSTEM_INSTRUCTION' "$FUNCTION_AGENT" && contains 'relationship_search_does_not_offer_finish_until_post_identity_evidence' "$FUNCTION_AGENT" && contains 'ToolChoiceMode::Required' "$FUNCTION_AGENT" && contains 'tool_choice_mode == ToolChoiceMode::Required' "$OPENROUTER" && contains 'tool_choice_mode == ToolChoiceMode::Required' "$OPENAI_COMPATIBLE" && contains '必要来源已明确不可得并可如实披露' "$FUNCTION_AGENT" && contains '数据时间：北京时间 YYYY-MM-DD HH:MM；行情口径：' "$FUNCTION_AGENT" && contains '未明确标注 forward 时不得称为 Forward PE' "$FUNCTION_AGENT" && contains '关系、事件与估值证据纪律' "$PROMPT_FILE" && contains '搜索摘要明确陈述的有限事实只能按原范围使用' "$SOUL" && contains '未取得资产负债表中的现金、债务或可直接使用的企业价值' "$SOUL" && contains 'this metadata does not establish a market session' "$DATA_FETCH" && contains '只有 `extended_hours` 的规范化 bar 可以核验美股扩展时段' "$DATA_FETCH" && contains '年度数据不得写成 TTM' "$FUNCTION_AGENT" && contains '输入不完整时使用一种可严谨计算的方法并明确披露缺项，禁止补数' "$INVESTMENT_GUARD" && contains 'disclosed_valuation_gap' "$INVESTMENT_GUARD"; then
  record success "29.same-agent-evidence-stage-gate" "DataFetch search counts only as identity discovery; at least one post-identity business evidence attempt is required before the same Agent can see finish_research, while relationship, valuation, and time claims remain evidence-grounded"
else
  record fail "29.same-agent-evidence-stage-gate" "identity-only search can again unlock completion, or the Agent can fabricate relationship, valuation, or quote-time claims from incomplete evidence"
fi
