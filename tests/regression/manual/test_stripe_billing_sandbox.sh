...
[[ "$HONE_STRIPE_PRICE_ID" == price_* ]] || {
  echo "[FAIL] HONE_STRIPE_PRICE_ID must start with price_" >&2
  exit 1
}

product_json="$(stripe products retrieve "$HONE_STRIPE_PRODUCT_ID")"
price_json="$(stripe prices retrieve "$HONE_STRIPE_PRICE_ID")"
...
' "$HONE_STRIPE_PRODUCT_ID" "$HONE_STRIPE_PRICE_ID" <<<"$price_json" >/dev/null
...
echo "[PASS] Stripe test catalog is annual, linked, and the webhook rejects unsigned input"
