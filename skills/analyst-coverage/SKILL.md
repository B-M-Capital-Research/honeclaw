...
**无 ticker 的市场级问法（如「今天各大投行研报摘要」）：先定范围再取数。** 用当日 `gainers_losers` / `sector_performance` / 用户持仓与关注列表圈出 8–15 个标的，逐个取 `analyst_actions`，把当日动作汇总成一张表，再按板块归并讲。
...
- 结尾站一次队：本轮判断相对卖方共识是更乐观、更谨慎还是一致，分歧落在哪一个可观察变量上（下季指引、产能、订单、某个价格）。
