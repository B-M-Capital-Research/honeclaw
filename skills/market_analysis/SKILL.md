...
上面的四态归因只回答了"发生了什么"。异动类答案在此之后还要走完量化、第一性原理、应对三段，三段写在同一条回复里。给完催化事实再补一句"后续关注产能与订单节奏"，属于没写完。
...
3. **量化完必须与实际涨跌幅对照一句**：事件的可算影响是 X%，该时段实际波动是 Y%，结论落在"市场反应过度 / 大致匹配 / 反应不足"三者之一，并说明差额由什么解释（估值透支、动量资金踩踏、板块联动、另有未确认催化）。承接上例：6.6% 的可算稀释对不上当日 12%~14% 的跌幅，差额要归到别的原因上，不能默认稀释解释了全部跌幅。
...
9. **给 2–4 条证伪信号**，每条写清看哪个数据、什么阈值、什么时候能看到。"关注后续进展"不算证伪信号。
10. **不给精确仓位比例、买卖点指令和收益承诺**。用户在本轮提到自己的持仓或成本价时，成本价语境交给 Position Advice 处理，本段只保留事件层面的条件化动作。
...
5. **每一侧至少给一个本轮已核验的数字支撑判定**（区间涨幅、超预期幅度、指引变化任一）。只有定性叙述时如实标注"本轮未取到该侧的量化对照"，并降低置信度。
