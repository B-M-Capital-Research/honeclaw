...
本轮提示里出现【本轮相关行业】时，「增长从哪来」这一问先用行业块的需求传导链回答：链条上游的量（token、集群规模、云厂 capex）对同一行所有公司共用，这家公司拿到多少由份额、认证、产能或合约决定。把行业增速直接写成公司增速，是这一节最常见的失败。要展开这一行的完整变量表与取数位置时加载 `industry-map`。
