...
4. Independent forecast bridge: start from management guidance only as a calibration anchor, then adjust revenue and profit for volume, price, mix, cost, capacity, product ramp, customer timing, FX, and other company-specific drivers. Do not copy the guide midpoint, guide upper bound, or consensus and rename it a forecast.
5. Uncertainty band: set an explicit neutral tolerance for each decision metric using estimate dispersion, revision volatility, and business volatility. A tiny arithmetic difference inside that band is `与分析师持平`, not a confident beat or miss.
...
This is a strict mode-specific output contract. Do not use Hone's normal research-answer structure. Write like the old Workflow: direct, compact, and analytical, with the conclusion carried by the financial logic instead of process narration. Do not add a preface, timestamp, quote-basis line, executive summary, source appendix, valuation section, scenario section, risk checklist, or closing sentence outside the required headings. Render the same Markdown in chat and PDF.
...
- Immediately below `# 1. 整体分析`, begin with exactly one of `超出分析师预期`、`低于分析师预期`、`与分析师持平`. Give the call before any factor, background, or market-data discussion.
- Begin `1.2.1` with the same call. Explain it in one or two connected paragraphs, not a checklist. Start from the independent revenue/profit forecast versus the current consensus snapshot; use management guidance as one input, never as the forecast itself. State naturally when provider estimates disagree or when confidence is limited.
- Under `1.2.2`, turn the operating bridge into compact, explicit assumptions. Name the fiscal period and include `机构预期` and `独立预测` for revenue and at least one profitability metric, plus their percentage gaps and the neutral bands used for the call. For a high-growth company, explain why the gross-margin assumption is plausible. Do not enumerate generic bull/base/bear cases.
- Under `1.2.3`, write a chronological comparison rather than a data dump: comparable historical guidance-versus-actual outcomes, current management guidance, dated current consensus and revisions, then the most relevant recent contracts, orders, product releases, customer changes, or regulatory news. State whether each major catalyst was included in guidance and how much of it can affect the reporting period. Explain how it changes revenue, profit, or margin assumptions and therefore supports the opening call. Include the current quote or rating only when it materially changes the expectations bar.
...
```json
{
  ...
  "metrics": {
    "revenue": {"consensus": 8.44, "forecast": 8.62, "unit": "USD billions", "tolerance": 0.08, "report_consensus": "84.4 亿美元", "report_forecast": "86.2 亿美元"},
    "adjusted_eps": {"consensus": 34.80, "forecast": 36.20, "unit": "USD/share", "tolerance": 0.70, "report_consensus": "34.80 美元", "report_forecast": "36.20 美元"}
  },
  ...
  "forecast_bridge": [
    {"driver": "named operating driver", "metric": "revenue", "direction": "up", "affected_period": "FY2026 Q4", "evidence": "source and date"}
  ]
}
```

`metrics` must contain revenue and at least one profit metric. `tolerance` is an absolute amount in the stated unit, not a percentage. `report_consensus` and `report_forecast` are the exact human-readable values used in `1.2.2`; the renderer requires both to appear there so private audit values and published numbers cannot diverge. The renderer recomputes the direction for every `decision_metrics` entry and rejects a call that does not match the forecast, consensus, and tolerance. It also rejects missing consensus provenance, insufficient guidance history without an explanation, missing guidance-inclusion work, or a forecast bridge that is too thin.
