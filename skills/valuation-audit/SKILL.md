...
- **Market Implied 必做**（就是原来的反向估值，不用 Reverse DCF）：写出现价对应 FY+1 / FY+2 / FY+3 的 Forward PE、EV/Sales、EV/EBITDA，
  ...
  要求毛利率 X% vs 指引 Y%）。
- **树内公司的收尾三行**（三情景表之后紧接着写，不放附录）：`Earnings Revision Optionality：High / Medium / Low —— 证据`；
  `DCF：Disabled / 0%`（用户明确要求才给 Reference DCF，最高 5%，不改 Base 目标价）；`Integrity Check：PASS / WARN / FAIL —— 触发的
  Hard checks 逐条点名`（TAM/MSA/融资框架≠收入；峰值季度 EPS×4；同一增长既进 EPS 又提倍数；收入 / RPO / CapEx / 网络 TAM 重复；
  行业块禁止清单里的每一条）。这三行是自检与披露，缺一行就是没写完。
