...
| 差异 | 合格供应商家数、是否被点名指定、认证周期与切换成本 | `web_search`、`transcript` |
| 差异 | 客户验证进度与客户集中度 | `sec_filings` 的客户集中度披露、`transcript` |
| 差异 | 份额趋势、定价权兑现（毛利率相对同业的水平与方向） | `peers` 拿同业列表后比较各家同期营收增速与毛利率、`segments` 看收入结构变化 |

取不到的那项写"本轮未取到 X"，不要用行业印象补（`financials` 的 `hone_statement_coverage` 已标出哪张表没取到）。注意 **毛利率扩张是稀缺的兑现证据，不是稀缺本身**：价格涨、backlog 增而毛利率没动，说明涨价被上游或成本吃掉，稀缺没落进这家的利润表，这种情形要把差异化分往下压。
