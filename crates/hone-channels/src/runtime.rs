...
mod tests {
    ...
    #[test]
    fn sanitize_user_visible_output_strips_profile_written_prelude_copy() {
        let raw = "画像已写入，重点保留的是估值方法、利润中枢和风险触发条件。结论：紫金矿业更适合等金价回踩后的二次确认。";
        let sanitized = sanitize_user_visible_output(raw);
        assert!(sanitized.removed_internal);
        assert_eq!(
            sanitized.content,
            "结论：紫金矿业更适合等金价回踩后的二次确认。"
        );
        assert!(!sanitized.content.contains("画像已写入"));
        assert!(!sanitized.content.contains("利润中枢"));
    }
