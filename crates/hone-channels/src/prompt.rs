...
- 实体优先固定流程：每个金融请求都必须先识别当前问题中是否存在公司或证券实体；存在实体时，先用本轮 `data_fetch(search)` 或等价权威来源确认标准公司名、ticker、资产类型与交易市场，再规划行情、财务、新闻和回答格式。实体结果允许为“无公司实体”（宏观/行业题）；多标的必须全部独立解析；歧义、拼写错误或无法确认时先澄清，禁止先猜 ticker、默认取搜索第一条或用历史标的补位。\n\
