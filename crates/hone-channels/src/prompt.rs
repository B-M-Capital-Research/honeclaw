...
- 实体优先固定流程：每个金融请求都必须先识别本轮实体范围，并明确归入“当前文本点名证券 / 用户持仓或关注 / 宏观市场或行业板块 / 确认未点名实体”之一。当前文本点名证券时，先用本轮 `data_fetch(search)` 或等价权威来源确认标准公司名、ticker、资产类型与交易市场，再规划行情、财务、新闻和回答格式；普通 ticker 是标准输入，显式代码只做 exact-symbol 匹配。用户持仓或关注类请求必须先以真实 `portfolio(view)` 为真相源，再逐一核验返回的 ticker；不得从历史对话猜持仓。多标的必须全部独立解析；歧义、拼写错误或无法确认时先澄清，禁止猜 ticker、默认取搜索第一条或用历史标的补位。PE、DCF、FCF、API、ARR、EBITDA 等指标或技术缩写不能仅凭大写外形绑定证券；但 `$AI`、`ticker API`、`股票代码 ARR` 等显式代码语法仍须进入 exact-symbol 查询。\n\
- 时间与行情首行：公司、证券、市场或板块回答必须把服务端生成的“数据时间：北京时间 …；行情口径：…”作为第一条可见内容，随后展示本轮已确认实体与同代码最新可得行情，再按对应模板回答用户问题。只要本轮行情已经成功返回，就禁止声称“没有实时行情请求”“没有请求数据”“行情能力不可用”；应如实写“最新可得、非逐笔”并保留数据源时间。\n\
...
mod tests {
    ...
    fn build_prompt_bundle_always_includes_finance_domain_policy() {
        ...
        assert!(bundle.system_prompt().contains("实体优先固定流程"));
        assert!(
            bundle
                .system_prompt()
                .contains("当前文本点名证券 / 用户持仓或关注")
        );
        assert!(
            bundle
                .system_prompt()
                .contains("真实 `portfolio(view)` 为真相源")
        );
        assert!(
            bundle
                .system_prompt()
        ...
        assert!(bundle.system_prompt().contains("时间与行情首行"));
