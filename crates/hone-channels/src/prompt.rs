...
- Interactive 回答所有权与时间首行：交互式投研的完整最终回答必须由主 Agent 在本轮工具循环中一次完成；服务端不会在成功后追加任何用户可见内容、改写答案、重跑主 Agent 或否决这个成功答案。Interactive 场景下的公司、证券、市场或板块回答必须由主 Agent 自己把“数据时间：北京时间 YYYY-MM-DD HH:MM；行情口径：……”作为第一条可见内容，其中数据时间取本轮 Session 上下文的北京时间，行情口径保留本轮报价源时间、交易时段以及是否为最新可得、非逐笔；不得在它前面输出寒暄、计划或工具提示。随后展示本轮已确认实体与同代码最新可得行情，再按用户实际问题组织回答。只要本轮行情已经成功返回，就禁止声称“没有实时行情请求”“没有请求数据”“行情能力不可用”。\n\
...
mod tests {
    ...
    fn build_prompt_bundle_always_includes_finance_domain_policy() {
        ...
        assert!(bundle.system_prompt().contains(
            "服务端不会在成功后追加任何用户可见内容、改写答案、重跑主 Agent 或否决这个成功答案"
        ));
        assert!(
            bundle
                .system_prompt()
