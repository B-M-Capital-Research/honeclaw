        ...
        if options.dedicated_earnings_workflow {
            ...
            system_prompt.push_str(
                "\n\n【管理员财报工作流系统覆盖】\n\
                 ...
                 必须完成实体核验、当前财务数据获取和针对性网页搜索。本轮工具结果是唯一事实来源；\
                 只存在于模型记忆中的内容一律视为缺失。重要事实缺少或相互矛盾时必须针对该断言\
                 继续搜索；仍无法核验就明确说明或省略，严禁编造来源、链接、机构、引语、数字、\
                 事件或因果关系。每项重要数字、评级和事件必须带本轮真实来源名称、日期与 URL；\
                 调用 renderer 时还必须提交 evidence_manifest：每条重大断言逐字映射到本轮同一来源 URL\
                 的原文摘录。运行时会在 PDF 执行前核对 URL、摘录和数字；失败时继续定向搜索或删除断言。\
                 不得为凑新闻数量或满足 PDF 版式虚构内容。renderer 只负责\
                 将最终 Markdown 排版成 PDF，不要求 preview_audit、固定新闻条数、固定页数或额外\
                 报告结构。只有 skill_tool 返回 success=true 且 PDF artifact 已生成后才能结束；\
                 最终回复必须基于 renderer 返回的 validated_report_markdown，不得新增已验证报告之外的\
                 数字、事件、机构观点或因果判断，并明确提示 PDF 附件。\
        ...
        if execution.runner_request.agent_owned_finance_loop && self.actor.channel == "web" {
            execution.runner_request.service_owned_initial_prefix = Some(
                ServiceOwnedInitialPrefix {
                    ...
                    // buffering prevents a later model/tool failure from
                    // turning an irreversible header into canned refusal copy.
        ...
        if response.success {
            ...
            if let Some(prefix) = committed_visible_prefix.as_deref() {
                ...
                let service_owned_prefix = execution
                    ...
                    == Some(prefix);
                let visible_partial = prefix.to_string();
                let mut metadata = HashMap::new();
                metadata.insert("run_failed".to_string(), Value::Bool(true));
                metadata.insert("error_kind".to_string(), Value::String(format!("{kind:?}")));
