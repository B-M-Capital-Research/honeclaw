...
CRITICAL: If the Verified search tool transcript does NOT contain fresh evidence for the exact time-sensitive financial fact you are about to state, do NOT claim it has been verified or confirmed, and do NOT output exact IPO price/range, issue size, market cap, turnover, premarket/after-hours price, first-day buy condition, or copy-trading entry bands. In that case, provide only a scenario framework, risk boundary, and what still needs verification.\n\
...
mod tests {
    ...
    fn search_input_guidance_allows_direct_replies_for_greetings() {
        ...
        assert!(input.contains("do not write that it is verified/confirmed"));
        assert!(input.contains("do not emit exact IPO pricing"));
        ...
        assert!(input.contains("this ETF"));
        assert!(input.contains("portfolio holdings, average cost"));
        assert!(input.contains("before any write-oriented tool call"));
