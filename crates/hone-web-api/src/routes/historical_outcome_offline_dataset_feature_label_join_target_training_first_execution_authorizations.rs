//! Append-only independent first-execution review chain for one exact training runner.
//!
...
//! process, reads no training data, creates no model or metrics and grants no reward, shadow,
...
use super::historical_outcome_offline_dataset_feature_label_join_target_training_isolated_runners::{
    HistoricalOutcomeTrainingIsolatedRunnerRecord,
    first_execution_authorization_review_eligible_training_runners,
};
use crate::state::AppState;

const REGISTRY_SCHEMA_VERSION: &str =
    "hone-historical-outcome-training-first-execution-authorization-registry-v1";
const REVIEW_SCHEMA_VERSION: &str =
    "hone-historical-outcome-training-first-execution-authorization-review-v1";
const POLICY_VERSION: &str =
    "hone-historical-outcome-training-first-execution-authorization-v1-single-use-24h";
...
pub(crate) enum HistoricalOutcomeTrainingFirstExecutionAuthorizationVerdict {
    ApprovedForOneFutureIsolatedTrainingInvocation,
    ChangesRequested,
    Rejected,
}

#[derive(Debug, Deserialize)]
pub(crate) struct ReviewHistoricalOutcomeTrainingFirstExecutionAuthorizationRequest {
    ...
    fixed_three_arm_three_seed_sixty_five_feature_nine_target_suite_confirmed: bool,
    train_validation_and_sealed_holdout_isolation_confirmed: bool,
    exact_read_only_training_store_mount_and_no_other_data_access_confirmed: bool,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub(crate) struct HistoricalOutcomeTrainingFirstExecutionAuthorizationReview {
    ...
    pub verdict: HistoricalOutcomeTrainingFirstExecutionAuthorizationVerdict,
    ...
    pub fixed_three_arm_three_seed_sixty_five_feature_nine_target_suite_confirmed: bool,
    pub train_validation_and_sealed_holdout_isolation_confirmed: bool,
    pub exact_read_only_training_store_mount_and_no_other_data_access_confirmed: bool,
    ...
    pub one_future_isolated_training_invocation_authorized: bool,
    pub authorization_claimed: bool,
    pub invocation_endpoint_available: bool,
    pub training_run_started: bool,
...
pub(crate) struct HistoricalOutcomeTrainingFirstExecutionAuthorizationItem {
    pub runner: HistoricalOutcomeTrainingIsolatedRunnerRecord,
    pub current_binding: bool,
    pub latest_review: Option<HistoricalOutcomeTrainingFirstExecutionAuthorizationReview>,
    pub one_future_isolated_training_invocation_authorized: bool,
...
#[derive(Debug, Clone, Serialize)]
pub(crate) struct HistoricalOutcomeTrainingFirstExecutionAuthorizationRegistry {
    pub schema_version: String,
    pub policy_version: String,
    pub items: Vec<HistoricalOutcomeTrainingFirstExecutionAuthorizationItem>,
...
#[derive(Debug, Clone)]
pub(crate) struct HistoricalOutcomeTrainingFirstExecutionAuthorizationReadinessSummary {
    pub review_eligible_runner_count: usize,
    pub reviewed_runner_count: usize,
    pub approved_runner_count: usize,
    pub unexpired_authorization_count: usize,
...
#[derive(Debug, Clone, Serialize)]
pub(crate) struct ApprovedHistoricalOutcomeTrainingFirstExecutionAuthorization {
    pub runner: HistoricalOutcomeTrainingIsolatedRunnerRecord,
    pub review: HistoricalOutcomeTrainingFirstExecutionAuthorizationReview,
}

pub(crate) async fn handle_get_historical_outcome_offline_dataset_training_first_execution_authorizations(
    ...
    match build_registry(&state, Utc::now()).await {
        ...
        Err(error) => {
            warn!(%error, "historical outcome training first execution authorization registry read failed");
            crate::routes::json_error(
                StatusCode::INTERNAL_SERVER_ERROR,
                "历史结果训练隔离首次执行授权复核暂时不可用",
            )
        }
    }
}

pub(crate) async fn handle_review_historical_outcome_offline_dataset_training_first_execution_authorization(
    State(state): State<Arc<AppState>>,
    headers: HeaderMap,
    AxumPath(isolated_runner_id): AxumPath<String>,
    Json(request): Json<ReviewHistoricalOutcomeTrainingFirstExecutionAuthorizationRequest>,
) -> Response {
    let admin =
        ...
        .await
    {
        Ok(_) => match build_registry(&state, Utc::now()).await {
            Ok(value) => Json(value).into_response(),
            Err(error) => {
                warn!(%error, "historical outcome training first execution authorization registry reload failed");
                crate::routes::json_error(
                    StatusCode::INTERNAL_SERVER_ERROR,
                    "历史结果训练隔离首次执行授权复核暂时不可用",
                )
...
pub(crate) async fn historical_outcome_offline_dataset_training_first_execution_authorization_readiness_summary(
    state: &AppState,
) -> Result<HistoricalOutcomeTrainingFirstExecutionAuthorizationReadinessSummary, String> {
    let registry = build_registry(state, Utc::now()).await?;
    Ok(
        HistoricalOutcomeTrainingFirstExecutionAuthorizationReadinessSummary {
...
pub(crate) async fn execution_attempt_eligible_training_authorizations(
    state: &AppState,
    now: DateTime<Utc>,
) -> Result<Vec<ApprovedHistoricalOutcomeTrainingFirstExecutionAuthorization>, String> {
    let runners = first_execution_authorization_review_eligible_training_runners(state).await?;
    ...
    for runner in runners {
        ...
        if authorization_unexpired(&review, now) {
            approved.push(
                ApprovedHistoricalOutcomeTrainingFirstExecutionAuthorization { runner, review },
            );
        }
    }
...
#[allow(dead_code)] // Consumed by the separately gated Stage 57 execution-attempt audit.
pub(crate) async fn exact_first_execution_authorization_for_training_audit(
    state: &AppState,
    isolated_runner_id: &str,
    review_id: &str,
    review_sha256: &str,
) -> Result<Option<ApprovedHistoricalOutcomeTrainingFirstExecutionAuthorization>, String> {
    let runners = first_execution_authorization_review_eligible_training_runners(state).await?;
        ...
        .map_err(|error| error.to_string())?
    {
        ...
        let bytes = tokio::fs::read(entry.path())
            .await
            .map_err(|error| error.to_string())?;
        let review: HistoricalOutcomeTrainingFirstExecutionAuthorizationReview =
        ...
        {
            return Err(
                "training first execution authorization audit chain is duplicated or misnamed"
                    .to_string(),
            );
        }
        reviews.push(review);
    ...
    let Some(review) = reviews.into_iter().find(|review| {
        review.review_id == review_id
            ...
            && review.one_future_isolated_training_invocation_authorized
    }) else {
        return Ok(None);
    };
    Ok(Some(
...
async fn build_registry(
    state: &AppState,
    now: DateTime<Utc>,
) -> Result<HistoricalOutcomeTrainingFirstExecutionAuthorizationRegistry, String> {
    let runners = first_execution_authorization_review_eligible_training_runners(state).await?;
    ...
    for runner in runners {
        ...
        let authorized = latest_review
            .as_ref()
            .is_some_and(|review| review.one_future_isolated_training_invocation_authorized);
        let unexpired = latest_review
            .as_ref()
            .is_some_and(|review| authorization_unexpired(review, now));
        items.push(HistoricalOutcomeTrainingFirstExecutionAuthorizationItem {
            runner,
            current_binding: true,
            latest_review,
            one_future_isolated_training_invocation_authorized: authorized,
    ...
    let approved_runner_count = items
        .iter()
        .filter(|item| item.one_future_isolated_training_invocation_authorized)
        .count();
    ...
    let authorization_status = if items.is_empty() {
        "waiting_for_current_registered_not_run_isolated_training_runner"
    } else if execution_attempt_eligible_count > 0 {
        "approved_for_one_future_isolated_training_invocation_not_started"
    ...
    Ok(HistoricalOutcomeTrainingFirstExecutionAuthorizationRegistry {
        ...
        scope: "第 56 阶段只由独立复核者对精确 runner、工件和完整上游链授予 24 小时内、最多一次的未来隔离训练调用资格。授权记录没有调用或 claim 入口，不读取训练副本、不执行训练、不生成模型或指标；输出校验、奖励、影子、订单、券商和交易仍关闭。".to_string(),
    })
}
...
) -> Result<HistoricalOutcomeTrainingFirstExecutionAuthorizationReview, String> {
    if !valid_id(isolated_runner_id) {
        return Err("训练隔离 runner ID 无效".to_string());
    }
    let _lock = acquire_lock(
        state,
        &format!("historical-outcome-training-first-execution-{isolated_runner_id}"),
    )
    .await?;
    let runner = first_execution_authorization_review_eligible_training_runners(state)
        .await?
        .into_iter()
        .find(|runner| runner.isolated_runner_id == isolated_runner_id)
        .ok_or_else(|| "当前没有这条绑定有效且可复核的训练隔离 runner".to_string())?;
...
fn new_review(
    ...
    latest: Option<&HistoricalOutcomeTrainingFirstExecutionAuthorizationReview>,
    reviewer_id: &str,
    request: ReviewHistoricalOutcomeTrainingFirstExecutionAuthorizationRequest,
    submitted_at: DateTime<Utc>,
) -> Result<HistoricalOutcomeTrainingFirstExecutionAuthorizationReview, String> {
    if latest.is_some_and(|review| review.one_future_isolated_training_invocation_authorized) {
    ...
    let mut review = HistoricalOutcomeTrainingFirstExecutionAuthorizationReview {
        ...
        sealed_read_only_inputs_and_root_filesystem_confirmed: request
            .sealed_read_only_inputs_and_root_filesystem_confirmed,
        ...
        fixed_three_arm_three_seed_sixty_five_feature_nine_target_suite_confirmed: request
            .fixed_three_arm_three_seed_sixty_five_feature_nine_target_suite_confirmed,
        train_validation_and_sealed_holdout_isolation_confirmed: request
            .train_validation_and_sealed_holdout_isolation_confirmed,
        exact_read_only_training_store_mount_and_no_other_data_access_confirmed: request
            .exact_read_only_training_store_mount_and_no_other_data_access_confirmed,
        authorization_single_use_and_24_hour_expiry_confirmed: request
            .authorization_single_use_and_24_hour_expiry_confirmed,
        authorization_execution_output_validation_and_training_separation_confirmed: request
        ...
        one_future_isolated_training_invocation_authorized: false,
    ...
    let approved = review.verdict
        == HistoricalOutcomeTrainingFirstExecutionAuthorizationVerdict::ApprovedForOneFutureIsolatedTrainingInvocation;
    review.one_future_isolated_training_invocation_authorized =
...
fn validate_expected_binding(
    ...
    let stage_52 = &runner.implementation.approved_registration_review;
    if runner.isolated_runner_spec_sha256 != request.expected_isolated_runner_spec_sha256
        ...
        || stage_52.review_sha256 != request.expected_stage_52_review_sha256
        || stage_52.registration_sha256 != request.expected_stage_51_registration_sha256
        || stage_52.claim_sha256 != request.expected_stage_51_claim_sha256
        || stage_52.result_sha256 != request.expected_stage_51_result_sha256
        || stage_52.suite_specification_sha256 != request.expected_suite_specification_sha256
        || stage_52.training_store_dataset_sha256 != request.expected_training_store_dataset_sha256
        || stage_52.rows_sha256 != request.expected_rows_sha256
        || stage_52.excluded_rows_sha256 != request.expected_excluded_rows_sha256
        || stage_52.target_commitments_sha256 != request.expected_target_commitments_sha256
    {
...
fn review_all_checks(review: &HistoricalOutcomeTrainingFirstExecutionAuthorizationReview) -> bool {
    review.exact_runner_and_complete_upstream_binding_confirmed
        ...
        && review.fixed_three_arm_three_seed_sixty_five_feature_nine_target_suite_confirmed
        && review.train_validation_and_sealed_holdout_isolation_confirmed
        && review.exact_read_only_training_store_mount_and_no_other_data_access_confirmed
...
fn validate_review(
    review: &HistoricalOutcomeTrainingFirstExecutionAuthorizationReview,
    runner: &HistoricalOutcomeTrainingIsolatedRunnerRecord,
) -> Result<(), String> {
    let approved = review.verdict
        == HistoricalOutcomeTrainingFirstExecutionAuthorizationVerdict::ApprovedForOneFutureIsolatedTrainingInvocation;
    ...
    if review.schema_version != REVIEW_SCHEMA_VERSION
        ...
        || !authority_closed
    {
        return Err("historical outcome training first execution authorization is invalid or over-authorized".to_string());
    }
    Ok(())
}
...
async fn latest_review(
    state: &AppState,
    runner: &HistoricalOutcomeTrainingIsolatedRunnerRecord,
) -> Result<Option<HistoricalOutcomeTrainingFirstExecutionAuthorizationReview>, String> {
    let directory = review_directory(state, runner);
    let mut entries = match tokio::fs::read_dir(&directory).await {
        Ok(value) => value,
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok(None),
        ...
        .map_err(|error| error.to_string())?
    {
        ...
        if entry.path().file_stem().and_then(|value| value.to_str())
            != Some(review.review_id.as_str())
        {
            return Err("training first execution authorization filename mismatch".to_string());
        }
        if !ids.insert(review.review_id.clone()) || !hashes.insert(review.review_sha256.clone()) {
            return Err("training first execution authorization contains duplicates".to_string());
        }
        reviews.push(review);
    }
...
fn latest_review_from_records(
    reviews: &[HistoricalOutcomeTrainingFirstExecutionAuthorizationReview],
) -> Result<Option<HistoricalOutcomeTrainingFirstExecutionAuthorizationReview>, String> {
    if reviews.is_empty() {
        return Ok(None);
    }
    ...
    if reviews.iter().any(|review| {
        ...
        return Err(
            "training first execution authorization approval must terminate the chain".to_string(),
        );
    }
    if tips.len() != 1 {
        return Err("training first execution authorization chain has invalid tips".to_string());
    }
    let mut cursor = Some(tips[0]);
    let mut visited = BTreeSet::new();
    while let Some(review) = cursor {
        if !visited.insert(review.review_id.clone()) {
            return Err("training first execution authorization chain has a cycle".to_string());
        }
        cursor = match (&review.previous_review_id, &review.previous_review_sha256) {
            (Some(previous_id), Some(previous_sha256)) => {
                let previous = by_id.get(previous_id.as_str()).ok_or_else(|| {
                    "training first execution authorization chain is disconnected".to_string()
                })?;
                if previous.review_sha256 != *previous_sha256 {
                    return Err(
                        "training first execution authorization previous hash mismatch".to_string(),
                    );
                }
                Some(*previous)
            }
            ...
            _ => {
                return Err(
                    "training first execution authorization previous link is invalid".to_string(),
                );
            }
        };
    }
    if visited.len() != reviews.len() {
        return Err("training first execution authorization chain is disconnected".to_string());
    }
    for review in reviews {
        ...
        {
            return Err("training first execution authorization actor chain mismatch".to_string());
        }
    }
...
fn review_directory(
    ...
    decision_root(state)
        .join("historical-outcome-training-first-execution-authorization-reviews")
...
async fn acquire_lock(state: &AppState, name: &str) -> Result<ReviewLock, String> {
    ...
    {
        return Err("invalid training first execution authorization lock name".to_string());
    }
...
pub(crate) mod tests {
    use super::*;
    use crate::routes::historical_outcome_offline_dataset_feature_label_join_target_training_isolated_runners::tests::record as runner_record;
    ...
    fn request(
        ...
        ReviewHistoricalOutcomeTrainingFirstExecutionAuthorizationRequest {
            ...
            fixed_three_arm_three_seed_sixty_five_feature_nine_target_suite_confirmed: all_checks,
            train_validation_and_sealed_holdout_isolation_confirmed: all_checks,
            exact_read_only_training_store_mount_and_no_other_data_access_confirmed: all_checks,
    ...
    fn approved_review(
        runner: HistoricalOutcomeTrainingIsolatedRunnerRecord,
        submitted_at: DateTime<Utc>,
    ) -> HistoricalOutcomeTrainingFirstExecutionAuthorizationReview {
        let request = request(
            &runner,
            HistoricalOutcomeTrainingFirstExecutionAuthorizationVerdict::ApprovedForOneFutureIsolatedTrainingInvocation,
            true,
    ...
    #[allow(dead_code)] // Fixture for the separately gated Stage 57 module.
    ...
    fn rehash(review: &mut HistoricalOutcomeTrainingFirstExecutionAuthorizationReview) {
    ...
    fn approval_is_single_use_24h_and_still_does_not_execute() {
        ...
        assert!(review.one_future_isolated_training_invocation_authorized);
        ...
        assert!(!review.training_run_started);
    ...
    fn exact_runner_artifact_and_upstream_binding_are_required() {
        ...
        let mut runner_drift_request = request(
            &runner,
            HistoricalOutcomeTrainingFirstExecutionAuthorizationVerdict::ChangesRequested,
            false,
            None,
        );
    ...
    #[test]
    fn exact_training_dataset_targets_and_independent_audits_are_required() {
        let runner = runner_record();
        ...
        target_drift_request.expected_target_commitments_sha256 = "c".repeat(64);
    ...
    #[test]
    fn runner_and_authorization_keep_training_store_closed_until_execution_claim() {
        let runner = runner_record();
        assert!(!runner.runner_contract.training_store_reads_allowed);
        let review = approved_review(runner, Utc::now());
        assert!(review.exact_read_only_training_store_mount_and_no_other_data_access_confirmed);
        assert!(!review.training_data_access_authorized);
        assert!(!review.training_run_started);
    }
    ...
    fn any_claim_execution_output_or_downstream_authority_tamper_fails_closed() {
        ...
        review.model_artifact_created = true;
        rehash(&mut review);
        assert!(validate_review(&review, &review.runner).is_err());
        review.model_artifact_created = false;
        review.metrics_created = true;
        rehash(&mut review);
        assert!(validate_review(&review, &review.runner).is_err());
        review.metrics_created = false;
        review.sealed_holdout_access_authorized = true;
        rehash(&mut review);
        assert!(validate_review(&review, &review.runner).is_err());
    }
}
