//! Stage 119 create-once registration of an isolated observation-ledger-transition runner specification.
//!
//! The record freezes one exact Stage 118 approval, a proposed future artifact identity, immutable
//! code revision, fixed unprivileged runtime contract, read-only content-addressed input and
//! create-once untrusted candidate output. It does not create transition source, an executable
//! artifact, a callable entrypoint, runtime, input mount/read, opening portfolio snapshot, ledger,
//! event, position, cash, NAV/performance, model/metric, training/RL, reward, order, broker or
//! trading capability.
...
use super::historical_outcome_offline_dataset_feature_label_join_target_training_sealed_holdout_evaluation_controlled_shadow_experiment_forward_observation_first_natural_forward_cycle_observation_ledger_transition_implementation_reviews::{
    ControlledShadowObservationLedgerTransitionImplementationReviewRecord,
    IndependentlyApprovedControlledShadowObservationLedgerTransitionImplementation,
    independently_approved_observation_ledger_transition_implementations_for_isolated_runner_registration,
};
...
const REGISTRY_SCHEMA_VERSION: &str =
    "hone-controlled-shadow-observation-ledger-transition-isolated-runner-registry-v1";
const RECORD_SCHEMA_VERSION: &str =
    "hone-controlled-shadow-observation-ledger-transition-isolated-runner-specification-v1";
const CONTRACT_SCHEMA_VERSION: &str = "hone-controlled-shadow-observation-ledger-transition-isolated-runner-zero-capability-contract-v1";
const POLICY_VERSION: &str = "hone-controlled-shadow-observation-ledger-transition-isolated-runner-create-once-v1-no-execution";
const STATUS: &str = "registered_not_authorized_not_run";
const NEXT_GATE: &str =
    "chain_external_first_observation_ledger_transition_execution_authorization_review";
const RUNTIME_IDENTITY: &str = "hone-natural-forward-observation-ledger-transition-runtime";
const RUNTIME_VERSION: &str = "v1-proposed-artifact-not-present-no-current-mount";
const FUTURE_INPUT_ENVELOPE: &str =
    "stage_114_admitted_observation_evidence_v1_read_only_content_addressed";
const FUTURE_OUTPUT_ENVELOPE: &str = "canonical_observation_ledger_transition_candidate_envelope_v1_create_once_untrusted_no_authoritative_financial_state";
...
pub(crate) enum ControlledShadowObservationLedgerTransitionIsolatedRunnerKind {
    EphemeralDeterministicObservationLedgerTransitionSpecification,
}

#[derive(Debug, Deserialize)]
pub(crate) struct RegisterControlledShadowObservationLedgerTransitionIsolatedRunnerRequest {
    ...
    runner_name: String,
    runner_kind: ControlledShadowObservationLedgerTransitionIsolatedRunnerKind,
    ...
    exact_current_stage_51_through_stage_118_binding_confirmed: bool,
    registrar_independent_from_stage_118_and_complete_prior_chain_confirmed: bool,
    ...
    proposed_artifact_identity_code_revision_and_reproduction_procedure_bound_but_artifact_not_present_confirmed:
        bool,
    all_eight_observation_ledger_transition_functions_and_canonical_schemas_preserved_confirmed:
        bool,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub(crate) struct ControlledShadowObservationLedgerTransitionIsolatedRunnerContract {
    pub schema_version: String,
    pub contract_sha256: String,
    pub stage_118_implementation_review_id: String,
    pub stage_118_implementation_review_sha256: String,
    pub stage_118_independent_audit_sha256: String,
    pub stage_117_implementation_id: String,
    pub stage_117_implementation_sha256: String,
    pub stage_117_implementation_contract_sha256: String,
    pub stage_116_specification_review_sha256: String,
    pub stage_115_specification_registration_sha256: String,
    pub stage_115_observation_ledger_transition_specification_sha256: String,
    pub stage_114_admission_review_sha256: String,
    ...
    pub future_input_stage_114_admission_required: bool,
    pub future_opening_portfolio_snapshot_admission_required: bool,
    pub opening_portfolio_snapshot_present: bool,
    pub financial_event_allowlist: Vec<String>,
    pub financial_event_allowlist_empty_without_opening_snapshot_required: bool,
    pub non_financial_observation_notice_only_without_opening_snapshot: bool,
    ...
    pub future_output_authoritative_financial_state_allowed: bool,
    ...
    pub ledger_event_writes_allowed: bool,
    pub position_writes_allowed: bool,
    pub cash_writes_allowed: bool,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub(crate) struct ControlledShadowObservationLedgerTransitionIsolatedRunnerRecord {
    pub schema_version: String,
    ...
    pub implementation_review:
        ControlledShadowObservationLedgerTransitionImplementationReviewRecord,
    pub excluded_prior_actor_ids: Vec<String>,
    pub runner_name: String,
    pub runner_kind: ControlledShadowObservationLedgerTransitionIsolatedRunnerKind,
    ...
    pub runner_contract: ControlledShadowObservationLedgerTransitionIsolatedRunnerContract,
    pub status: String,
    pub confirmations_complete: bool,
    pub exact_current_stage_51_through_stage_118_binding_confirmed: bool,
    pub registrar_independent_from_stage_118_and_complete_prior_chain_confirmed: bool,
    ...
    pub proposed_artifact_identity_code_revision_and_reproduction_procedure_bound_but_artifact_not_present_confirmed:
        bool,
    pub all_eight_observation_ledger_transition_functions_and_canonical_schemas_preserved_confirmed:
        bool,
...
pub(crate) struct ControlledShadowObservationLedgerTransitionIsolatedRunnerItem {
...
#[derive(Debug, Clone, Serialize)]
pub(crate) struct ControlledShadowObservationLedgerTransitionIsolatedRunnerRegistry {
    pub schema_version: String,
    pub policy_version: String,
    ...
    pub items: Vec<ControlledShadowObservationLedgerTransitionIsolatedRunnerItem>,
...
#[derive(Debug, Clone)]
pub(crate) struct ControlledShadowObservationLedgerTransitionIsolatedRunnerReadinessSummary {
    pub registration_eligible_count: usize,
    pub runner_count: usize,
    pub current_binding_runner_count: usize,
    pub first_execution_authorization_review_eligible_count: usize,
...
pub(crate) async fn handle_get_controlled_shadow_observation_ledger_transition_isolated_runners(
    ...
    match build_registry(&state).await {
        ...
        Err(error) => {
            warn!(%error, "observation-ledger-transition isolated runner registry failed");
            crate::routes::json_error(
                StatusCode::INTERNAL_SERVER_ERROR,
                "观察到账本转换 隔离 runner 规格登记表暂时不可用",
            )
        }
    }
}

pub(crate) async fn handle_register_controlled_shadow_observation_ledger_transition_isolated_runner(
    State(state): State<Arc<AppState>>,
    headers: HeaderMap,
    AxumPath(implementation_id): AxumPath<String>,
    Json(request): Json<RegisterControlledShadowObservationLedgerTransitionIsolatedRunnerRequest>,
) -> Response {
    let admin =
...
pub(crate) async fn controlled_shadow_observation_ledger_transition_isolated_runner_readiness_summary(
    state: &AppState,
) -> Result<ControlledShadowObservationLedgerTransitionIsolatedRunnerReadinessSummary, String> {
    let value = build_registry(state).await?;
    Ok(
        ControlledShadowObservationLedgerTransitionIsolatedRunnerReadinessSummary {
...
#[allow(dead_code)]
pub(crate) async fn observation_ledger_transition_runners_for_first_execution_authorization_review(
    state: &AppState,
) -> Result<Vec<ControlledShadowObservationLedgerTransitionIsolatedRunnerRecord>, String> {
    let approved =
        independently_approved_observation_ledger_transition_implementations_for_isolated_runner_registration(
            state,
        )
...
async fn build_registry(
    state: &AppState,
) -> Result<ControlledShadowObservationLedgerTransitionIsolatedRunnerRegistry, String> {
    let approved =
        independently_approved_observation_ledger_transition_implementations_for_isolated_runner_registration(
            state,
        )
    ...
    let mut items = read_runners(state)
        ...
        .map(|runner| {
            ...
            ControlledShadowObservationLedgerTransitionIsolatedRunnerItem {
    ...
    let runner_status = if approved.is_empty() {
        "waiting_stage_118_independently_approved_observation_ledger_transition_implementation"
    } else if first_execution_authorization_review_eligible_count > 0 {
        "observation_ledger_transition_isolated_runner_registered_not_authorized_not_run"
    } else if registration_eligible_count > 0 {
        "ready_for_observation_ledger_transition_isolated_runner_specification_registration"
    } else {
        "observation_ledger_transition_isolated_runner_binding_stale_fail_closed"
    };
    Ok(ControlledShadowObservationLedgerTransitionIsolatedRunnerRegistry {
        ...
        scope: "Stage 119 只把 Stage 118 独立批准绑定到未来 runner 工件身份、不可变代码版本、固定非特权 runtime、Stage 114 已准入只读输入和 create-once 不可信候选输出。当前没有源码、可执行工件、入口、runtime、挂载、输入读取或任何账本转换；期初组合快照不存在，金融事件白名单为空。登记后下一步只能进入 Stage 120 责任链外首次执行授权复核。".to_string(),
    })
}
...
async fn register_once(
...
) -> Result<ControlledShadowObservationLedgerTransitionIsolatedRunnerRecord, String> {
    ...
    let value = approved
        ...
        .ok_or_else(|| {
            "当前没有这条绑定有效且已独立批准的 Stage 117 观察到账本转换实现".to_string()
        })?;
    ...
    {
        return Err("这条 Stage 118 批准已经登记过隔离 runner 规格".to_string());
    }
...
fn validate_expected_binding(
    approved: &IndependentlyApprovedControlledShadowObservationLedgerTransitionImplementation,
    request: &RegisterControlledShadowObservationLedgerTransitionIsolatedRunnerRequest,
) -> Result<(), String> {
    ...
    {
        return Err(
            "Stage 114–118 观察到账本转换准入、规格、实现、审计或复核绑定已经变化".to_string(),
        );
    }
    Ok(())
}
...
fn new_record(
    ...
    {
        return Err("runner 规格登记人必须独立于 Stage 118 和完整 Stage 51–118 责任链".to_string());
    }
    if !request_checks(request) {
        return Err(
            "必须逐项确认 Stage 119 runner 规格、观察到账本转换语义与零权限边界".to_string(),
        );
    }
    ...
    let mut record = ControlledShadowObservationLedgerTransitionIsolatedRunnerRecord {
    ...
    validate_observation_ledger_transition_isolated_runner_for_review(&record)?;
    Ok(record)
}
...
) -> Result<ControlledShadowObservationLedgerTransitionIsolatedRunnerContract, String> {
    let implementation = &approved.implementation;
    let source = &implementation.implementation_contract;
    let specification = &source.exact_observation_ledger_transition_specification;
    let mut contract = ControlledShadowObservationLedgerTransitionIsolatedRunnerContract {
        schema_version: CONTRACT_SCHEMA_VERSION.to_string(),
        contract_sha256: String::new(),
        stage_118_implementation_review_id: approved.review.review_id.clone(),
        stage_118_implementation_review_sha256: approved.review.review_sha256.clone(),
        stage_118_independent_audit_sha256: approved.review.independent_audit.audit_sha256.clone(),
        stage_117_implementation_id: implementation.implementation_id.clone(),
        stage_117_implementation_sha256: implementation.implementation_sha256.clone(),
        stage_117_implementation_contract_sha256: source.contract_sha256.clone(),
        stage_116_specification_review_sha256: implementation
            .upstream_specification_review
            .review_sha256
            .clone(),
        stage_115_specification_registration_sha256: implementation
            .upstream_specification_registration
            .registration_sha256
            .clone(),
        stage_115_observation_ledger_transition_specification_sha256: implementation
            .upstream_specification_registration
            .specification
            .specification_sha256
            .clone(),
        stage_114_admission_review_sha256: specification.stage_114_review_sha256.clone(),
        ...
        future_input_stage_114_admission_required: true,
        future_opening_portfolio_snapshot_admission_required: true,
        opening_portfolio_snapshot_present: false,
        financial_event_allowlist: Vec::new(),
        financial_event_allowlist_empty_without_opening_snapshot_required: true,
        non_financial_observation_notice_only_without_opening_snapshot: true,
        ...
        future_output_authoritative_financial_state_allowed: false,
        ...
        ledger_event_writes_allowed: false,
        position_writes_allowed: false,
        cash_writes_allowed: false,
...
fn request_checks(
    ...
    request.exact_current_stage_51_through_stage_118_binding_confirmed
        && request.registrar_independent_from_stage_118_and_complete_prior_chain_confirmed
        ...
        && request.all_eight_observation_ledger_transition_functions_and_canonical_schemas_preserved_confirmed
...
pub(crate) fn validate_observation_ledger_transition_isolated_runner_for_review(
    record: &ControlledShadowObservationLedgerTransitionIsolatedRunnerRecord,
) -> Result<(), String> {
    ...
    let closed = !contract.source_artifact_present
        ...
        && !contract.ledger_event_writes_allowed
        && !contract.position_writes_allowed
        && !contract.cash_writes_allowed
    ...
    let valid = record.schema_version == RECORD_SCHEMA_VERSION
        ...
        && record.exact_current_stage_51_through_stage_118_binding_confirmed
        && record.registrar_independent_from_stage_118_and_complete_prior_chain_confirmed
        && record
            .implementation_review_audit_contract_and_specification_hashes_reproduced_confirmed
        && record
            .proposed_artifact_identity_code_revision_and_reproduction_procedure_bound_but_artifact_not_present_confirmed
        && record.all_eight_observation_ledger_transition_functions_and_canonical_schemas_preserved_confirmed
        && record
            .future_input_only_stage_114_admitted_read_only_content_addressed_output_confirmed
        ...
        && record
            .future_output_content_addressed_create_once_untrusted_and_independently_validated_confirmed
        && record.opening_portfolio_snapshot_absent_and_financial_event_allowlist_empty_confirmed
        && record.future_financial_events_require_separately_admitted_opening_snapshot_confirmed
        ...
        && runner_contract_matches_embedded_binding(record)
        ...
        && contract.future_input_stage_114_admission_required
        && contract.future_opening_portfolio_snapshot_admission_required
        ...
        && !contract.future_output_investment_semantics_allowed
        ...
        && contract.maximum_output_bytes == MAXIMUM_OUTPUT_BYTES
        && closed
        && record_closed;
    valid
        .then_some(())
        .ok_or_else(|| "Stage 119 观察到账本转换 runner 规格无效、漂移或越权".to_string())
}

fn runner_contract_matches_embedded_binding(
    record: &ControlledShadowObservationLedgerTransitionIsolatedRunnerRecord,
) -> bool {
    let implementation = &record.implementation;
    let review = &record.implementation_review;
    let source = &implementation.implementation_contract;
    let specification = &source.exact_observation_ledger_transition_specification;
    let contract = &record.runner_contract;
    review.implementation == *implementation
        && contract.stage_118_implementation_review_id == review.review_id
        && contract.stage_118_implementation_review_sha256 == review.review_sha256
        && contract.stage_118_independent_audit_sha256 == review.independent_audit.audit_sha256
        && contract.stage_117_implementation_id == implementation.implementation_id
        && contract.stage_117_implementation_sha256 == implementation.implementation_sha256
        && contract.stage_117_implementation_contract_sha256 == source.contract_sha256
        && contract.stage_116_specification_review_sha256
            == implementation.upstream_specification_review.review_sha256
        && contract.stage_115_specification_registration_sha256
            == implementation
                .upstream_specification_registration
                .registration_sha256
        && contract.stage_115_observation_ledger_transition_specification_sha256
            == implementation
                .upstream_specification_registration
                .specification
                .specification_sha256
        && contract.stage_114_admission_review_sha256 == specification.stage_114_review_sha256
        && contract.stage_113_validation_sha256 == specification.stage_113_validation_sha256
        && contract.stage_112_result_sha256 == specification.stage_112_result_sha256
        && contract.stage_111_claim_sha256 == specification.stage_111_claim_sha256
        && contract.exact_approved_implementation_contract == *source
        && contract.runtime_identity == RUNTIME_IDENTITY
        && contract.runtime_version == RUNTIME_VERSION
...
fn binding_matches(
    ...
    runner.implementation == approved.implementation
        ...
        && validate_observation_ledger_transition_isolated_runner_for_review(runner).is_ok()
}
...
fn fingerprint_without<T: Serialize>(value: &T, fields: &[&str]) -> Result<String, String> {
    ...
    let object = value
        .as_object_mut()
        .ok_or_else(|| "Stage 119 指纹载荷无效".to_string())?;
    for field in fields {
        object.remove(*field);
    }
    Ok(format!(
...
async fn read_runners(
        ...
        .map_err(|error| error.to_string())?
    {
        ...
        {
            let runner: ControlledShadowObservationLedgerTransitionIsolatedRunnerRecord =
            ...
            {
                return Err("Stage 119 runner 文件名与记录 ID 不一致".to_string());
            }
    ...
    for runner in &runners {
        ...
        {
            return Err("Stage 119 runner 登记重复或分叉".to_string());
        }
    }
    Ok(runners)
}
...
fn runner_root(state: &AppState) -> PathBuf {
    decision_root(state)
        .join("controlled-shadow-first-natural-forward-cycle-observation-ledger-transition-isolated-runners")
}
...
async fn acquire_lock(state: &AppState, implementation_id: &str) -> Result<RunnerLock, String> {
    ...
    let path = directory.join(format!(
        "controlled-shadow-observation-ledger-transition-isolated-runner-{implementation_id}.lock"
    ));
    ...
    tokio::fs::OpenOptions::new()
        ...
        .open(&path)
        .await
        .map_err(|_| "已有同一 Stage 119 runner 登记正在进行".to_string())?;
    Ok(RunnerLock(path))
}
...
mod tests {
    ...
    fn request_requires_every_zero_capability_confirmation() {
        let request = RegisterControlledShadowObservationLedgerTransitionIsolatedRunnerRequest {
            ...
            expected_specification_review_sha256: "1".repeat(64), expected_specification_registration_sha256: "2".repeat(64), expected_observation_ledger_transition_specification_sha256: "3".repeat(64),
            ...
            runner_name: "隔离行情物化器".to_string(), runner_kind: ControlledShadowObservationLedgerTransitionIsolatedRunnerKind::EphemeralDeterministicObservationLedgerTransitionSpecification,
            ...
            future_input_constraints: "只读 Stage 114 已准入内容寻址输出。".to_string(), future_output_constraints: "create-once 不可信输出。".to_string(),
            exact_current_stage_51_through_stage_118_binding_confirmed: true, registrar_independent_from_stage_118_and_complete_prior_chain_confirmed: true,
    ...
    #[test]
    fn next_gate_is_review_not_execution() {
        assert_eq!(
            NEXT_GATE,
            "chain_external_first_observation_ledger_transition_execution_authorization_review"
        );
        assert_eq!(STATUS, "registered_not_authorized_not_run");
    }

    #[test]
    fn candidate_output_cannot_claim_authoritative_financial_state() {
        assert!(FUTURE_INPUT_ENVELOPE.starts_with("stage_114_admitted_"));
        assert!(FUTURE_OUTPUT_ENVELOPE.contains("create_once_untrusted"));
        assert!(FUTURE_OUTPUT_ENVELOPE.contains("no_authoritative_financial_state"));
    }
}
