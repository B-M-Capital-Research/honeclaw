//! Immutable specification registry for a future isolated offline-dataset join/target runner.
//!
...
use super::historical_outcome_offline_dataset_feature_label_join_target_implementation_reviews::{
    ApprovedHistoricalOutcomeFeatureLabelJoinTargetImplementationReview,
    HistoricalOutcomeFeatureLabelJoinTargetImplementationReview,
    approved_historical_outcome_feature_label_join_target_implementation_reviews,
    validate_historical_outcome_feature_label_join_target_implementation_review_for_runner_registration,
};
use super::historical_outcome_offline_dataset_feature_label_join_target_implementations::HistoricalOutcomeFeatureLabelJoinTargetImplementationRecord;
use crate::state::AppState;

const REGISTRY_SCHEMA_VERSION: &str =
    "hone-historical-outcome-feature-label-join-target-isolated-runner-registry-v1";
const RUNNER_SCHEMA_VERSION: &str =
    "hone-historical-outcome-feature-label-join-target-isolated-runner-v1";
const POLICY_VERSION: &str =
    "hone-feature-label-join-target-isolated-runner-registration-v1-no-execution";
const CONTRACT_SCHEMA_VERSION: &str = "hone-feature-label-join-target-isolated-runner-contract-v1";
const RUNTIME_IDENTITY: &str = "hone-isolated-feature-label-join-target-runtime";
const RUNTIME_VERSION: &str = "v1-no-ambient-capabilities";
const STATUS: &str = "registered_not_run";
const INPUT_MOUNT_CONTRACT: &str = "未来经新的独立首次执行授权后，只能挂载本记录精确绑定的已独立复核实现、join/target 规范、正式特征工件和历史结果数据集作为只读输入；不得联网补数、替换输入、读取未绑定标签/训练/生产资料或继承宿主环境。";
const OUTPUT_CONTRACT: &str = "未来首次执行只能向一次性隔离目录 create-once 写入内容寻址的未验证 join/target 输出；不得覆盖、写入生产或训练存储、把输出当作正式 joined rows，或直接进入训练。当前登记不创建目录或输出。";
...
const MAXIMUM_MEMORY_MEBIBYTES: u32 = 4_096;
...
pub(crate) enum HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerKind {
    EphemeralDeterministicProcess,
}

#[derive(Debug, Deserialize)]
pub(crate) struct RegisterHistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRequest {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub(crate) struct HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerContract {
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub(crate) struct HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRecord {
    pub schema_version: String,
    ...
    pub implementation_review: HistoricalOutcomeFeatureLabelJoinTargetImplementationReview,
    pub excluded_prior_actor_ids: Vec<String>,
    pub registrar_independent_from_complete_approval_chain: bool,
    pub runner_name: String,
    pub runner_kind: HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerKind,
    pub runner_code_revision: String,
    pub runner_artifact_sha256: String,
    pub rationale: String,
    pub known_limitations: String,
    pub runner_contract: HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerContract,
    pub status: String,
...
pub(crate) struct HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerItem {
...
#[derive(Debug, Clone, Serialize)]
pub(crate) struct HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRegistry {
    pub schema_version: String,
    pub policy_version: String,
    pub eligible_reviews: Vec<ApprovedHistoricalOutcomeFeatureLabelJoinTargetImplementationReview>,
    pub allowed_runner_kinds: Vec<HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerKind>,
    pub registration_allowed: bool,
    pub items: Vec<HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerItem>,
...
#[derive(Debug, Clone)]
pub(crate) struct HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerReadinessSummary {
    pub runner_count: usize,
    pub current_binding_runner_count: usize,
    pub first_execution_authorization_review_eligible_count: usize,
    pub runner_status: String,
...
pub(crate) async fn handle_get_historical_outcome_offline_dataset_feature_label_join_target_isolated_runners(
    ...
    match build_registry(&state).await {
        ...
        Err(error) => {
            warn!(%error, "historical outcome feature-label join/target isolated runner registry read failed");
            crate::routes::json_error(
                StatusCode::INTERNAL_SERVER_ERROR,
                "历史结果join/target 隔离 runner 规范登记表暂时不可用",
            )
        }
    }
}

pub(crate) async fn handle_register_historical_outcome_feature_label_join_target_isolated_runner(
    State(state): State<Arc<AppState>>,
    headers: HeaderMap,
    Json(request): Json<RegisterHistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRequest>,
) -> Response {
    let admin =
    ...
    match register_once(&state, &admin.user_id, request).await {
        Ok(_) => match build_registry(&state).await {
            Ok(value) => Json(value).into_response(),
            Err(error) => {
                warn!(%error, "historical outcome feature-label join/target isolated runner registry reload failed");
                crate::routes::json_error(
                    StatusCode::INTERNAL_SERVER_ERROR,
                    "历史结果join/target 隔离 runner 规范登记表暂时不可用",
                )
...
pub(crate) async fn historical_outcome_feature_label_join_target_isolated_runner_readiness_summary(
    state: &AppState,
) -> Result<HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerReadinessSummary, String> {
    let registry = build_registry(state).await?;
    Ok(
        HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerReadinessSummary {
...
pub(crate) async fn first_execution_authorization_review_eligible_join_target_runners(
    state: &AppState,
) -> Result<Vec<HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRecord>, String> {
    let approved =
        approved_historical_outcome_feature_label_join_target_implementation_reviews(state).await?;
    let index = approved
        .iter()
...
async fn build_registry(
    state: &AppState,
) -> Result<HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRegistry, String> {
    ...
    let mut items = read_runners(state)
        ...
        .map(|runner| {
            let approved_review_binding_current = review_index
                .get(runner.implementation_review.review_id.as_str())
                .is_some_and(|value| binding_matches(&runner, value));
            HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerItem {
    ...
    let runner_status = if eligible_reviews.is_empty() {
        "waiting_for_current_approved_join_target_implementation_review"
    } else if runner_count == 0 {
        "waiting_for_immutable_isolated_join_target_runner_registration"
    } else if current_binding_runner_count == 0 {
        "registered_binding_stale"
    } else {
        STATUS
    };
    Ok(HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRegistry {
        schema_version: REGISTRY_SCHEMA_VERSION.to_string(),
        policy_version: POLICY_VERSION.to_string(),
        registration_allowed: !eligible_reviews.is_empty(),
        eligible_reviews,
        allowed_runner_kinds: vec![HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerKind::EphemeralDeterministicProcess],
        items,
        runner_count,
        ...
        scope: "第 40 阶段只登记绑定当前独立批准实现的 join/target 隔离 runner 规范，冻结 runner 工件、代码、运行时、只读精确输入、create-once 内容寻址输出和静态资源上限；不提供调用入口。唯一下一门禁是独立首次执行授权复核；当前不读标签、不执行 join、不分配目标、不创建 joined rows，也不训练、奖励、影子、订单、券商或交易。".to_string(),
    })
}
...
async fn register_once(
...
) -> Result<HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRecord, String> {
    ...
    let value = approved
        ...
        .ok_or_else(|| "当前没有这条绑定有效且已独立批准的隔离join/target 实现".to_string())?;
    ...
    {
        return Err(
            "完全相同的join/target 隔离 runner 规范已经登记；工件或规范变化时请登记新版本"
                .to_string(),
        );
    }
...
fn validate_expected_binding(
    approved: &ApprovedHistoricalOutcomeFeatureLabelJoinTargetImplementationReview,
    request: &RegisterHistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRequest,
) -> Result<(), String> {
    ...
    let spec = &specification_review.specification;
    ...
    if request.expected_implementation_id != implementation.implementation_id
        ...
        || request.expected_specification_sha256 != spec.specification_sha256
        || request.expected_specification_body_sha256 != spec.specification_body_sha256
        || request.expected_join_specification_sha256
            != spec.join_specification.specification_sha256
        || request.expected_target_specification_sha256
            != spec.target_specification.specification_sha256
        || request.expected_combined_artifact_sha256 != spec.combined_artifact_sha256
        || request.expected_dataset_content_sha256 != spec.dataset_content_sha256
...
fn new_record(
    ...
    let mut record = HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRecord {
...
fn runner_contract() -> HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerContract {
    let mut contract = HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerContract {
...
fn validate_record(
    record: &HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRecord,
) -> Result<(), String> {
    validate_historical_outcome_feature_label_join_target_implementation_review_for_runner_registration(
        &record.implementation_review,
    )?;
    let authority_closed = !record.first_execution_authorized
        && !record.join_target_execution_started
    ...
    if record.schema_version != RUNNER_SCHEMA_VERSION
        ...
        || !record
            ...
            .future_isolated_join_target_runner_registration_eligible
    ...
    {
        return Err(
            "historical outcome feature-label join/target isolated runner is invalid or over-authorized"
                .to_string(),
        );
    }
    Ok(())
...
async fn read_runners(
        ...
        .map_err(|error| error.to_string())?
    {
        ...
        let bytes = tokio::fs::read(entry.path())
            .await
            .map_err(|error| error.to_string())?;
        let runner: HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRecord =
        ...
        {
            return Err("feature-label join/target isolated runner path mismatch".to_string());
        }
        if !ids.insert(runner.isolated_runner_id.clone())
            || !fingerprints.insert(runner.isolated_runner_spec_sha256.clone())
        {
            return Err(
                "feature-label join/target isolated runner registry contains duplicates"
                    .to_string(),
            );
        }
        runners.push(runner);
...
fn runner_root(state: &AppState) -> PathBuf {
    decision_root(state).join("historical-outcome-feature-label-join-target-isolated-runners")
}
...
async fn acquire_lock(state: &AppState) -> Result<RunnerLock, String> {
    ...
    tokio::fs::create_dir_all(&directory)
        .await
        .map_err(|error| error.to_string())?;
    let path = directory.join("historical-outcome-feature-label-join-target-isolated-runner.lock");
    for attempt in 0..8 {
            ...
            .await
        {
            ...
            Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => {
                ...
                return Err("另一个管理员正在登记join/target 隔离 runner 规范".to_string());
            }
            Err(error) => return Err(error.to_string()),
        }
    }
    Err("join/target 隔离 runner 登记锁不可用".to_string())
}

#[cfg(test)]
pub(crate) mod tests {
    use super::*;
    use crate::routes::historical_outcome_offline_dataset_feature_label_join_target_implementation_reviews::tests::approved_review;

    fn approved() -> ApprovedHistoricalOutcomeFeatureLabelJoinTargetImplementationReview {
    ...
    fn request(
        ...
        RegisterHistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRequest {
            ...
            expected_specification_sha256: spec.specification_sha256.clone(),
            expected_specification_body_sha256: spec.specification_body_sha256.clone(),
            expected_join_specification_sha256: spec.join_specification.specification_sha256.clone(),
            expected_target_specification_sha256: spec.target_specification.specification_sha256.clone(),
            expected_combined_artifact_sha256: spec.combined_artifact_sha256.clone(),
            expected_dataset_content_sha256: spec.dataset_content_sha256.clone(),
            ...
            runner_name: "one-subject deterministic join/target runner".to_string(),
            runner_kind: HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerKind::EphemeralDeterministicProcess,
            runner_code_revision: "oldwang@join-target-runner123".to_string(),
    ...
    pub(crate) fn record() -> HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRecord {
    ...
    fn rehash(value: &mut HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRecord) {
    ...
    fn runner_freezes_runtime_resource_and_zero_ambient_capabilities() {
        ...
        assert_eq!(runner.runner_contract.maximum_memory_mebibytes, 4_096);
    ...
    fn registration_has_no_entrypoint_and_never_runs() {
        ...
        assert!(!runner.join_target_execution_started);
    ...
    fn any_artifact_sandbox_or_downstream_authority_tamper_fails_closed() {
        ...
        runner.runner_contract.label_store_reads_allowed = true;
        rehash(&mut runner);
        assert!(validate_record(&runner).is_err());
        runner.runner_contract = runner_contract();
