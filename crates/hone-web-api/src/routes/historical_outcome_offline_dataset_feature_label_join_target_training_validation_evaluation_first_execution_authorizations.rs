//! Stage 62 append-only independent first-execution review chain for one exact validation runner.
//!
...
//! process, reads no validation data, evaluates or selects no candidate and grants no reward, shadow,
...
use super::historical_outcome_offline_dataset_feature_label_join_target_training_validation_evaluation_isolated_runners::{
    HistoricalOutcomeValidationEvaluationIsolatedRunnerRecord,
    first_execution_authorization_review_eligible_validation_evaluation_runners,
};
use crate::state::AppState;

const REGISTRY_SCHEMA_VERSION: &str =
    "hone-historical-outcome-validation-evaluation-first-execution-authorization-registry-v1";
const REVIEW_SCHEMA_VERSION: &str =
    "hone-historical-outcome-validation-evaluation-first-execution-authorization-review-v1";
const POLICY_VERSION: &str =
    "hone-historical-outcome-validation-evaluation-first-execution-authorization-v1-single-use-24h";
...
pub(crate) enum HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationVerdict {
    ApprovedForOneFutureIsolatedValidationEvaluationInvocation,
    ChangesRequested,
    Rejected,
}

#[derive(Debug, Deserialize)]
pub(crate) struct ReviewHistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationRequest {
    ...
    fixed_three_arm_three_seed_sixty_five_feature_nine_target_protocol_confirmed: bool,
    validation_only_no_training_update_and_sealed_holdout_isolation_confirmed: bool,
    exact_read_only_validation_and_candidate_mounts_and_no_other_data_access_confirmed: bool,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub(crate) struct HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationReview {
    ...
    pub verdict: HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationVerdict,
    ...
    pub fixed_three_arm_three_seed_sixty_five_feature_nine_target_protocol_confirmed: bool,
    pub validation_only_no_training_update_and_sealed_holdout_isolation_confirmed: bool,
    pub exact_read_only_validation_and_candidate_mounts_and_no_other_data_access_confirmed: bool,
    ...
    pub one_future_isolated_validation_evaluation_invocation_authorized: bool,
...
pub(crate) struct HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationItem {
    pub runner: HistoricalOutcomeValidationEvaluationIsolatedRunnerRecord,
    pub current_binding: bool,
    pub latest_review:
        Option<HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationReview>,
    pub one_future_isolated_validation_evaluation_invocation_authorized: bool,
...
#[derive(Debug, Clone, Serialize)]
pub(crate) struct HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationRegistry {
    pub schema_version: String,
    pub policy_version: String,
    pub items: Vec<HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationItem>,
...
#[derive(Debug, Clone)]
pub(crate) struct HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationReadinessSummary {
    pub reviewed_runner_count: usize,
    pub approved_runner_count: usize,
    pub unexpired_authorization_count: usize,
    pub one_shot_authorized_count: usize,
...
#[derive(Debug, Clone, Serialize)]
pub(crate) struct ApprovedHistoricalOutcomeValidationEvaluationFirstExecutionAuthorization {
    pub runner: HistoricalOutcomeValidationEvaluationIsolatedRunnerRecord,
    pub review: HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationReview,
}

pub(crate) async fn handle_get_historical_outcome_validation_evaluation_first_execution_authorizations(
    ...
    match build_registry(&state, Utc::now()).await {
        ...
        Err(error) => {
            warn!(%error, "historical outcome validation evaluation first execution authorization registry read failed");
            crate::routes::json_error(
                StatusCode::INTERNAL_SERVER_ERROR,
                "历史结果 validation 评估首次执行授权复核暂时不可用",
            )
        }
    }
}

pub(crate) async fn handle_review_historical_outcome_validation_evaluation_first_execution_authorization(
    State(state): State<Arc<AppState>>,
    headers: HeaderMap,
        ...
        .await
    {
        Ok(_) => match build_registry(&state, Utc::now()).await {
            Ok(value) => Json(value).into_response(),
            Err(error) => {
                warn!(%error, "historical outcome validation evaluation first execution authorization registry reload failed");
                crate::routes::json_error(
                    StatusCode::INTERNAL_SERVER_ERROR,
                    "历史结果 validation 评估首次执行授权复核暂时不可用",
                )
...
pub(crate) async fn historical_outcome_validation_evaluation_first_execution_authorization_readiness_summary(
    state: &AppState,
) -> Result<HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationReadinessSummary, String>
{
    let registry = build_registry(state, Utc::now()).await?;
    Ok(
        HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationReadinessSummary {
...
pub(crate) async fn execution_attempt_eligible_validation_evaluation_authorizations(
    state: &AppState,
    now: DateTime<Utc>,
) -> Result<Vec<ApprovedHistoricalOutcomeValidationEvaluationFirstExecutionAuthorization>, String> {
    let runners =
        first_execution_authorization_review_eligible_validation_evaluation_runners(state).await?;
...
#[allow(dead_code)] // Consumed only by a separately gated Stage 63 execution-attempt audit.
pub(crate) async fn exact_first_execution_authorization_for_validation_evaluation_audit(
    state: &AppState,
    isolated_runner_id: &str,
    review_id: &str,
    review_sha256: &str,
) -> Result<Option<ApprovedHistoricalOutcomeValidationEvaluationFirstExecutionAuthorization>, String>
{
    let runners =
        first_execution_authorization_review_eligible_validation_evaluation_runners(state).await?;
        ...
        .map_err(|error| error.to_string())?
    {
        ...
        let bytes = tokio::fs::read(entry.path())
            .await
            .map_err(|error| error.to_string())?;
        let review: HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationReview =
        ...
        {
            return Err(
                "validation evaluation first execution authorization audit chain is duplicated or misnamed"
                    .to_string(),
            );
        }
        reviews.push(review);
    ...
    let Some(review) = reviews.into_iter().find(|review| {
        review.review_id == review_id
            ...
            && review.one_future_isolated_validation_evaluation_invocation_authorized
    }) else {
        return Ok(None);
    };
    Ok(Some(
        ApprovedHistoricalOutcomeValidationEvaluationFirstExecutionAuthorization { runner, review },
    ))
}
...
async fn build_registry(
    state: &AppState,
    now: DateTime<Utc>,
) -> Result<HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationRegistry, String> {
    let runners =
        first_execution_authorization_review_eligible_validation_evaluation_runners(state).await?;
    ...
    for runner in runners {
        ...
        items.push(
            HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationItem {
                runner,
                current_binding: true,
                latest_review,
                one_future_isolated_validation_evaluation_invocation_authorized: authorized,
    ...
    let approved_runner_count = items
        .iter()
        .filter(|item| item.one_future_isolated_validation_evaluation_invocation_authorized)
        .count();
    ...
    let authorization_status = if items.is_empty() {
        "waiting_for_current_registered_not_run_validation_evaluation_runner"
    } else if execution_attempt_eligible_count > 0 {
        "approved_for_one_future_isolated_validation_evaluation_invocation_not_started"
    ...
    Ok(HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationRegistry {
        ...
        scope: "第 62 阶段只由完整 Stage 61/60/59/58/57 链之外的独立复核者，对精确 runner、九个候选工件和冻结统计协议授予 24 小时内、最多一次的未来隔离 validation 评估调用资格。授权记录没有 claim 或调用入口，不挂载或读取 validation 标签、不执行评估、不选择候选、不创建输出；sealed holdout、模型/指标库、奖励、影子、订单、券商和交易仍关闭。".to_string(),
    })
}
...
) -> Result<HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationReview, String> {
    if !valid_id(isolated_runner_id) {
        return Err("validation 评估隔离 runner ID 无效".to_string());
    }
    let _lock = acquire_lock(
        state,
        &format!("historical-outcome-validation-evaluation-first-execution-{isolated_runner_id}"),
    )
    .await?;
    let runner = first_execution_authorization_review_eligible_validation_evaluation_runners(state)
        .await?
        .into_iter()
        .find(|runner| runner.isolated_runner_id == isolated_runner_id)
        .ok_or_else(|| "当前没有这条绑定有效且可复核的 validation 评估隔离 runner".to_string())?;
...
fn new_review(
    ...
    latest: Option<&HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationReview>,
    reviewer_id: &str,
    request: ReviewHistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationRequest,
    ...
    let mut review = HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationReview {
        ...
        future_exact_read_only_validation_and_candidate_mounts_confirmed: request
            .future_exact_read_only_validation_and_candidate_mounts_confirmed,
        ...
        fixed_three_arm_three_seed_sixty_five_feature_nine_target_protocol_confirmed: request
            .fixed_three_arm_three_seed_sixty_five_feature_nine_target_protocol_confirmed,
        validation_only_no_training_update_and_sealed_holdout_isolation_confirmed: request
            .validation_only_no_training_update_and_sealed_holdout_isolation_confirmed,
        exact_read_only_validation_and_candidate_mounts_and_no_other_data_access_confirmed: request
            .exact_read_only_validation_and_candidate_mounts_and_no_other_data_access_confirmed,
        ...
        one_future_isolated_validation_evaluation_invocation_authorized: false,
    ...
    let approved = review.verdict
        == HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationVerdict::ApprovedForOneFutureIsolatedValidationEvaluationInvocation;
    review.one_future_isolated_validation_evaluation_invocation_authorized =
...
fn validate_expected_binding(
    ...
    if runner.isolated_runner_spec_sha256 != request.expected_isolated_runner_spec_sha256
        ...
        || runner.implementation.upstream_validation.validation_sha256
            != request.expected_upstream_validation_sha256
        || runner.implementation.upstream_validation.output_sha256
...
fn review_all_checks(
    review: &HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationReview,
) -> bool {
    review.exact_runner_and_complete_upstream_binding_confirmed
        ...
        && review.future_exact_read_only_validation_and_candidate_mounts_confirmed
        ...
        && review.fixed_three_arm_three_seed_sixty_five_feature_nine_target_protocol_confirmed
        && review.validation_only_no_training_update_and_sealed_holdout_isolation_confirmed
        && review.exact_read_only_validation_and_candidate_mounts_and_no_other_data_access_confirmed
...
fn validate_review(
    ...
    let authority_closed = !review.authorization_claimed
        ...
        && !review.validation_feature_access_authorized
    ...
    if review.schema_version != REVIEW_SCHEMA_VERSION
        ...
        || !authority_closed
    {
        return Err("historical outcome validation evaluation first execution authorization is invalid or over-authorized".to_string());
    }
    Ok(())
}
...
async fn latest_review(
    state: &AppState,
    runner: &HistoricalOutcomeValidationEvaluationIsolatedRunnerRecord,
) -> Result<Option<HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationReview>, String>
{
    let directory = review_directory(state, runner);
    let mut entries = match tokio::fs::read_dir(&directory).await {
        Ok(value) => value,
        ...
        .map_err(|error| error.to_string())?
    {
        ...
        {
            return Err(
                "validation evaluation first execution authorization filename mismatch".to_string(),
            );
        }
        if !ids.insert(review.review_id.clone()) || !hashes.insert(review.review_sha256.clone()) {
            return Err(
                "validation evaluation first execution authorization contains duplicates"
                    .to_string(),
            );
        }
        reviews.push(review);
...
fn latest_review_from_records(
    reviews: &[HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationReview],
) -> Result<Option<HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationReview>, String>
{
    if reviews.is_empty() {
        return Ok(None);
    ...
    if reviews.iter().any(|review| {
        ...
        return Err(
            "validation evaluation first execution authorization approval must terminate the chain"
                .to_string(),
        );
    }
    if tips.len() != 1 {
        return Err(
            "validation evaluation first execution authorization chain has invalid tips"
                .to_string(),
        );
    }
    ...
    while let Some(review) = cursor {
        if !visited.insert(review.review_id.clone()) {
            return Err(
                "validation evaluation first execution authorization chain has a cycle".to_string(),
            );
        }
        cursor = match (&review.previous_review_id, &review.previous_review_sha256) {
            (Some(previous_id), Some(previous_sha256)) => {
                let previous = by_id.get(previous_id.as_str()).ok_or_else(|| {
                    "validation evaluation first execution authorization chain is disconnected"
                        .to_string()
                })?;
                if previous.review_sha256 != *previous_sha256 {
                    return Err(
                        "validation evaluation first execution authorization previous hash mismatch".to_string(),
                    );
                }
                Some(*previous)
            }
            ...
            _ => {
                return Err(
                    "validation evaluation first execution authorization previous link is invalid"
                        .to_string(),
                );
            }
        };
    ...
    if visited.len() != reviews.len() {
        return Err(
            "validation evaluation first execution authorization chain is disconnected".to_string(),
        );
    }
    for review in reviews {
        ...
        {
            return Err(
                "validation evaluation first execution authorization actor chain mismatch"
                    .to_string(),
            );
        }
    }
...
fn review_directory(
    ...
    decision_root(state)
        .join("historical-outcome-validation-evaluation-first-execution-authorization-reviews")
...
async fn acquire_lock(state: &AppState, name: &str) -> Result<ReviewLock, String> {
    ...
    {
        return Err(
            "invalid validation evaluation first execution authorization lock name".to_string(),
        );
    }
...
pub(crate) mod tests {
    use super::*;
    use crate::routes::historical_outcome_offline_dataset_feature_label_join_target_training_validation_evaluation_isolated_runners::tests::record as runner_record;
    ...
    fn request(
        ...
        ReviewHistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationRequest {
            ...
            expected_upstream_validation_sha256: runner
                .implementation
                .upstream_validation
                .validation_sha256
                .clone(),
            expected_upstream_output_sha256: runner
                .implementation
                .upstream_validation
                .output_sha256
                .clone(),
            ...
            future_exact_read_only_validation_and_candidate_mounts_confirmed: all_checks,
    ...
    fn approved_review(
        runner: HistoricalOutcomeValidationEvaluationIsolatedRunnerRecord,
        submitted_at: DateTime<Utc>,
    ) -> HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationReview {
        let request = request(
            &runner,
            HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationVerdict::ApprovedForOneFutureIsolatedValidationEvaluationInvocation,
            true,
    ...
    fn rehash(review: &mut HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationReview) {
    ...
    fn approval_is_single_use_24h_and_still_does_not_execute() {
        ...
        assert!(review.one_future_isolated_validation_evaluation_invocation_authorized);
        ...
        assert!(!review.validation_feature_access_authorized);
        assert!(!review.validation_label_access_authorized);
    ...
    fn exact_runner_artifact_and_upstream_binding_are_required() {
        ...
        let mut runner_drift_request = request(
            &runner,
            HistoricalOutcomeValidationEvaluationFirstExecutionAuthorizationVerdict::ChangesRequested,
            false,
            None,
        );
    ...
    #[test]
    fn exact_candidate_set_upstream_and_independent_audits_are_required() {
        let runner = runner_record();
        ...
        target_drift_request.expected_candidate_set_sha256 = "c".repeat(64);
    ...
    #[test]
    fn runner_and_authorization_keep_validation_data_closed_until_execution_claim() {
        let runner = runner_record();
        assert!(!runner.runner_contract.validation_features_access_allowed);
        assert!(!runner.runner_contract.validation_labels_access_allowed);
