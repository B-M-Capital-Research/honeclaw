//! Append-only independent first-execution review chain for one exact feature-label join/target
//! runner.
//!
...
//! process, opens no generic label store, creates no output and grants no downstream manifest,
//! bundle, joined-row, training, reward, shadow, order, broker or trading authority.
...
use super::historical_outcome_offline_dataset_feature_label_join_target_isolated_runners::{
    HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRecord,
    first_execution_authorization_review_eligible_join_target_runners,
};
use crate::state::AppState;

const REGISTRY_SCHEMA_VERSION: &str =
    "hone-historical-outcome-feature-label-join-target-first-execution-authorization-registry-v1";
const REVIEW_SCHEMA_VERSION: &str =
    "hone-historical-outcome-feature-label-join-target-first-execution-authorization-review-v1";
const POLICY_VERSION: &str = "hone-historical-outcome-feature-label-join-target-first-execution-authorization-v1-single-use-24h";
...
pub(crate) enum HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationVerdict {
    ApprovedForOneFutureIsolatedJoinTargetInvocation,
    ChangesRequested,
    Rejected,
}

#[derive(Debug, Deserialize)]
pub(crate) struct ReviewHistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationRequest {
    ...
    deterministic_one_to_one_join_nine_target_and_canonical_schema_contract_confirmed: bool,
    point_in_time_missingness_purge_embargo_split_and_sealed_holdout_confirmed: bool,
    no_generic_label_or_training_store_access_confirmed: bool,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub(crate) struct HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationReview {
    ...
    pub runner: HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRecord,
    pub submitted_at: DateTime<Utc>,
    pub authorization_valid_until: DateTime<Utc>,
    pub reviewer_id: String,
    pub excluded_prior_actor_ids: Vec<String>,
    pub verdict: HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationVerdict,
    ...
    pub deterministic_one_to_one_join_nine_target_and_canonical_schema_contract_confirmed: bool,
    pub point_in_time_missingness_purge_embargo_split_and_sealed_holdout_confirmed: bool,
    pub no_generic_label_or_training_store_access_confirmed: bool,
    ...
    pub one_future_isolated_join_target_invocation_authorized: bool,
    pub authorization_claimed: bool,
    pub invocation_endpoint_available: bool,
    pub join_target_execution_started: bool,
...
pub(crate) struct HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationItem {
    pub runner: HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRecord,
    pub current_binding: bool,
    pub latest_review:
        Option<HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationReview>,
    pub one_future_isolated_join_target_invocation_authorized: bool,
...
#[derive(Debug, Clone, Serialize)]
pub(crate) struct HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationRegistry {
    pub schema_version: String,
    pub policy_version: String,
    pub items: Vec<HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationItem>,
...
#[derive(Debug, Clone)]
pub(crate) struct HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationReadinessSummary
{
    pub review_eligible_runner_count: usize,
    pub reviewed_runner_count: usize,
    pub approved_runner_count: usize,
...
#[derive(Debug, Clone, Serialize)]
pub(crate) struct ApprovedHistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorization {
    pub runner: HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRecord,
    pub review: HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationReview,
}

pub(crate) async fn handle_get_historical_outcome_offline_dataset_feature_label_join_target_first_execution_authorizations(
    ...
    match build_registry(&state, Utc::now()).await {
        ...
        Err(error) => {
            warn!(%error, "historical outcome feature-label join/target first execution authorization registry read failed");
            crate::routes::json_error(
                StatusCode::INTERNAL_SERVER_ERROR,
                "历史结果join/target 隔离首次执行授权复核暂时不可用",
            )
        }
    }
}

pub(crate) async fn handle_review_historical_outcome_offline_dataset_feature_label_join_target_first_execution_authorization(
    State(state): State<Arc<AppState>>,
    headers: HeaderMap,
        ...
        .await
    {
        Ok(_) => match build_registry(&state, Utc::now()).await {
            Ok(value) => Json(value).into_response(),
            Err(error) => {
                warn!(%error, "historical outcome feature-label join/target first execution authorization registry reload failed");
                crate::routes::json_error(
                    StatusCode::INTERNAL_SERVER_ERROR,
                    "历史结果join/target 隔离首次执行授权复核暂时不可用",
                )
...
pub(crate) async fn historical_outcome_offline_dataset_feature_label_join_target_first_execution_authorization_readiness_summary(
    state: &AppState,
) -> Result<
    HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationReadinessSummary,
    String,
> {
...
pub(crate) async fn execution_attempt_eligible_join_target_authorizations(
    state: &AppState,
    now: DateTime<Utc>,
) -> Result<Vec<ApprovedHistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorization>, String>
{
    let runners = first_execution_authorization_review_eligible_join_target_runners(state).await?;
...
pub(crate) async fn exact_first_execution_authorization_for_join_target_audit(
    state: &AppState,
    isolated_runner_id: &str,
    review_id: &str,
    review_sha256: &str,
) -> Result<
    Option<ApprovedHistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorization>,
    String,
> {
    let runners = first_execution_authorization_review_eligible_join_target_runners(state).await?;
        ...
        .map_err(|error| error.to_string())?
    {
        ...
        let bytes = tokio::fs::read(entry.path())
            .await
            .map_err(|error| error.to_string())?;
        let review: HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationReview =
        ...
        {
            return Err(
                "feature-label join/target first execution authorization audit chain is duplicated or misnamed"
                    .to_string(),
            );
        }
        reviews.push(review);
    ...
    let Some(review) = reviews.into_iter().find(|review| {
        review.review_id == review_id
            ...
            && review.one_future_isolated_join_target_invocation_authorized
    }) else {
        return Ok(None);
    };
    Ok(Some(
...
async fn build_registry(
    state: &AppState,
    now: DateTime<Utc>,
) -> Result<HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationRegistry, String> {
    let runners = first_execution_authorization_review_eligible_join_target_runners(state).await?;
    ...
    for runner in runners {
        ...
        let authorized = latest_review
            .as_ref()
            .is_some_and(|review| review.one_future_isolated_join_target_invocation_authorized);
        let unexpired = latest_review
            .as_ref()
            .is_some_and(|review| authorization_unexpired(review, now));
        items.push(
            HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationItem {
                runner,
                current_binding: true,
                latest_review,
                one_future_isolated_join_target_invocation_authorized: authorized,
    ...
    let approved_runner_count = items
        .iter()
        .filter(|item| item.one_future_isolated_join_target_invocation_authorized)
        .count();
    ...
    let authorization_status = if items.is_empty() {
        "waiting_for_current_registered_not_run_isolated_join_target_runner"
    } else if execution_attempt_eligible_count > 0 {
        "approved_for_one_future_isolated_join_target_invocation_not_started"
    ...
    Ok(HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationRegistry {
        ...
        join_target_execution_started: false,
        ...
        scope: "第 41 阶段只由独立复核者对精确 runner、工件和完整上游链授予 24 小时内、最多一次的未来 join/target 隔离调用资格。授权记录没有调用或 claim 入口，不读取通用标签库、不执行、不生成输出；joined rows、输出校验、训练、奖励、影子、订单、券商和交易仍关闭。".to_string(),
    })
}
...
) -> Result<HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationReview, String> {
    if !valid_id(isolated_runner_id) {
        return Err("join/target 隔离 runner ID 无效".to_string());
    }
    let _lock = acquire_lock(
        state,
        &format!("historical-outcome-join-target-first-execution-{isolated_runner_id}"),
    )
    .await?;
    let runner = first_execution_authorization_review_eligible_join_target_runners(state)
        .await?
        .into_iter()
        .find(|runner| runner.isolated_runner_id == isolated_runner_id)
        .ok_or_else(|| "当前没有这条绑定有效且可复核的join/target 隔离 runner".to_string())?;
...
fn new_review(
    ...
    latest: Option<&HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationReview>,
    reviewer_id: &str,
    request: ReviewHistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationRequest,
    ...
    let mut review = HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationReview {
        ...
        deterministic_one_to_one_join_nine_target_and_canonical_schema_contract_confirmed: request.deterministic_one_to_one_join_nine_target_and_canonical_schema_contract_confirmed,
        point_in_time_missingness_purge_embargo_split_and_sealed_holdout_confirmed: request.point_in_time_missingness_purge_embargo_split_and_sealed_holdout_confirmed,
        no_generic_label_or_training_store_access_confirmed: request.no_generic_label_or_training_store_access_confirmed,
        ...
        one_future_isolated_join_target_invocation_authorized: false,
    ...
    let approved = review.verdict
        == HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationVerdict::ApprovedForOneFutureIsolatedJoinTargetInvocation;
    review.one_future_isolated_join_target_invocation_authorized =
...
fn validate_expected_binding(
    ...
    let specification_review = &runner.implementation.approved_review;
    ...
    if runner.isolated_runner_spec_sha256 != request.expected_isolated_runner_spec_sha256
        ...
        || implementation_review.review_contract.contract_sha256
            != request.expected_implementation_review_contract_sha256
        || implementation_review.independent_audit.audit_sha256
            != request.expected_implementation_independent_audit_sha256
        || specification_review.review_id != request.expected_specification_review_id
        || specification_review.review_sha256 != request.expected_specification_review_sha256
        || specification_review.independent_audit.audit_sha256
            != request.expected_specification_review_audit_sha256
        || specification.specification_sha256 != request.expected_specification_sha256
        || specification.specification_body_sha256 != request.expected_specification_body_sha256
        ...
        || specification.combined_artifact_sha256 != request.expected_combined_artifact_sha256
        || specification.dataset_content_sha256 != request.expected_dataset_content_sha256
    {
...
fn review_all_checks(
    ...
    review.exact_runner_and_complete_upstream_binding_confirmed
        ...
        && review.deterministic_one_to_one_join_nine_target_and_canonical_schema_contract_confirmed
        && review.point_in_time_missingness_purge_embargo_split_and_sealed_holdout_confirmed
        && review.no_generic_label_or_training_store_access_confirmed
...
fn validate_review(
    ...
    let authority_closed = !review.authorization_claimed
        ...
        && !review.join_target_execution_started
    ...
    if review.schema_version != REVIEW_SCHEMA_VERSION
        ...
        || !authority_closed
    {
        return Err("historical outcome feature-label join/target first execution authorization is invalid or over-authorized".to_string());
    }
    Ok(())
}
...
async fn latest_review(
    state: &AppState,
    runner: &HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRecord,
) -> Result<Option<HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationReview>, String>
{
    let directory = review_directory(state, runner);
    let mut entries = match tokio::fs::read_dir(&directory).await {
        Ok(value) => value,
        ...
        .map_err(|error| error.to_string())?
    {
        ...
        {
            return Err(
                "feature-label join/target first execution authorization filename mismatch"
                    .to_string(),
            );
        }
        if !ids.insert(review.review_id.clone()) || !hashes.insert(review.review_sha256.clone()) {
            return Err(
                "feature-label join/target first execution authorization contains duplicates"
                    .to_string(),
            );
        }
        reviews.push(review);
...
fn latest_review_from_records(
    reviews: &[HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationReview],
) -> Result<Option<HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationReview>, String>
{
    if reviews.is_empty() {
        return Ok(None);
    ...
    if tips.len() != 1 {
        return Err(
            "feature-label join/target first execution authorization chain has invalid tips"
                .to_string(),
        );
    }
    ...
    while let Some(review) = cursor {
        if !visited.insert(review.review_id.clone()) {
            return Err(
                "feature-label join/target first execution authorization chain has a cycle"
                    .to_string(),
            );
        }
        cursor = match (&review.previous_review_id, &review.previous_review_sha256) {
            (Some(previous_id), Some(previous_sha256)) => {
                let previous = by_id.get(previous_id.as_str()).ok_or_else(|| {
                    "feature-label join/target first execution authorization chain is disconnected".to_string()
                })?;
                if previous.review_sha256 != *previous_sha256 {
                    return Err(
                        "feature-label join/target first execution authorization previous hash mismatch"
                            .to_string(),
                    );
                }
                Some(*previous)
            ...
            _ => {
                return Err(
                    "feature-label join/target first execution authorization previous link is invalid"
                        .to_string(),
                );
            }
        };
    ...
    if visited.len() != reviews.len() {
        return Err(
            "feature-label join/target first execution authorization chain is disconnected"
                .to_string(),
        );
    }
    for review in reviews {
        ...
        {
            return Err(
                "feature-label join/target first execution authorization actor chain mismatch"
                    .to_string(),
            );
        }
    }
...
fn review_directory(
    ...
    decision_root(state)
        .join("historical-outcome-feature-label-join-target-first-execution-authorization-reviews")
...
async fn acquire_lock(state: &AppState, name: &str) -> Result<ReviewLock, String> {
    ...
    {
        return Err(
            "invalid feature-label join/target first execution authorization lock name".to_string(),
        );
    }
    ...
    for attempt in 0..8 {
            ...
            .await
        {
            ...
            Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => {
                ...
                return Err("另一个管理员正在复核该join/target 隔离首次执行授权".to_string());
            }
            Err(error) => return Err(error.to_string()),
        }
    }
    Err("join/target 隔离首次执行授权复核锁不可用".to_string())
}

#[cfg(test)]
pub(crate) mod tests {
    use super::*;
    use crate::routes::historical_outcome_offline_dataset_feature_label_join_target_isolated_runners::tests::record as runner_record;
    ...
    fn request(
        ...
        ReviewHistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationRequest {
            ...
            expected_implementation_review_contract_sha256: implementation_review
                .review_contract
                .contract_sha256
                .clone(),
            expected_implementation_independent_audit_sha256: implementation_review
                ...
                .audit_sha256
                .clone(),
            expected_specification_review_id: specification_review.review_id.clone(),
            expected_specification_review_sha256: specification_review.review_sha256.clone(),
            expected_specification_review_audit_sha256: specification_review
                .independent_audit
                .audit_sha256
                .clone(),
            expected_specification_sha256: specification.specification_sha256.clone(),
            expected_specification_body_sha256: specification
            ...
            deterministic_one_to_one_join_nine_target_and_canonical_schema_contract_confirmed: all_checks,
            point_in_time_missingness_purge_embargo_split_and_sealed_holdout_confirmed: all_checks,
            no_generic_label_or_training_store_access_confirmed: all_checks,
    ...
    fn approved_review(
        runner: HistoricalOutcomeFeatureLabelJoinTargetIsolatedRunnerRecord,
        submitted_at: DateTime<Utc>,
    ) -> HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationReview {
        let request = request(
            &runner,
            HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationVerdict::ApprovedForOneFutureIsolatedJoinTargetInvocation,
            true,
    ...
    fn rehash(
        review: &mut HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationReview,
    ) {
    ...
    fn approval_is_single_use_24h_and_still_does_not_execute() {
        ...
        assert!(review.one_future_isolated_join_target_invocation_authorized);
        ...
        assert!(!review.join_target_execution_started);
        assert!(!review.output_artifact_created);
        assert!(!review.label_access_authorized);
        assert!(!review.feature_join_performed);
        assert!(!review.semantic_target_assigned);
        assert!(!review.joined_dataset_created);
    ...
    fn exact_runner_artifact_and_upstream_binding_are_required() {
        ...
        let mut runner_drift_request = request(
            &runner,
            HistoricalOutcomeFeatureLabelJoinTargetFirstExecutionAuthorizationVerdict::ChangesRequested,
            false,
            None,
        );
    ...
    #[test]
    fn exact_target_specification_and_independent_audits_are_required() {
        let runner = runner_record();
        ...
        target_drift_request.expected_target_specification_sha256 = "c".repeat(64);
    ...
    #[test]
    fn runner_and_authorization_keep_generic_label_and_training_stores_closed() {
        let runner = runner_record();
        assert!(!runner.runner_contract.label_store_reads_allowed);
        assert!(!runner.runner_contract.training_store_reads_allowed);
        let review = approved_review(runner, Utc::now());
        assert!(review.no_generic_label_or_training_store_access_confirmed);
        assert!(!review.label_access_authorized);
    ...
    fn any_claim_execution_output_or_downstream_authority_tamper_fails_closed() {
        ...
        review.label_access_authorized = true;
        rehash(&mut review);
        assert!(validate_review(&review, &review.runner).is_err());
        review.label_access_authorized = false;
        review.joined_dataset_created = true;
        rehash(&mut review);
        assert!(validate_review(&review, &review.runner).is_err());
        review.joined_dataset_created = false;
